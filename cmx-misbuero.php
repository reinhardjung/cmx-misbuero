...
\add_action('admin_menu', static function (): void {
	...
	\remove_menu_page('edit.php?post_type=emails');
}, 1000);
...
function cmx_check_and_create_subdomain_admin() {
	...
	$sub = $parts[0]; // "kunde"
		...
		$smtp_mail_settings = static function () use ($parse_email_list): array {
			...
			$mail_transport_settings = static function () use ($smtp_mail_settings): array {
				...
				if ($context === 'magic_login' && \function_exists(__NAMESPACE__ . '\\cmx_misbuero_support_smtp_settings')) {
					return cmx_misbuero_support_smtp_settings();
				}
			...
			$resolve_mail_sender = static function () use ($sub, $smtp_mail_settings): array {
				...
				$is_magic_login_context = $context === 'magic_login';
				$support_smtp = $is_magic_login_context && \function_exists(__NAMESPACE__ . '\\cmx_misbuero_support_smtp_settings')
					? cmx_misbuero_support_smtp_settings()
					: [];
				$smtp = $is_magic_login_context ? $support_smtp : $smtp_mail_settings();
				$support_email = \function_exists(__NAMESPACE__ . '\\cmx_misbuero_support_email')
					? \sanitize_email((string) cmx_misbuero_support_email())
					: \sanitize_email('support@misbuero.ch');

				$from_email = $is_magic_login_context && \is_email($support_email)
					? $support_email
					: ($is_beleg_context ? (string) ($smtp['alias_belege'] ?? '') : (string) ($smtp['alias_general'] ?? ''));
				...
				$reply_to_email = $is_magic_login_context
					? $support_email
					: \sanitize_email((string) ($smtp['reply'] ?? ''));
				if (!\is_email($reply_to_email)) {
					$reply_to_email = $from_email;
				}
				$reply_to_name = $name;

				$envelope_email = $is_magic_login_context && !empty($support_smtp['enabled']) && \is_email((string) ($support_smtp['email_address'] ?? ''))
					? (string) $support_smtp['email_address']
					: (\is_email((string) ($smtp['email_address'] ?? ''))
	...
	// update_option('blogdescription', 'Der neue Untertitel der Website');
			...
			add_filter('wp_mail', function($args) use ($resolve_mail_sender, $get_configured_bcc, $should_skip_configured_bcc) {
		...
		// Letzte Instanz: PHPMailer selbst überschreiben (falls SMTP-Plugins vorher From setzen)
			add_action('phpmailer_init', function($phpmailer) use ($resolve_mail_sender, $mail_transport_settings, $get_configured_bcc, $should_skip_configured_bcc): void {
