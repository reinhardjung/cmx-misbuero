...
/**
 * ==========================================================
 * Admin-Spalte „Stufe“ in der Kontakte-Liste
 * (Position: direkt vor der Spalte „Karte“)
 * ==========================================================
 */

/**
 * Spalten-Reihenfolge anpassen und Stufe einfügen
 */
\add_filter('manage_edit-kontakte_columns', function (array $columns): array {
	$new = [];
	foreach ($columns as $key => $label) {
		// Vor der Karte-Spalte die Stufen-Spalte einfügen
		if ($key === 'karte') {
			$new['stufen'] = 'Stufe';
		}
		$new[$key] = $label;
	}
	// Falls „karte“ fehlt, Stufe am Ende ergänzen
	if (!isset($new['stufen'])) {
		$new['stufen'] = 'Stufe';
	}
	return $new;
...
/**
 * Zellinhalt für Spalte „Stufe“ ausgeben
 */
\add_action('manage_kontakte_posts_custom_column', function (string $column, int $post_id) {
	if ($column !== 'stufen') return;

	$terms = \wp_get_object_terms($post_id, 'stufen');
	if (\is_wp_error($terms) || empty($terms)) {
	...
	$term = $terms[0];
	$desc = \trim((string) $term->description);
	$label = $term->name . ($desc !== '' ? ' – ' . $desc : '');
	echo '<span>' . \esc_html($label) . '</span>';
}, 10, 2);

/**
 * Sortierbarkeit optional aktivieren (nach Stufenname)
 */
\add_filter('manage_edit-kontakte_sortable_columns', function (array $sortable): array {
	$sortable['stufen'] = 'stufe';
	return $sortable;
});
