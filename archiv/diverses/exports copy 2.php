...
/**
 * Datei: exports.php
 * Export aller "kontakte" als CSV (UTF-8 BOM; Semikolon)
 * Enthält: Stammdaten, Adressen (Rechnung/Liefer), Stufe, Kontakt-Länder (Taxo),
 * Kommunikation (Telefon/E-Mail inkl. Label-Namen)
 * PLUS garantiert: Roh-Metas cmx_datum, _cmx_rechnung_land, _cmx_liefer_land.
 */
...
/* ===== Helper (alle mit Guards) ===== */
if (!function_exists(__NAMESPACE__.'\\cmx_term_name_by_slug')) {
	function cmx_term_name_by_slug(string $tax, string $slug): string {
...
if (!function_exists(__NAMESPACE__.'\\cmx_domain_core_from_url')) {
	function cmx_domain_core_from_url(string $url): string {
		...
		$n = \count($labels);
		if ($n===0) return '';
		$twoPart = ['co.uk','org.uk','ac.uk','gov.uk','com.au','net.au','org.au','gov.au','co.jp','ne.jp','or.jp','com.br','com.ar','com.mx','com.tr','co.nz','org.nz'];
		if ($n>=3) {
...
if (!function_exists(__NAMESPACE__.'\\cmx_read_stammdaten')) {
	function cmx_read_stammdaten(int $post_id): array {
		...
		$url_raw  = (string)\get_post_meta($post_id, CMX_KONTAKTE_META_URL,       true);
		$url_norm = cmx_normalize_url_for_href($url_raw);
		$domain   = $url_norm ? cmx_domain_core_from_url($url_norm) : '';
		// Datum (primär _cmx_kontakte_datum, aber Roh-Export kommt separat)
...
if (!function_exists(__NAMESPACE__.'\\cmx_read_adressen')) {
	function cmx_read_adressen(int $post_id): array {
		$r_land_raw = (string)\get_post_meta($post_id, CMX_RECHNUNG_META_LAND,true);
		$l_land_raw = (string)\get_post_meta($post_id, CMX_LIEFER_META_LAND, true);
		$r = [
			...
			'land_raw' => $r_land_raw,
			'land_label' => cmx_country_label_from_slug($r_land_raw),
		];
		$l = [
			...
			'land_raw' => $l_land_raw,
			'land_label' => cmx_country_label_from_slug($l_land_raw),
		];
...
if (!function_exists(__NAMESPACE__.'\\cmx_read_stufe')) {
	function cmx_read_stufe(int $post_id): array {
		...
		$first = reset($terms);
		return ['id'=>(string)$first->term_id,'slug'=>(string)$first->slug,'name'=>(string)$first->name];
	}
}
if (!function_exists(__NAMESPACE__.'\\cmx_read_laender_terms')) {
	function cmx_read_laender_terms(int $post_id): array {
		$tax = 'kontakte_laender';
		if (!\taxonomy_exists($tax)) return ['ids'=>'','slugs'=>'','names'=>''];
		$terms = \get_the_terms($post_id,$tax);
		if (\is_wp_error($terms) || empty($terms)) return ['ids'=>'','slugs'=>'','names'=>''];
		$ids=$slugs=$names=[];
		foreach ($terms as $t){ $ids[]=(string)$t->term_id; $slugs[]=(string)$t->slug; $names[]=(string)$t->name; }
		return ['ids'=>implode('|',$ids),'slugs'=>implode('|',$slugs),'names'=>implode('|',$names)];
	}
}
...
if (!function_exists(__NAMESPACE__.'\\cmx_render_kontakte_export_page')) {
	function cmx_render_kontakte_export_page(): void {
		...
		echo '<p>CSV ist UTF-8 (BOM), Semikolon-getrennt. Enthält alle relevanten Felder inkl. Roh-Metas <code>cmx_datum</code>, <code>_cmx_rechnung_land</code>, <code>_cmx_liefer_land</code>.</p>';
...
if (!function_exists(__NAMESPACE__.'\\cmx_handle_kontakte_export')) {
...
/* ===== CSV-Streaming ===== */
if (!function_exists(__NAMESPACE__.'\\cmx_stream_kontakte_csv')) {
	function cmx_stream_kontakte_csv(): void {
		...
		$headers = [
			...
			// >>> GENAU DIE DREI ORIGINAL-NAMEN <<<
			'cmx_datum','_cmx_rechnung_land','_cmx_liefer_land',
		];
		fputcsv($out, $headers, ';');

		$paged=1; $per_page=500;
		do{
			$q = new \WP_Query([
				'post_type'=>'kontakte',
			...
			foreach($q->posts as $p){
				...
				$landT = cmx_read_laender_terms($post_id);

				// Roh-Metas explizit (GENAU diese Keys):
				$meta_cmx_datum        = (string)\get_post_meta($post_id, 'cmx_datum', true);
				$meta_rechnung_land    = (string)\get_post_meta($post_id, '_cmx_rechnung_land', true);
				$meta_liefer_land      = (string)\get_post_meta($post_id, '_cmx_liefer_land',  true);
				...
				$row = [
					...
					(string)$addr['rechnung']['land'],        // slug (lower)
					(string)$addr['rechnung']['land_label'],  // Name
					...
					(string)$addr['liefer']['land'],          // slug (lower)
					(string)$addr['liefer']['land_label'],    // Name
