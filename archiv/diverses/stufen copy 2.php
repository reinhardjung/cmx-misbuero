<?php
/**
 * CMX – Kontakte: Taxonomie "Stufen" (A–D) als Single-Select (Radio)
 * + eigene SIDE-Metabox
 * + Admin-Dropdown-Filter
 * + Admin-Spalte „Stufe“ (Core: taxonomy-stufen) direkt vor „Karte“ + Sortierung
 *
...
function cmx_register_tax_stufen(): void {
	...
	\register_taxonomy('stufen', ['kontakte'], [
		...
		'public'                => false,
		'show_ui'               => true,
		'show_admin_column'     => true,   // Core-Spalte taxonomy-stufen
		'show_in_rest'          => false,
		'hierarchical'          => false,
		'meta_box_cb'           => '__return_false', // Standard-Metabox aus
		'query_var'             => 'stufe',
		'rewrite'               => false,
	]);

	// OPTIONAL: Default-Terme
...
function cmx_stufen_box_html(\WP_Post $post): void {
	...
	if (\is_wp_error($terms) || empty($terms)) {
		// OPTIONAL: Defaults on-the-fly anlegen
		// foreach ([['A','a'],['B','b'],['C','c'],['D','d']] as [$name,$slug]) {
		// 	if (!\term_exists($slug, 'stufen')) \wp_insert_term($name, 'stufen', ['slug' => $slug]);
		// }
...
\add_action('pre_get_posts', function (\WP_Query $query) {
	...
	$query->set($query_var, null); // Kollisionen vermeiden
});

/* =========================================================
 * 7) Admin-Spalte „Stufe“ (Core: taxonomy-stufen) VOR „Karte“ positionieren
 *    + Sortierbarkeit über Spaltenkopf (orderby=stufe)
 * ========================================================= */
\add_filter('manage_edit-kontakte_columns', function (array $columns): array {
	// Eigene Alt-Spalte entfernen, falls vorhanden
	unset($columns['stufen']);

	// Core-Spaltenlabel sichern und vorerst entfernen
	$stufen_label = $columns['taxonomy-stufen'] ?? 'Stufe';
	unset($columns['taxonomy-stufen']);

	// „Karte“ ans Ende verschieben
	$karte_label = null;
	if (isset($columns['karte'])) {
		$karte_label = $columns['karte'];
		unset($columns['karte']);
	}

	// Neu aufbauen: bisherige -> taxonomy-stufen -> karte
	$new = [];
	foreach ($columns as $key => $label) {
		$new[$key] = $label;
	}
	$new['taxonomy-stufen'] = $stufen_label;
	if ($karte_label !== null) {
		$new['karte'] = $karte_label;
	}
	return $new;
}, 99);

\add_filter('manage_edit-kontakte_sortable_columns', function (array $sortable): array {
	$sortable['taxonomy-stufen'] = 'stufe';
	return $sortable;
});

/**
 * Sortierung nach Stufen-Name implementieren (JOIN auf Terms)
 */
\add_filter('posts_clauses', function (array $clauses, \WP_Query $q) {
	if (!\is_admin() || !$q->is_main_query()) return $clauses;
	if (($q->get('post_type') ?: '') !== 'kontakte') return $clauses;
	if ($q->get('orderby') !== 'stufe') return $clauses;

	global $wpdb;
	$join  = " LEFT JOIN {$wpdb->term_relationships} cmx_tr ON {$wpdb->posts}.ID = cmx_tr.object_id ";
	$join .= " LEFT JOIN {$wpdb->term_taxonomy}    cmx_tt ON cmx_tr.term_taxonomy_id = cmx_tt.term_taxonomy_id AND cmx_tt.taxonomy = 'stufen' ";
	$join .= " LEFT JOIN {$wpdb->terms}            cmx_t  ON cmx_tt.term_id = cmx_t.term_id ";

	$clauses['join']   .= $join;
	$clauses['groupby'] = "{$wpdb->posts}.ID";

	$direction = (strtoupper((string) $q->get('order')) === 'DESC') ? 'DESC' : 'ASC';
	$clauses['orderby'] = " COALESCE(cmx_t.name, '') {$direction}, {$wpdb->posts}.ID {$direction} ";
