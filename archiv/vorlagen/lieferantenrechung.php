...
<?php
	...
	// >>> NEU: Gesamtbetrag anhand der real angezeigten Zeilensummen berechnen
	$__compute_total = function(array $rows): float {
		$sum = 0.0;
		foreach ($rows as $row) {
			$lt = (float)($row['line_total'] ?? 0);
			// Fallback, wenn Zeile als 0 reinkommt (z. B. negativer Preis nicht übernommen):
			if (abs($lt) < 0.0000001) {
				$qty   = (float)($row['qty'] ?? 0);
...
<?php
// Sichtbarkeit der Spalten dynamisch
...
$colCount  = 1;                   // Positions-Nr.
$colCount += $showArtNr ? 1 : 0;  // Artikel-Nr.
...
<table>
	...
	<?php if (!empty($tpl['positions'])): ?>
		<?php $i = 0; foreach ($tpl['positions'] as $row): $z = ($i % 2 === 0) ? 'zebra-even' : 'zebra-odd'; $posNr = $i + 1; ?>
			<tr class="position <?= $z ?>">
				...
				<!-- Beschreibung (Spalte 3) = Belegtext aus CPT „Artikel“ -->
				<td>
					<?php
				...
				<td class="text-right">
					<?php
						$lt = (float)($row['line_total'] ?? 0);
						if (abs($lt) < 0.0000001) {
							// Fallback: aus Menge * Einzelpreis neu berechnen, falls 0 hereinkommt
							$qty   = (float)($row['qty'] ?? 0);
							$upris = (float)($row['unit_price'] ?? 0);
							$calc  = $qty * $upris;
							if (abs($calc) > 0.0000001) $lt = $calc;
	...
	<!-- TOTAL (aus den oben angezeigten Zeilen neu berechnet) -->
