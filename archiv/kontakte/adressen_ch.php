...
/**
 * ==========================================================
 * Adress-Metabox (Rechnung/Lieferung)
 * - Land-Select befüllt aus Taxonomie "kontakte_laender"
 * - Layout: Strasse | Zusatz | Land (schmal) | PLZ (schmal) | Ort
 * - Sichtbares Suchfeld „Adresse suchen (CH)“
	...
	$render_tab = static function(string $key, array $vals, array $countries): void {
		...
		echo '<div class="cmx-ac-wrap">
			 <!--- <label><strong>Adresse suchen (CH)</strong></label><br> --->
				<input type="text" class="cmx-ac-input" placeholder="'.esc_attr($key === 'liefer' ? 'Adresse suchen (Lieferung)…' : 'Adresse suchen (Rechnung)…').'" aria-label="Adresse suchen (CH)">
			  </div>';
...
function cmx_addr_ac_enqueue($hook): void {
	...
	$countries = cmx_countries_from_taxonomy();
	$chSlug    = cmx_countries_default_slug($countries, 'ch'); // falls "ch" nicht existiert → erster Term
