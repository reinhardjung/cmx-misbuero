...
/**
 * Helper: Kundenart-Taxonomie ermitteln (kundenart | kontakt_type)
 */
function cmx_kundenart_tax(): string {
	if (\taxonomy_exists('kundenart'))    return 'kundenart';
	if (\taxonomy_exists('kontakt_type')) return 'kontakt_type';
	return 'kontakt_type';
}
...
/* ==========================================================
 * Flexible Quellen (Kommunikation)
 * ========================================================== */
...
function cmx_meta_keys_tel_1(): array {
	...
	$list = array_merge($list, ['cmx_tel_1','telefon_1','tel_1','phone_1','telefon','tel','phone']);
...
function cmx_meta_keys_email_1(): array {
	...
	$list = \defined($c) ? [\constant($c)] : [];
	$list = array_merge($list, ['cmx_email_1','email_1','e_mail_1','kontakt_email','email','e_mail','mail']);
...
function cmx_kontakte_custom_column($column, $post_id) {
	...
	if ($column === 'cmx_kategorie') {
		...
		if (!\taxonomy_exists($tax)) { echo '<span style="color:#999;">–</span>'; return; }

		$terms = \get_the_terms($post_id, $tax);
		if (\is_wp_error($terms) || empty($terms)) { echo '<span style="color:#999;">–</span>'; return; }
	...
	// Kundenart (klickbar)
	if ($column === 'kontakt_type') {
		$tax   = cmx_kundenart_tax();
		$terms = \get_the_terms($post_id, $tax);
		if (\is_wp_error($terms) || empty($terms)) { echo '<span style="color:#999;">–</span>'; return; }
		...
		foreach ($terms as $t) {
			$url = \add_query_arg(
				['post_type' => 'kontakte', 'filter_kontakt_type' => (string) $t->term_id],
				admin_url('edit.php')
			);
			$links[] = '<a href="' . esc_url($url) . '" title="Nach &bdquo;'.esc_attr($t->name).'&ldquo; filtern">' . esc_html($t->name) . '</a>';
		}
	...
	if ($column === 'cmx_tel_1') {
		...
		if ($val === '') $val = cmx_get_first_meta_value($post_id, cmx_meta_keys_tel_1());
		if ($val === '') { echo '<span style="color:#999;">–</span>'; return; }
	...
	if ($column === 'cmx_email_1') {
		...
		if ($val === '') $val = cmx_get_first_meta_value($post_id, cmx_meta_keys_email_1());
		if ($val === '') { echo '<span style="color:#999;">–</span>'; return; }
	...
	if ($column === 'cmx_gmaps') {
		...
		if ($addr === '') { echo '<span style="color:#999;">–</span>'; return; }
...
function cmx_kontakte_tax_filters() {
	...
	cmx_render_tax_filter_dropdown(taxonomy:$kundenart_tax, param:'filter_kontakt_type', label:'Alle Kundenarten');

	// NEU: Kundenkategorie
	$kundkat_tax = cmx_kundenkategorie_tax();
	cmx_render_tax_filter_dropdown(taxonomy:$kundkat_tax, param:'filter_kundenkategorie', label:'Alle Kundenkategorien');
