...
if (!function_exists(__NAMESPACE__ . '\\cmxbu_get_me_contact')) {
	function cmxbu_get_me_contact(): array {
		...
		// Kommunikationsdaten: zuerst flache Metakeys, dann Array-Fallback
		$telefon_1 = get_post_meta($id, 'cmx_kommunikation[telefon_1]', true);
		$email_1   = get_post_meta($id, 'cmx_kommunikation[email_1]', true);

		$kommunikation = get_post_meta($id, 'cmx_kommunikation', true);
		if (!is_array($kommunikation)) $kommunikation = [];

		if ($telefon_1 === '' && isset($kommunikation['telefon_1'])) {
			$telefon_1 = $kommunikation['telefon_1'];
		}
		if ($email_1 === '' && isset($kommunikation['email_1'])) {
			$email_1 = $kommunikation['email_1'];
		}
		...
		$land    = cmxbu_meta_first($id, ['_land', '_country', 'land', 'country'], 'CH');

		// einfache Aufbereitung
		$telefon_1 = is_array($telefon_1) ? trim((string)reset($telefon_1)) : trim((string)$telefon_1);
		$email_1   = is_array($email_1)   ? trim((string)reset($email_1))   : trim((string)$email_1);
...
if (!function_exists(__NAMESPACE__.'\\cmxbu_get_beleg_positionen_calc')) {
	function cmxbu_get_beleg_positionen_calc(int $post_id, array $opts=[]): array {
		...
		foreach ($rows as $i=>$r) {
			...
			$title      = $artikel_id ? get_the_title($artikel_id) : ($to_str($r['artikel_name'] ?? ''));
			$artnr      = '';
...
add_action('save_post_belege', __NAMESPACE__.'\\cmxbu_generate_document_on_save', 999, 3);
	function cmxbu_generate_document_on_save(int $post_id, \WP_Post $post, bool $update): void {
		...
		$me    = cmxbu_get_me_contact();
		var_dump(cmxbu_get_me_contact()); exit;
