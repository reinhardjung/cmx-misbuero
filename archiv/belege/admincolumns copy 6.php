<?php
/**
 * Datei: src/belege/admin-list-kontakte-projekte.php (NEU, CLEAN)
 * Funktionsumfang:
 *  - Ein einziges, sauberes Filter-UI oben: Kategorie, Projekt (Tax ODER Meta), Kontakt
 *  - Klick in den Spalten:
 *      • Kategorie → filtert die Liste
 *      • Projekt   → filtert die Liste (Tax ODER Meta)
 *      • Kontakt   → öffnet die Bearbeiten-Seite des Kontakts (wenn ID vorhanden)
 *  - Alte/duplizierte Filter werden ausgeblendet (DOM-Selektor-basiert)
 */
...
if (!function_exists(__NAMESPACE__.'\\cmx_used_contact_ids')) {
	function cmx_used_contact_ids(): array {
		...
		if (\post_type_exists(cmx_kontakte_cpt())) {
			...
			foreach ($kontakt_ids as $id) $ids[(int)$id] = true;
		}
...
/** =========================
 * DUPLIKATE Filter-UI ausblenden (falls Alt-Code noch hooked)
 * ========================= */
\add_action('admin_head', function(){
	...
	?>
	<style>
		/* Zeige nur unsere Filter-Box */
		#cmx-filters-wrap select[name="cmx_proj"],
		#cmx-filters-wrap select[name="<?php echo esc_attr(cmx_tax_beleg_projekt()); ?>"],
		#cmx-filters-wrap select[name="cmx_kontakt"] { display: inline-block; }
		/* Verberge etwaige doppelte Filter gleicher Namen ausserhalb unserer Box */
		select[name="cmx_proj"]:not(#cmx-filters-wrap select[name="cmx_proj"]),
		select[name="cmx_kontakt"]:not(#cmx-filters-wrap select[name="cmx_kontakt"]) { display: none !important; }
	</style>
	<?php
});
...
\add_action('restrict_manage_posts', function($post_type = '', $which = ''){
	...
	// Projekt (Tax ODER Meta-Fallback – Meta nur, wenn KEINE Tax verfügbar)
...
\add_action('pre_get_posts', function($q){
	...
	if (!empty($_GET['cmx_proj'])) {
		...
		if (strpos($val, 'id:') === 0) {
			...
			$or = ['relation' => 'OR'];
			foreach (cmx_meta_projekt_ids() as $k) { $or[] = ['key'=>$k, 'value'=>$pid, 'compare'=>'=']; }
			foreach (cmx_meta_projekt_txts() as $k) { $or[] = ['key'=>$k, 'value'=>$pid, 'compare'=>'=']; } // falls ID als Text gespeichert
			$meta_query[] = $or;
		} elseif (strpos($val, 'txt:') === 0) {
			$txt = substr($val, 4);
			$or  = ['relation' => 'OR'];
			foreach (cmx_meta_projekt_txts() as $k) { $or[] = ['key'=>$k, 'value'=>$txt, 'compare'=>'=']; }
			$meta_query[] = $or;
		}
	}

	// Kontakt (ID, OR über alle Kontakt-Keys)
	if (!empty($_GET['cmx_kontakt']) && (int)$_GET['cmx_kontakt'] > 0) {
		$id = (string)(int)$_GET['cmx_kontakt'];
		$or = ['relation' => 'OR'];
		foreach (cmx_meta_kontakt_ids() as $k) { $or[] = ['key'=>$k, 'value'=>$id, 'compare'=>'=']; }
		$meta_query[] = $or;
	}
...
/** =========================
 * Spalten-Render: klickbare Links/FILTER-Links
 * ========================= */
\add_action('manage_'.'belege'.'_posts_custom_column', function($col, $post_id){

	// Kontakt → Bearbeiten-Link (wenn ID), sonst erste Zeile aus Adresse
	...
	// Projekt → Tax: Listen-Filter-Link | Meta: id:/txt: Filter-Link
