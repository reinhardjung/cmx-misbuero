...
// var_dump($dates['currency']); exit;
		...
		try{
			...
			if ($page_count > 1) {
					...
					$y = 790;   // Y-Position (tiefer = weiter unten, höherer Wert = näher am Seitenrand unten)
...
// Nur Folgeseiten (Seite > 1) mit Kopf
$canvas->page_script(function($pageNum, $pageCount, $canvas, $fontMetrics) use ($beleg_titel) {
    if ($pageNum > 1) {
        ...
        // Kopftext ausgeben (z. B. Belegtitel)
        $canvas->text(
            50,    // X-Position
            55,    // Y-Position – oberhalb des Inhalts, unterhalb Seitenrand
            $beleg_titel,
            $font,
            11,
            [0.1, 0.1, 0.1]
        ...
        // Feine Linie unterhalb des Kopfbereichs
