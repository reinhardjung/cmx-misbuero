...
function cmx_render_beleg_metabox(\WP_Post $post): void {
	...
	echo '<p><label id="cmx_label_projekt" data-edit="'.\esc_attr($proj_edit_link).'" data-list="'.\esc_attr($proj_list_link).'" style="cursor:pointer" target="_blank" title="Zum Projekt-Modul springen oder Eintrag editieren"><strong>Projekt</strong></label><br>';
		...
		(function(){
			...
			function wireLabel(lblId, hiddenId, listUrl){
				...
				lbl.addEventListener("click", function(e){
					...
					/* Wenn Eintrag existiert → direkt Edit-Ansicht, sonst Listenansicht des Moduls */
					if(id){
						/* Edit-Link steckt im data-edit (serverseitig gesetzt) */
						const edit=this.getAttribute("data-edit")||"";
						if(edit){ window.location.href=edit; return; }
					}
					window.location.href = this.getAttribute("data-list") || listUrl || "#";
				});
			}

			/* Projektschild & Kontaktschild verdrahten */
