...
if (!function_exists(__NAMESPACE__ . '\cmx_get_artikel_nr')) {
	function cmx_get_artikel_nr($post_id){
		...
		// => ignorieren (das ist kein Wert), aber zur Sicherheit checken,
		// ob evtl. im Unterstrich-Feld doch ein echter Wert steckt.
		if ($nr === '' || $nr === null) {
...
/* ------------------------------
 * AJAX: Artikel-Suche (Label = ARTNR – Titel) inkl. SKU-Suche
 * ------------------------------ */
add_action('wp_ajax_cmx_search_artikel', function() {
	...
	$args = [
		'post_type'      => 'artikel',
		'post_status'    => 'any',           // auch Entwürfe etc.
...
/* ------------------------------
 * JS – Eigene Artikelliste (SKU + Titel) mit Pfeiltasten (↑/↓/Enter/Esc)
 * jQuery UI Autocomplete wird explizit deaktiviert.
 * ------------------------------ */
	...
	jQuery(function($){
		...
		function makeNavigator(inputEl, listEl, chooseCb){
			...
			inputEl.addEventListener('keydown', e=>{
				// ↓ öffnet bei Bedarf die Liste + setzt ersten Eintrag aktiv
				if(e.key==='ArrowDown'){
		...
		function fetchArtikel(term, cb){
			$.getJSON(AJAX_URL, { action:'cmx_search_artikel', term: term||'' }, function(data){
				// Endpoint liefert [{label, value, nr, title}, ...]
		...
		function initArtikelSuggest($ctx){
			$ctx.find('.cmx-artikel-autocomplete').each(function(){
				...
				// 1) jQuery UI Autocomplete hart abschalten (falls aktiv)
				try{
					if($.ui && $.ui.autocomplete && $input.data('ui-autocomplete')){
						$input.autocomplete('destroy');
					}
				}catch(e){}
				$input.off('.autocomplete'); // eventuelle Restevents lösen

				if($input.data('cmx-suggest-ready')) return;

				// 2) eigene Liste einfügen (relativ zur Zelle)
				...
				function doSearch(q){
					fetchArtikel(q, (rows)=>{ nav.render(rows); });
				}
				...
				// Fokus/Click → Liste (auch leere Suche)
