...
\add_action('restrict_manage_posts', function (string $post_type, string $which) {
	...
	echo '<option value="-1"'.selected($current, '-1', false).'>— Kein Lieferant —</option>';
...
\add_action('parse_query', function (\WP_Query $q) {
	...
	/* ---- Lieferanten-Filter ---- */
	// WICHTIG: Rohwert lesen, um "-1" (Kein Lieferant) sauber zu erkennen.
	$lieferant_raw = isset($_GET['cmx_lieferant']) ? (string)$_GET['cmx_lieferant'] : '0';
	$lieferant     = absint($lieferant_raw);

	/* A) explizit: KEIN Lieferant */
	if ($lieferant_raw === '-1') {
		...
		// Danach nicht in den "> 0"-Zweig fallen
	} elseif ($lieferant > 0) {
		/* B) konkreter Lieferant (ID) */
		$meta_query = (array)$q->get('meta_query');
		$meta_query[] = [
