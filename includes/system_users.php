...
if (!\function_exists(__NAMESPACE__ . '\\cmx_system_assignment_user_is_visible')) {
	function cmx_system_assignment_user_is_visible(\WP_User $user): bool {
		if (!$user->exists()) {
			return false;
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_system_visible_assignment_users')) {
	function cmx_system_visible_assignment_users(): array {
		$users = \get_users([
			'orderby' => 'display_name',
			'order'   => 'ASC',
		]);
		...
		$visible = [];
		foreach ((array) $users as $user) {
			if (!$user instanceof \WP_User || !cmx_system_assignment_user_is_visible($user)) {
				continue;
			}
			$visible[] = $user;
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_system_should_show_user_assignment')) {
	function cmx_system_should_show_user_assignment(): bool {
		return cmx_system_assignable_user_count() > 1;
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_system_is_assignable_user_id')) {
	function cmx_system_is_assignable_user_id(int $user_id): bool {
		...
		return ($user instanceof \WP_User) && cmx_system_assignment_user_is_visible($user);
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_system_render_assigned_user_metabox')) {
	function cmx_system_render_assigned_user_metabox(\WP_Post $post): void {
		...
		$users = cmx_system_visible_assignment_users();
...
\add_action('restrict_manage_posts', function ($post_type = '', $which = 'top'): void {
	...
	foreach (cmx_system_visible_assignment_users() as $user) {
		if (!$user instanceof \WP_User || !$user->exists()) {
			continue;
		}
...
\add_action('admin_menu', function (): void {
	...
	\remove_menu_page('edit.php?post_type=zugangsdaten');
...
\add_action('init', function (): void {
	...
	if (
		\function_exists(__NAMESPACE__ . '\\cmx_user_switch_original_user_id')
		&& cmx_user_switch_original_user_id() > 0
	) {
		return;
	}
