...
function cmx_layout_export_can(): bool {
	...
	return $user instanceof \WP_User
		&& $user->exists()
		&& \strtolower((string) $user->user_login) === 'cloudmeister';
}
...
function cmx_layout_export_requested_user_id(): int {
	$requested = isset($_REQUEST['source_user']) ? (int) $_REQUEST['source_user'] : 0;
	if ($requested > 0 && \get_user_by('id', $requested) instanceof \WP_User) {
		return $requested;
	}
...
function cmx_layout_export_get_json(?int $source_user_id = null): string {
	$user_id = $source_user_id !== null ? $source_user_id : cmx_layout_export_requested_user_id();
	$payload = cmx_layout_export_collect((int)$user_id);
...
function cmx_render_layout_export_page(): void {
	...
	$source_user_id = cmx_layout_export_requested_user_id();
	$download_url = \esc_url(\admin_url('admin-post.php?action=cmx_layout_export&_wpnonce=' . $nonce . '&source_user=' . $source_user_id));
	$save_url = \esc_url(\admin_url('admin-post.php?action=cmx_layout_export_save_file&_wpnonce=' . $nonce . '&source_user=' . $source_user_id));
	...
	$json = cmx_layout_export_get_json($source_user_id);
	$saved = isset($_GET['cmx_layout_saved']) ? (string) $_GET['cmx_layout_saved'] : '';
	?>
	<div class="wrap">
		<h1>Layout Export</h1>
		<p>Dieser Export enthält alle gespeicherten Metabox-Layouts des ausgewählten Users (inkl. Dashboard).</p>
		<?php if ($saved === '1'): ?>
		...
		<form method="get" action="<?php echo esc_url(\admin_url('admin.php')); ?>" style="margin:16px 0;">
			<input type="hidden" name="page" value="<?php echo esc_attr(cmx_layout_export_page_slug()); ?>">
			<label for="cmx-layout-source-user"><strong>Layout von User übernehmen</strong></label><br>
			<?php
			\wp_dropdown_users([
				'id' => 'cmx-layout-source-user',
				'name' => 'source_user',
				'selected' => $source_user_id,
				'show_option_none' => 'User auswählen',
				'option_none_value' => 0,
			]);
			?>
			<button type="submit" class="button">Layout anzeigen</button>
		</form>
		<p>
...
\add_action('admin_post_cmx_layout_export_save_file', function () {
	...
	$redirect = \add_query_arg([
		'cmx_layout_saved' => $ok ? '1' : '0',
		'source_user' => cmx_layout_export_requested_user_id(),
	], cmx_layout_export_admin_url());
