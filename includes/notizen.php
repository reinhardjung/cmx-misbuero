...
if (!\function_exists(__NAMESPACE__ . '\\cmx_notizen_supported_post_types')) {
	function cmx_notizen_supported_post_types(): array {
		return ['kontakte', 'artikel', 'belege', 'dokumente', 'projekte', 'infrastruktur', 'zugangsdaten'];
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_notizen_meta_key_for_post_type')) {
	function cmx_notizen_meta_key_for_post_type(string $post_type): string {
		$map = [
			...
			'infrastruktur' => '_cmx_infrastruktur_intern_notizen',
			'zugangsdaten' => '_cmx_zugangsdaten_intern_notizen',
		];
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_notizen_legacy_meta_keys')) {
	function cmx_notizen_legacy_meta_keys(string $post_type): array {
		$map = [
			...
			'zugangsdaten' => ['_cmx_zugangsdaten_notizen'],
		];
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_notizen_allowed_html')) {
	function cmx_notizen_allowed_html(): array {
		...
		foreach (['table', 'thead', 'tbody', 'tfoot', 'tr', 'th', 'td', 'caption'] as $tag) {
			$allowed[$tag] = \array_merge(
				['class' => [], 'colspan' => [], 'rowspan' => [], 'scope' => []],
				\is_array($allowed[$tag] ?? null) ? $allowed[$tag] : []
			);
		}
		...
		if (($raw === '' || $raw === null) && $post_type !== '') {
			foreach (cmx_notizen_legacy_meta_keys($post_type) as $legacy_key) {
				...
				if ($legacy_raw !== '' && $legacy_raw !== null) {
					if ($post_type === 'zugangsdaten' && $legacy_key === '_cmx_zugangsdaten_notizen') {
						$legacy_text = \function_exists(__NAMESPACE__ . '\\cmx_zugangsdaten_decrypt')
							? cmx_zugangsdaten_decrypt((string) $legacy_raw)
							: '';
						if ($legacy_text === '') {
							continue;
						}
						$raw = [[
							'betreff' => 'Allgemein',
							'datum'   => (string) \get_post_modified_time('Y-m-d', false, $post_id),
							'zeit'    => (string) \get_post_modified_time('H:i', false, $post_id),
							'text'    => $legacy_text,
							'quelle'  => '',
						]];
					} else {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_notizen_render_row')) {
	...
	function cmx_notizen_render_row($index, array $row, bool $is_template = false): void {
		...
		if (\stripos($text_value, '<table') !== false) {
			echo '<div class="cmx-intern-notiz-html-preview">' . \wp_kses($text_value, cmx_notizen_allowed_html()) . '</div>';
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_render_central_notizen_box')) {
	function cmx_render_central_notizen_box(\WP_Post $post): void {
		...
		echo '#cmx-intern-notizen-list .cmx-intern-notiz-html-preview{overflow:auto;margin:0 0 8px;padding:8px;border:1px solid #dcdcde;background:#fff;}';
		echo '#cmx-intern-notizen-list .cmx-intern-notiz-html-preview table{width:100%;border-collapse:collapse;margin:0;}';
		echo '#cmx-intern-notizen-list .cmx-intern-notiz-html-preview th,#cmx-intern-notizen-list .cmx-intern-notiz-html-preview td{border:1px solid #dcdcde;padding:5px 7px;text-align:left;vertical-align:top;}';
		echo '#cmx-intern-notizen-list .cmx-intern-notiz-html-preview th{background:#f6f7f7;font-weight:600;}';
		echo '#cmx-intern-notizen-list .cmx-intern-notiz-html-preview h4{margin:0 0 8px;font-size:13px;}';
		...
		(function(){
			...
			const richEditorsEnabled = <?php echo $post_type === 'zugangsdaten' ? 'false' : 'true'; ?>;
			...
			function hasEditorApi() {
				return !!(richEditorsEnabled && window.wp && wp.editor && typeof wp.editor.initialize === 'function' && typeof wp.editor.remove === 'function');
			}
			...
			function addRow(seed){
				...
				initEditor(row);
				if (richEditorsEnabled && !hasEditorApi()) {
					scheduleEditorBoot(150);
				}
			}
			if (richEditorsEnabled) {
				initAllEditors();
			}
			if (richEditorsEnabled && !hasEditorApi()) {
...
\add_action('admin_enqueue_scripts', function (): void {
	...
	if ((string) ($screen->post_type ?? '') === 'zugangsdaten') {
		return;
	}
