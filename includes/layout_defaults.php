...
function cmx_layout_defaults_builtin(): array {
	return [
		...
		'meta-box-order_belege' => [
			'side' => 'cmx_savebox,cmx-beleg-abo,cmx_beleg_summe_box,cmx_beleg_waehrung,cmx_beleg_anzahlungen,cmx_belege_mwst_box,belege_zahlungsgrunddiv,cmx_uploads_box,cmx_dokumente_box,cmx_beleg_anhaenge_box',
...
function cmx_layout_defaults_ensure_current_user(): void {
	if (!\is_admin()) return;

	$user_id = (int) \get_current_user_id();
	if ($user_id <= 0) return;

	cmx_layout_defaults_apply_to_user($user_id);
}

\add_action('admin_init', __NAMESPACE__ . '\\cmx_layout_defaults_ensure_current_user', 30);
