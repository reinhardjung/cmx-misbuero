...
if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_favicon_url')) {
	function cmx_misbuero_favicon_url(): string {
		$relative = 'assets/favicon.png';
		$base_dir = \dirname(__DIR__);
		$url = (string) \plugins_url($relative, $base_dir . '/cmx-misbuero.php');
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_favicon_links')) {
	function cmx_misbuero_favicon_links(): string {
		$url = \esc_url(cmx_misbuero_favicon_url());
		if ($url === '') {
			return '';
		}

		return '<link rel="icon" href="' . $url . '" type="image/png">' . "\n"
			. '<link rel="shortcut icon" href="' . $url . '" type="image/png">' . "\n"
			. '<link rel="apple-touch-icon" href="' . $url . '">';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_print_favicon_links')) {
	function cmx_misbuero_print_favicon_links(): void {
		if (\function_exists('has_site_icon') && \has_site_icon()) {
			return;
		}

		echo cmx_misbuero_favicon_links() . "\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}

\add_action('wp_head', __NAMESPACE__ . '\\cmx_misbuero_print_favicon_links', 1);

if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_serve_favicon_request')) {
	function cmx_misbuero_serve_favicon_request(): void {
		$request_path = (string) \wp_parse_url((string) ($_SERVER['REQUEST_URI'] ?? ''), \PHP_URL_PATH);
		if (\rtrim($request_path, '/') !== '/favicon.ico') {
			return;
		}
		if (\function_exists('has_site_icon') && \has_site_icon()) {
			return;
		...
		$path = \dirname(__DIR__) . '/assets/favicon.png';
		...
		if ((string) ($_SERVER['REQUEST_METHOD'] ?? '') === 'HEAD') {
			exit;
		}
...
\add_action('init', __NAMESPACE__ . '\\cmx_misbuero_serve_favicon_request', -1000);
\add_action('template_redirect', __NAMESPACE__ . '\\cmx_misbuero_serve_favicon_request', 0);

if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_config_string')) {
	function cmx_misbuero_config_string(string $name, string $default = ''): string {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_first_config_string')) {
	function cmx_misbuero_first_config_string(array $names, string $default = ''): string {
		foreach ($names as $name) {
			$value = cmx_misbuero_config_string((string) $name);
			if ($value !== '') {
				return $value;
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_support_email')) {
	function cmx_misbuero_support_email(): string {
		$configured = cmx_misbuero_first_config_string([
			'CMX_MAGIC_LOGIN_SMTP_FROM',
			'CMX_MAGIC_LOGIN_SMTP_USER',
			'MIS_BUERO_SUPPORT_EMAIL',
		]);
		$email = \sanitize_email($configured);

		return \is_email($email) ? $email : 'support@misbuero.ch';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_support_smtp_settings')) {
	function cmx_misbuero_support_smtp_settings(): array {
		$host = \sanitize_text_field(cmx_misbuero_first_config_string([
			'CMX_MAGIC_LOGIN_SMTP_HOST',
			'MIS_BUERO_SUPPORT_SMTP_HOST',
		]));
		$user = \sanitize_email(cmx_misbuero_first_config_string([
			'CMX_MAGIC_LOGIN_SMTP_USER',
			'MIS_BUERO_SUPPORT_SMTP_USER',
		], cmx_misbuero_support_email()));
		$pass = cmx_misbuero_first_config_string([
			'CMX_MAGIC_LOGIN_SMTP_PASSWORD',
			'CMX_MAGIC_LOGIN_SMTP_PASS',
			'MIS_BUERO_SUPPORT_SMTP_PASSWORD',
			'MIS_BUERO_SUPPORT_SMTP_PASS',
		]);
		$port = (int) cmx_misbuero_first_config_string([
			'CMX_MAGIC_LOGIN_SMTP_PORT',
			'MIS_BUERO_SUPPORT_SMTP_PORT',
		], '587');
		if ($port <= 0) {
			$port = 587;
		}

		$secure = \strtolower(\trim(cmx_misbuero_first_config_string([
			'CMX_MAGIC_LOGIN_SMTP_SECURE',
			'MIS_BUERO_SUPPORT_SMTP_SECURE',
		], 'tls')));
		if (!\in_array($secure, ['', 'tls', 'ssl'], true)) {
			$secure = 'tls';
		}

		$from = \sanitize_email(cmx_misbuero_first_config_string([
			'CMX_MAGIC_LOGIN_SMTP_FROM',
			'MIS_BUERO_SUPPORT_SMTP_FROM',
			'MIS_BUERO_SUPPORT_EMAIL',
		], $user));
		if (!\is_email($from)) {
			$from = $user;
		}
		$from_name = \sanitize_text_field(cmx_misbuero_first_config_string([
			'CMX_MAGIC_LOGIN_SMTP_FROM_NAME',
			'MIS_BUERO_SUPPORT_SMTP_FROM_NAME',
		], 'Mis Büro Support'));
		...
		return [
			...
			'alias_belege' => $from,
			'reply' => $from,
			'email_name' => $from_name !== '' ? $from_name : 'Mis Büro Support',
			'bcc' => [],
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_uses_legacy_support_email')) {
	function cmx_misbuero_uses_legacy_support_email(string $email): bool {
		$email = \strtolower(\trim($email));
		return $email === '' || \in_array($email, ['wp.admin@misbuero.ch'], true);
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_misbuero_migrate_support_email_defaults')) {
	function cmx_misbuero_migrate_support_email_defaults(): void {
		$support_email = \sanitize_email(cmx_misbuero_support_email());
		if (!\is_email($support_email)) {
			return;
		}

		$admin_email = \sanitize_email((string) \get_option('admin_email'));
		if (cmx_misbuero_uses_legacy_support_email($admin_email)) {
			\update_option('admin_email', $support_email);
		}

		$cloudmeister = \get_user_by('login', 'cloudmeister');
		if ($cloudmeister instanceof \WP_User && $cloudmeister->exists()) {
			$user_email = \sanitize_email((string) $cloudmeister->user_email);
			if (cmx_misbuero_uses_legacy_support_email($user_email)) {
				\wp_update_user([
					'ID' => (int) $cloudmeister->ID,
		...
		if (\get_option($settings_key . '__flat', null) !== null) {
			$flat_website_email = \sanitize_email((string) \get_option($settings_key . '__website__email'));
			if (cmx_misbuero_uses_legacy_support_email($flat_website_email)) {
				\update_option($settings_key . '__website__email', $support_email);
			}

			$flat_button_url = \trim((string) \get_option($settings_key . '__website__contact__button_url'));
			if ($flat_button_url === '' || \strtolower($flat_button_url) === 'mailto:wp.admin@misbuero.ch') {
				\update_option($settings_key . '__website__contact__button_url', 'mailto:' . $support_email);
			}

			return;
		}

		$options = (array) \get_option($settings_key, []);
		$changed = false;

		$website = \is_array($options['website'] ?? null) ? (array) $options['website'] : [];
		$website_email = \sanitize_email((string) ($website['email'] ?? ''));
		if (cmx_misbuero_uses_legacy_support_email($website_email)) {
			$website['email'] = $support_email;
			$changed = true;
		}

		$contact = \is_array($website['contact'] ?? null) ? (array) $website['contact'] : [];
		$button_url = \trim((string) ($contact['button_url'] ?? ''));
		if ($button_url === '' || \strtolower($button_url) === 'mailto:wp.admin@misbuero.ch') {
			$contact['button_url'] = 'mailto:' . $support_email;
		...
		if ($changed) {
			...
			\update_option($settings_key, $options);
		}
...
\add_action('init', __NAMESPACE__ . '\\cmx_misbuero_migrate_support_email_defaults', 20);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_upload_file_is_readable')) {
	function cmx_upload_file_is_readable(string $path): bool {
		$path = \trim($path);
		if ($path === '') {
			return false;
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_upload_url_to_path')) {
	function cmx_upload_url_to_path(string $url): string {
		$url = \trim($url);
		if ($url === '') {
			return '';
		}
		...
		return $relative !== '' ? $basedir . '/' . $relative : '';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_email_default_contact_logo_path')) {
	function cmx_email_default_contact_logo_path(): string {
		$path = \function_exists(__NAMESPACE__ . '\\cmx_local_base_path')
			? \trailingslashit((string) cmx_local_base_path()) . 'logo.png'
			: '';
		if ($path !== '' && \function_exists(__NAMESPACE__ . '\\cmx_upload_file_is_readable') && cmx_upload_file_is_readable($path)) {
			return $path;
		}

		$uploads = \wp_get_upload_dir();
		$fallback_path = \trailingslashit(\wp_normalize_path((string) ($uploads['basedir'] ?? ''))) . 'misbuero/archiv/bilder/kontakte/logo.png';
		if (\function_exists(__NAMESPACE__ . '\\cmx_upload_file_is_readable') && cmx_upload_file_is_readable($fallback_path)) {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_email_default_contact_logo_url')) {
	function cmx_email_default_contact_logo_url(): string {
		$path = \function_exists(__NAMESPACE__ . '\\cmx_email_default_contact_logo_path')
			? (string) cmx_email_default_contact_logo_path()
			: '';
		if ($path === '') {
			return '';
		}
		...
		if (\function_exists(__NAMESPACE__ . '\\cmx_local_base_url')) {
			return \esc_url_raw(\trailingslashit((string) cmx_local_base_url()) . 'logo.png');
		}
		...
		if ($basedir === '/' || $baseurl === '' || \strpos($normalized_path, $basedir) !== 0) {
			return '';
		}

		return \esc_url_raw($baseurl . '/' . \ltrim((string) \substr($normalized_path, \strlen($basedir)), '/'));
	}
}
	...
	function cmx_contact_logo_url(int $post_id): string {
		...
		if (\function_exists(__NAMESPACE__ . '\\cmx_kl_active_gallery_item')) {
			...
			if (\is_array($active_item)) {
				...
				if ($active_path === '' && $active_url !== '') {
					$active_path = cmx_upload_url_to_path($active_url);
				}
				if ($active_url !== '' && cmx_upload_file_is_readable($active_path)) {
					return $active_url;
				}
			}
		}
		...
		if ($local_path === '' && $local_url !== '') {
			$local_path = cmx_upload_url_to_path($local_url);
		}
		if ($local_url !== '' && cmx_upload_file_is_readable($local_path)) {
			return $local_url;
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_email_self_logo_url')) {
	function cmx_email_self_logo_url(): string {
		$post_id = \function_exists(__NAMESPACE__ . '\\cmx_email_self_contact_id')
			? (int) cmx_email_self_contact_id()
			: 0;
		if ($post_id > 0 && \function_exists(__NAMESPACE__ . '\\cmx_contact_logo_url')) {
		...
		$default_contact_logo_url = \function_exists(__NAMESPACE__ . '\\cmx_email_default_contact_logo_url')
			? (string) cmx_email_default_contact_logo_url()
			: '';
		if ($default_contact_logo_url !== '') {
			return $default_contact_logo_url;
		}

		$fallback_path = \dirname(__DIR__) . '/assets/logo_fallback.png';
		return \is_file($fallback_path)
			? (string) \plugins_url('assets/logo_fallback.png', \dirname(__DIR__) . '/cmx-misbuero.php')
			: '';
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_email_self_logo_path')) {
	function cmx_email_self_logo_path(): string {
		$post_id = \function_exists(__NAMESPACE__ . '\\cmx_email_self_contact_id')
			...
			: 0;
		if ($post_id <= 0) {
			$default_contact_logo_path = \function_exists(__NAMESPACE__ . '\\cmx_email_default_contact_logo_path')
				? (string) cmx_email_default_contact_logo_path()
				: '';
			if ($default_contact_logo_path !== '') {
				return $default_contact_logo_path;
			}

			$fallback_path = \dirname(__DIR__) . '/assets/logo_fallback.png';
			return \is_file($fallback_path) && \is_readable($fallback_path) ? $fallback_path : '';
		}
		...
		if (\function_exists(__NAMESPACE__ . '\\cmx_kl_active_gallery_item')) {
			...
			if (\is_array($active_item)) {
				$active_path = \trim((string) ($active_item['path'] ?? ''));
				if ($active_path !== '' && cmx_upload_file_is_readable($active_path)) {
					return $active_path;
				}
			}
		}
		...
		$local_path = \trim((string) \get_post_meta($post_id, '_cmx_local_image_kontakte_path', true));
		if ($local_path !== '' && cmx_upload_file_is_readable($local_path)) {
			return $local_path;
		}
		...
		if ($thumb_id > 0) {
			...
			if ($thumb_path !== '' && cmx_upload_file_is_readable($thumb_path)) {
				return $thumb_path;
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_email_self_logo_outlook_cid')) {
	function cmx_email_self_logo_outlook_cid(): string {
		return 'cmx-self-logo@cmx-misbuero';
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_email_self_logo_outlook_style')) {
	function cmx_email_self_logo_outlook_style(string $img_style = '', int $width = 0, int $height = 0): string {
		$style = $img_style !== ''
			? $img_style
			: 'display:block;max-width:180px;width:100%;height:auto;max-height:101px;border:0;outline:none;text-decoration:none;';
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_email_logo_file_is_readable')) {
	function cmx_email_logo_file_is_readable(string $path): bool {
		$path = \trim($path);
		if ($path === '' || !\is_file($path) || !\is_readable($path)) {
			return false;
		}
		if (\function_exists(__NAMESPACE__ . '\\cmx_upload_file_is_readable') && cmx_upload_file_is_readable($path)) {
			return true;
		}

		$fallback_path = \dirname(__DIR__) . '/assets/logo_fallback.png';
		return \wp_normalize_path($path) === \wp_normalize_path($fallback_path);
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_email_embed_self_logo_for_phpmailer')) {
	function cmx_email_embed_self_logo_for_phpmailer($phpmailer): void {
		...
		if (
			$logo_path === ''
			|| !\function_exists(__NAMESPACE__ . '\\cmx_email_logo_file_is_readable')
			|| !cmx_email_logo_file_is_readable($logo_path)
		) {
			return;
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_email_self_logo_html')) {
	function cmx_email_self_logo_html(string $img_style = '', bool $prefer_outlook_embed = false): string {
		...
		if ($img_style === '') {
			$img_style = 'display:block;max-width:180px;width:100%;height:auto;max-height:101px;border:0;outline:none;text-decoration:none;';
		}
		...
		$default_src = $can_embed_inline ? ('cid:' . $embedded_cid) : $logo_url;
		$default_src_attr = $can_embed_inline
			? \esc_attr($default_src)
			: \esc_url($default_src);
		$dimension_attrs = \function_exists(__NAMESPACE__ . '\\cmx_email_inline_img_dimension_attributes')
			...
			: '';
		$default_img_html = '<img src="' . $default_src_attr . '" alt="Das bin ich Logo"' . $dimension_attrs . ' style="' . \esc_attr($img_style) . '" border="0">';
		...
		if ($prefer_outlook_embed) {
			...
			$outlook_style = \function_exists(__NAMESPACE__ . '\\cmx_email_self_logo_outlook_style')
				...
				: $img_style;
			$outlook_src_attr = $can_embed_inline
				? \esc_attr($outlook_src)
				: \esc_url($outlook_src);
			$outlook_img_html = '<img src="' . $outlook_src_attr . '" alt="Das bin ich Logo"' . $outlook_dimension_attrs . ' style="' . \esc_attr($outlook_style) . '" border="0" hspace="0" vspace="0">';
			$outlook_html = '<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;"><tr><td style="padding:0;text-align:right;">' . $outlook_img_html . '</td></tr></table>';

			return '<!--[if mso]>' . $outlook_html . '<![endif]--><!--[if !mso]><!-->' . $default_html . '<!--<![endif]-->';
		}
