...
CADDY_LOG="/var/log/caddy/${DOMAIN}.access.log"
SITE_LOG_DIR="/var/www/${DOMAIN}/logs"
...
for file in meta dns http_timings http_headers services journal_recent kernel_oom site_logs mysql_processlist; do
...
append_shell site_logs "
if [ -d '${SITE_LOG_DIR}' ]; then
	ls -lah '${SITE_LOG_DIR}'
	printf '\\n'
	for file in error.log access.log php-error.log; do
		if [ -f '${SITE_LOG_DIR}/'\${file} ]; then
			printf '== %s ==\\n' \"\${file}\"
			tail -n 200 '${SITE_LOG_DIR}/'\${file}
			printf '\\n'
		fi
	done
else
	printf 'Site log dir not found: %s\\n' '${SITE_LOG_DIR}'
fi
if [ -f '${CADDY_LOG}' ]; then
	printf '== caddy access log ==\\n'
	tail -n 200 '${CADDY_LOG}'
	printf '\\n'
fi
"
