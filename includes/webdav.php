...
function cmx_dav_normalize_rel_path(string $relPath): string {
	// Öffentliche Datei-URLs kodieren Sonderzeichen pro Pfadsegment (z. B.
	// "@" als "%40"). Vor der Dateisuche muss daraus wieder derselbe
	// Dateiname werden, unter dem der Upload gespeichert wurde.
	$relPath = rawurldecode(str_replace('\\', '/', $relPath));
	$parts = [];
...
function cmx_dav_module_file_path(string $module, string $relPath): string {
	...
	if ($root !== 'archiv' && str_starts_with($relPath, 'misbuero/archiv/')) {
		return cmx_dav_archive_file_path($relPath);
	}
	$modulePrefix = $root !== '' ? 'misbuero/' . $root . '/' : '';
	if ($modulePrefix !== '' && str_starts_with($relPath, $modulePrefix)) {
		$relPath = cmx_dav_normalize_rel_path(substr($relPath, strlen($modulePrefix)));
	}
...
function cmx_dav_module_file_url(string $module, string $relPath): string {
	...
	$relPath = cmx_dav_normalize_rel_path($relPath);
	if ($root !== 'archiv' && str_starts_with($relPath, 'misbuero/archiv/')) {
		return cmx_dav_archive_file_url($relPath);
	}
	$modulePrefix = $root !== '' ? 'misbuero/' . $root . '/' : '';
	if ($modulePrefix !== '' && str_starts_with($relPath, $modulePrefix)) {
		$relPath = cmx_dav_normalize_rel_path(substr($relPath, strlen($modulePrefix)));
...
function cmx_dav_public_missing_media_response(string $baseUri, string $requestPath): bool {
	$method = \strtoupper((string) ($_SERVER['REQUEST_METHOD'] ?? 'GET'));
	if ($method !== 'GET' && $method !== 'HEAD') {
		return false;
	}

	$relPath = cmx_dav_normalize_rel_path(cmx_dav_relative_from_base_uri($baseUri, $requestPath));
	$extension = \strtolower((string) \pathinfo($relPath, \PATHINFO_EXTENSION));
	if ($relPath === '' || !\in_array($extension, ['jpg', 'jpeg', 'png', 'webp', 'gif', 'heic', 'heif', 'mp4', 'mov', 'webm', 'pdf'], true)) {
		return false;
	}

	// Fehlende, öffentlich eingebundene Medien dürfen nicht bis zur DAV-
	// Authentifizierung durchfallen, da Browser sonst einen Login-Dialog öffnen.
	\status_header(404);
	\nocache_headers();
...
add_action('init', function () {
	...
	if ($uploadProfile === 'media' && cmx_dav_public_missing_media_response($baseUri, (string) $path)) {
		exit;
	}
