...
add_action('admin_footer', function (): void {
	...
	$targets = [
		...
		'm' => \admin_url('edit.php?post_type=emails'),
	...
	(function(){
		...
		document.addEventListener('keydown', function(event) {
			...
			if (event.key === 'Tab' && !event.altKey && !event.ctrlKey && !event.metaKey && !event.shiftKey && !isEditableTarget(event.target)) {
			...
			if (event.key === 'Enter' && !event.altKey && !event.ctrlKey && !event.metaKey && !event.shiftKey && !isEditableTarget(event.target)) {
		...
		function($post) use ($screen) {
			...
			if ($show_actions) {
				...
				if ($is_pending_belegeingang) {
					...
					if ($delete_link || $dup_link !== '' || $new_beleg_url !== '' || $new_beleg_from_artikel_url !== '' || $new_beleg_from_carent_url !== '' || $artikel_katalog_icon_html !== '' || $carent_vertrag_icon_html !== '' || $carent_vertrag_pdf_icon_html !== '' || $show_pdf_icons || $belegeingang_confirm_url !== '' || $kontakt_belege_url !== '' || $kontakt_telefonbuch_url !== '') {
