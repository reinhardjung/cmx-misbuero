...
final class Settings {
	...
	public static function get(): array {
		...
		$data = self::merge_defaults($saved, $defaults);
		// Eine bewusst leer gespeicherte Website-E-Mail darf nicht erneut durch
		// die Support-Adresse aus den Vorgaben ersetzt werden.
		if (\array_key_exists('email', $saved)) {
			$data['email'] = (string) $saved['email'];
		}
		// Ein bewusst leer gespeicherter Button-Link aktiviert das interne Kontaktformular.
		if (isset($saved['contact']) && \is_array($saved['contact']) && \array_key_exists('button_url', $saved['contact'])) {
			$data['contact']['button_url'] = (string) $saved['contact']['button_url'];
		}
	...
	public static function render(): void {
		...
		// Erster Submit-Button im Formular: Enter in Eingabefeldern speichert normal
		// und kann dadurch niemals die nachfolgende Preset-Aktion auslösen.
		echo '<button type="submit" class="screen-reader-text" data-cmx-website-enter-save>' . \esc_html__('Einstellungen speichern', 'cmx-misbuero') . '</button>';
		...
		echo '<button type="button" class="button button-secondary" data-cmx-website-apply-preset data-confirm="' . \esc_attr($preset_confirm) . '">' . \esc_html__('Branchen-Preset übernehmen', 'cmx-misbuero') . '</button>';
	...
	public static function sanitize_option($new, $old): array {
		...
		$apply_preset = isset($_POST['cmx_website_apply_preset'])
			? \sanitize_key((string) \wp_unslash($_POST['cmx_website_apply_preset']))
			: '';
		if ($apply_preset === 'confirmed') {
	...
	private static function contact_editor(string $base, array $data): void {
		...
		echo '<p class="description">' . \esc_html__('Leer lassen: internes Kontaktformular anzeigen. Eine eingetragene Webadresse wird stattdessen in einem neuen Fenster geöffnet.', 'cmx-misbuero') . '</p>';
		self::textarea(
			$base . '[contact][thank_you_text]',
			(string) ($section['thank_you_text'] ?? __('Vielen Dank für die Nachricht – wir kümmern uns darum.', 'cmx-misbuero')),
			__('Text nach erfolgreichem Versand', 'cmx-misbuero'),
			2
		);
		self::end_box();
	}
	...
	private static function sanitize_contact(array $section): array {
		return [
			...
			'thank_you_text' => \sanitize_textarea_field((string) ($section['thank_you_text'] ?? __('Vielen Dank für die Nachricht – wir kümmern uns darum.', 'cmx-misbuero'))),
		];
	}
	...
	private static function file(string $name, string $field_name, string $path, string $label, string $fallback_url = ''): void {
		$url = '';
		if ($path !== '' && \class_exists(__NAMESPACE__ . '\\Renderer')) {
			$url = Renderer::module_file_url($path);
		} elseif ($path !== '' && \function_exists('CLOUDMEISTER\\CMX\\Buero\\cmx_dav_module_file_url')) {
		...
		if ($url !== '') {
			...
			echo '<span class="description">' . \esc_html(\sprintf(__('Datei nicht gefunden: %s', 'cmx-misbuero'), \basename($path))) . '</span>';
			echo '<label class="cmx-website-remove"><input type="checkbox" name="' . \esc_attr(\preg_replace('/\]$/', '_remove]', $name)) . '" value="1"> ' . \esc_html__('entfernen', 'cmx-misbuero') . '</label>';
		} elseif ($fallback_url !== '') {
...
(function(){
	...
	document.addEventListener('keydown', function(event){
		...
		if (!target || !target.matches || !target.matches('input:not([type="button"]):not([type="submit"]):not([type="reset"]):not([type="file"]):not([type="checkbox"]):not([type="radio"]):not([type="color"])')) return;
		var form = target.closest('form');
		if (!form || !form.querySelector('[data-cmx-website-apply-preset]')) return;
		event.preventDefault();
		event.stopImmediatePropagation();
		var presetFlag = form.querySelector('input[name="cmx_website_apply_preset"]');
		if (presetFlag) presetFlag.remove();
		var saveButton = form.querySelector('#submit, input[type="submit"][name="submit"], button[type="submit"][name="submit"]');
	...
	document.addEventListener('click', function(event){
		var preset = event.target.closest('[data-cmx-website-apply-preset]');
		if (preset) {
			event.preventDefault();
			var form = preset.closest('form');
			if (!form || !window.confirm(preset.getAttribute('data-confirm') || 'Preset wirklich übernehmen?')) return;
			if (typeof form.reportValidity === 'function' && !form.reportValidity()) return;
			var flag = form.querySelector('input[name="cmx_website_apply_preset"]');
			if (!flag) {
				flag = document.createElement('input');
				flag.type = 'hidden';
				flag.name = 'cmx_website_apply_preset';
