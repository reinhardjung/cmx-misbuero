...
if (!\function_exists(__NAMESPACE__ . '\\cmx_cpt_transfer_nonce_url')) {
	/**
	 * Build a nonce URL for redirects.
	 *
	 * wp_nonce_url() HTML-encodes query separators. Passing that value directly
	 * to wp_safe_redirect() turns parameters such as `_wpnonce` into
	 * `amp;_wpnonce`, so nonce validation fails on the export endpoint.
	 */
	function cmx_cpt_transfer_nonce_url(string $url, string $action): string {
		return (string) \add_query_arg('_wpnonce', \wp_create_nonce($action), $url);
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_cpt_transfer_collect_ids')) {
	function cmx_cpt_transfer_collect_ids(string $post_type): array {
		...
		$args = (array) \apply_filters('cmx_cpt_transfer_collect_args', $args, $post_type, $request, $selected);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_cpt_transfer_relation_targets')) {
	function cmx_cpt_transfer_relation_targets(string $meta_key, string $meta_path = ''): array {
		$custom_targets = (array) \apply_filters('cmx_cpt_transfer_relation_targets', [], $meta_key, $meta_path);
		$custom_targets = \array_values(\array_unique(\array_filter(\array_map('sanitize_key', $custom_targets))));
		if ($custom_targets !== []) {
		...
		$direct = [
			...
			'_cmx_infrastruktur_kontakt_id' => ['kontakte'],
	...
	function cmx_cpt_transfer_dataset(string $post_type, array $post_ids): array {
		...
		foreach ($post_ids as $post_id) {
			...
			foreach (cmx_cpt_transfer_meta_keys_for_post($post->ID) as $meta_key) {
				...
				foreach ($rows as &$row) {
					$row['meta_value'] = (string) \apply_filters(
						'cmx_cpt_transfer_export_meta_value',
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_cpt_transfer_import_zip')) {
	...
	function cmx_cpt_transfer_import_dirs(array $dirs, bool $update_existing, array $selected_post_types = [], bool $clear_first = false): array {
		...
		foreach ($meta_by_post as $post_id => $meta_items) {
			$post_type = (string) \get_post_type((int) $post_id);
			foreach ($meta_items as $key => $rows) {
				...
				$value = $has_path ? $value : (string) ($rows[0]['value'] ?? '');
				$value = \apply_filters('cmx_cpt_transfer_import_meta_value', $value, $post_type, (int) $post_id, (string) $key);
		...
		foreach ($dirs as $dir) {
			foreach (cmx_cpt_transfer_read_csv($dir . '/terms.csv') as $row) {
				...
				if (!\is_wp_error($term)) {
					...
					if ($term_id > 0) {
						$append = (bool) \apply_filters('cmx_cpt_transfer_append_term', true, $taxonomy, (int) $post_id);
		...
		\do_action('cmx_cpt_transfer_import_completed', $id_map, $effective_post_types);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_cpt_transfer_views')) {
	function cmx_cpt_transfer_views(array $views): array {
		foreach ($views as $key => $html) {
			$label = \strtolower(\trim(\wp_strip_all_tags((string) $html)));
			if (\in_array($label, ['exportieren', 'importieren'], true)) {
				unset($views[$key]);
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_cpt_transfer_bulk_export_scope_args')) {
	function cmx_cpt_transfer_bulk_export_scope_args(array $request): array {
		$excluded = [
			'action' => true,
			'action2' => true,
			'post' => true,
			...
			'cmx_cpt_transfer_do_import' => true,
		];
		$args = [];
...
\add_action('load-edit.php', static function (): void {
	...
	$action = isset($_REQUEST['action']) && !\is_array($_REQUEST['action'])
		...
		: '';
	if ($action === '' || $action === '-1') {
		$action = isset($_REQUEST['action2']) && !\is_array($_REQUEST['action2'])
			? \sanitize_key((string) \wp_unslash($_REQUEST['action2']))
			: '';
	}
	if (!\in_array($action, ['cmx_cpt_transfer_import', 'cmx_cpt_transfer_export'], true)) {
		return;
	}
	...
	if ($action === 'cmx_cpt_transfer_import') {
		\wp_safe_redirect((string) \add_query_arg([
			'post_type' => $post_type,
			'cmx_cpt_transfer_import' => 1,
		], \admin_url('edit.php')));
		exit;
	}

	$export_args = [
		'action' => 'cmx_cpt_transfer_export',
		'post_type' => $post_type,
	];
	$post_ids = isset($_REQUEST['post'])
		? \array_values(\array_unique(\array_filter(\array_map('intval', (array) \wp_unslash($_REQUEST['post'])))))
		: [];
	if ($post_ids !== []) {
		$export_args['post'] = $post_ids;
	} else {
		$export_args = \array_merge(
			$export_args,
			cmx_cpt_transfer_bulk_export_scope_args((array) $_REQUEST)
		);
	}

	$export_url = cmx_cpt_transfer_nonce_url(
		\add_query_arg($export_args, \admin_url('admin-post.php')),
		'cmx_cpt_transfer_export'
	);
	$export_url = (string) \apply_filters('cmx_cpt_transfer_export_url', $export_url, $post_type);
...
\add_action('admin_init', static function (): void {
	foreach (cmx_cpt_transfer_supported_post_types() as $post_type) {
		...
		\add_filter('bulk_actions-edit-' . $post_type, static function (array $actions): array {
			if (cmx_cpt_transfer_can()) {
				$actions['cmx_cpt_transfer_export'] = 'Exportieren';
				$actions['cmx_cpt_transfer_import'] = 'Importieren';
			}
			return $actions;
		});
		\add_filter('handle_bulk_actions-edit-' . $post_type, static function (string $redirect, string $action, array $post_ids) use ($post_type): string {
			if (!cmx_cpt_transfer_can()) {
				return $redirect;
			}
			...
			$export = cmx_cpt_transfer_nonce_url(
				\add_query_arg($export_args, \admin_url('admin-post.php')),
				'cmx_cpt_transfer_export'
			);
			return (string) \apply_filters('cmx_cpt_transfer_export_url', $export, $post_type);
		}, 10, 3);
	}
}, 99);
	...
	<div class="notice notice-info" style="padding:18px;margin-top:15px;">
		...
		<form method="post" enctype="multipart/form-data">
			...
			<p>
				...
				<a href="<?php echo \esc_url(\add_query_arg(['post_type' => $post_type], \admin_url('edit.php'))); ?>" style="margin-left:8px;">Abbrechen</a>
			</p>
		</form>
	</div>
	<?php
...
\add_action('admin_footer-edit.php', function (): void {
	...
	$import_url = (string) \add_query_arg([
		'post_type' => $post_type,
		'cmx_cpt_transfer_import' => 1,
	], \admin_url('edit.php'));
	?>
	<script>
	(function(){
		const listForm = document.getElementById('posts-filter');
		if (!listForm) return;

		listForm.addEventListener('submit', function(event){
			const submitter = event.submitter || null;
			const bulkActions = submitter && submitter.closest
				? submitter.closest('.bulkactions')
				: null;
			const selector = bulkActions
				? bulkActions.querySelector('select[name="action"], select[name="action2"]')
				: null;
			if (!selector) return;

			if (selector.value === 'cmx_cpt_transfer_import') {
				event.preventDefault();
				event.stopImmediatePropagation();
				window.location.assign(<?php echo \wp_json_encode($import_url); ?>);
				return;
			}

			if (selector.value !== 'cmx_cpt_transfer_export') return;
			const selected = listForm.querySelectorAll('input[name="post[]"]:checked');
			if (selected.length) return;

			event.preventDefault();
			event.stopImmediatePropagation();
			HTMLFormElement.prototype.submit.call(listForm);
		}, true);
	})();
