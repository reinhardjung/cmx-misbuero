...
function cmx_artikel_variant_flat_slots(int $post_id): array {
	...
	foreach (\array_keys((array) \get_post_meta($post_id)) as $meta_key) {
		if (!\preg_match('/^_cmx_artikel_variant_(\d+)_(sku|anzahl|left_taxonomy|left_term_id|right_taxonomy|right_term_id|einheit_term_id|ek|aufwand|vk|belegtext|verkaufbar|katalog|website|onlineshop|pos|nicht_mehr_lieferbar|archiviert|woo_product_id|woo_variation_id|woo_variation_sku)$/', (string) $meta_key, $matches)) {
			continue;
		}
...
function cmx_artikel_variant_row_normalize(array $row, array $left_choices, array $right_choices, array $base = []): array {
	...
	return [
		...
		'website' => \array_key_exists('website', $row)
			? (!empty($row['website']) ? 1 : 0)
			: (int) ($default['website'] ?? 0),
		'onlineshop' => \array_key_exists('onlineshop', $row)
			? (!empty($row['onlineshop']) ? 1 : 0)
			: (int) ($default['onlineshop'] ?? 0),
		'pos' => \array_key_exists('pos', $row)
			? (!empty($row['pos']) ? 1 : 0)
			: (int) ($default['pos'] ?? 0),
		'nicht_mehr_lieferbar' => \array_key_exists('nicht_mehr_lieferbar', $row)
			? (!empty($row['nicht_mehr_lieferbar']) ? 1 : 0)
			: (int) ($default['nicht_mehr_lieferbar'] ?? 0),
		'archiviert' => \array_key_exists('archiviert', $row)
			? (!empty($row['archiviert']) ? 1 : 0)
			: (int) ($default['archiviert'] ?? 0),
...
function cmx_artikel_variant_rows_sort_by_sku(array $rows): array {
	$indexed_rows = [];
	...
	\usort($indexed_rows, static function (array $left, array $right): int {
		$left_sku = (string) ($left['sku'] ?? '');
		$right_sku = (string) ($right['sku'] ?? '');
		$left_empty = $left_sku === '';
		$right_empty = $right_sku === '';

		if ($left_empty !== $right_empty) {
			return $left_empty ? 1 : -1;
		}

		$comparison = \strnatcasecmp($left_sku, $right_sku);
		return $comparison !== 0
			? $comparison
			: ((int) ($left['index'] ?? 0) <=> (int) ($right['index'] ?? 0));
	});

	return \array_values(\array_map(static fn(array $entry): array => (array) $entry['row'], $indexed_rows));
}
...
function cmx_artikel_variant_rows_persist(int $post_id, array $variant_rows, array $left_choices = [], array $right_choices = []): array {
	...
	foreach ($normalized_rows as $index => $row) {
		...
		foreach (cmx_artikel_variant_row_fields() as $field) {
			$value = $row[$field] ?? '';
			if ($field === 'verkaufbar' || $field === 'katalog' || $field === 'website' || $field === 'onlineshop' || $field === 'pos' || $field === 'nicht_mehr_lieferbar' || $field === 'archiviert' || $field === 'woo_product_id' || $field === 'woo_variation_id' || $field === 'left_term_id' || $field === 'right_term_id' || $field === 'einheit_term_id') {
...
function cmx_artikel_variant_row_markup(
	...
	echo '<label title="wenn nicht ist es i.R. ein interner Artikel"><input type="checkbox" name="cmx_artikel_variants[' . \esc_attr($index_attr) . '][verkaufbar]" class="cmx-variant-verkaufbar" data-variant-field-key="verkaufbar" value="1" ' . \checked((int) ($row['verkaufbar'] ?? 1), 1, false) . '> verkaufbar</label>';
	echo '<label title="im Katalog zeigen"><input type="checkbox" name="cmx_artikel_variants[' . \esc_attr($index_attr) . '][katalog]" class="cmx-variant-katalog" data-variant-field-key="katalog" value="1" ' . \checked((int) ($row['katalog'] ?? 1), 1, false) . '> Katalog</label>';
	echo '<label title="auf der Website anzeigen"><input type="checkbox" name="cmx_artikel_variants[' . \esc_attr($index_attr) . '][website]" class="cmx-variant-website" data-variant-field-key="website" value="1" ' . \checked((int) ($row['website'] ?? 0), 1, false) . '> Website</label>';
	echo '<label title="im OnlineShop verfügbar"><input type="checkbox" name="cmx_artikel_variants[' . \esc_attr($index_attr) . '][onlineshop]" class="cmx-variant-onlineshop" data-variant-field-key="onlineshop" value="1" ' . \checked((int) ($row['onlineshop'] ?? 0), 1, false) . '> OnlineShop</label>';
	echo '<label title="im POS verfügbar"><input type="checkbox" name="cmx_artikel_variants[' . \esc_attr($index_attr) . '][pos]" class="cmx-variant-pos" data-variant-field-key="pos" value="1" ' . \checked((int) ($row['pos'] ?? 0), 1, false) . '> POS</label>';
	echo '<label><input type="checkbox" name="cmx_artikel_variants[' . \esc_attr($index_attr) . '][nicht_mehr_lieferbar]" class="cmx-variant-nicht-mehr-lieferbar" data-variant-field-key="nicht_mehr_lieferbar" value="1" ' . \checked((int) ($row['nicht_mehr_lieferbar'] ?? 0), 1, false) . '> nicht mehr lieferbar</label>';
	echo '<label title="Artikel wird nicht mehr genutzt"><input type="checkbox" name="cmx_artikel_variants[' . \esc_attr($index_attr) . '][archiviert]" class="cmx-variant-archiviert" data-variant-field-key="archiviert" value="1" ' . \checked((int) ($row['archiviert'] ?? 0), 1, false) . '> archiviert</label>';
	echo '</div>';
...
function cmx_artikel_waehrung_preise_box_html(\WP_Post $post): void {
	...
	$settings_option = \defined(__NAMESPACE__ . '\\CMX_SETTINGS_MAIN') ? (string) \constant(__NAMESPACE__ . '\\CMX_SETTINGS_MAIN') : 'cmx_einstellungen';
	$settings = (array) \get_option($settings_option, []);
	...
	echo '<style>
		...
		.cmx-price-row .cmx-f{position:relative;display:flex;flex-direction:column;min-width:140px;flex:1 1 180px;max-width:320px}
		...
		.cmx-price-row .cmx-f .cmx-taxonomy-picker{display:none;position:absolute;left:0;right:0;bottom:0;z-index:20;margin:0;background:#fff}
		...
		.cmx-price-row .cmx-f input[type="number"],
		.cmx-price-row .cmx-f input[type="text"],
		.cmx-price-row .cmx-f select{box-sizing:border-box;min-height:40px;height:40px;padding-top:0;padding-bottom:0;line-height:38px}
		.cmx-price-row .cmx-f select{padding-left:12px;padding-right:32px}
	...
	.cmx-price-row .cmx-check-left{display:flex;align-items:center;column-gap:24px;row-gap:14px;white-space:nowrap;flex-wrap:wrap}
	.cmx-price-row .cmx-check-left label{position:relative;top:-5px}
	.cmx-price-row .cmx-check-left input[type="checkbox"]{transform:translateY(2px)}
...
\add_action('save_post_artikel', function (int $post_id, \WP_Post $post) {
	...
	if (isset($_POST['cmx_artikel_konditionen_payload'])) {
		...
		if (\is_array($raw_variant_rows)) {
			foreach ($raw_variant_rows as $index => $row) {
				...
				$variant_rows[] = cmx_artikel_variant_row_normalize([
					...
					'website' => isset($row['website']) ? 1 : 0,
					'onlineshop' => isset($row['onlineshop']) ? 1 : 0,
					'pos' => isset($row['pos']) ? 1 : 0,
					'nicht_mehr_lieferbar' => isset($row['nicht_mehr_lieferbar']) ? 1 : 0,
					'archiviert' => isset($row['archiviert']) ? 1 : 0,
		...
		$variant_rows = cmx_artikel_variant_rows_sort_by_sku($variant_rows);
