...
function cmx_import_clear_variant_meta(int $post_id): void {
	if (\function_exists(__NAMESPACE__ . '\\cmx_artikel_variant_clear_flat_rows')) {
		...
		foreach (\array_keys((array) \get_post_meta($post_id)) as $meta_key) {
			if (\preg_match('/^_cmx_artikel_variant_\d+_(sku|anzahl|left_taxonomy|left_term_id|right_taxonomy|right_term_id|einheit_term_id|ek|aufwand|vk|belegtext|verkaufbar|katalog|website|onlineshop|pos|nicht_mehr_lieferbar|archiviert|woo_product_id|woo_variation_id|woo_variation_sku)$/', (string) $meta_key)) {
				\delete_post_meta($post_id, $meta_key);
			}
		}
...
function cmx_artikel_import_collect_variant_rows(array $row, array $row_l): array {
	...
	foreach ($all_keys as $raw_key) {
		$key = \strtolower((string) $raw_key);
		if (!\preg_match('/^variant_(\d+)_(sku|anzahl|left_taxonomy|left_term_slug|left_term_name|right_taxonomy|right_term_slug|right_term_name|einheit_slug|einheit_name|ek|aufwand|vk|belegtext|verkaufbar|katalog|website|onlineshop|pos|nicht_mehr_lieferbar|archiviert|woo_product_id|woo_variation_id|woo_variation_sku)$/', $key, $matches)) {
			continue;
		}
		$slot = (int) ($matches[1] ?? 0);
		if ($slot <= 0) {
	...
	foreach ($collected as $variant_row) {
		...
		$row_out = [
			...
			'website' => cmx_artikel_import_bool_from_csv($variant_row['website'] ?? '') ? 1 : 0,
			'onlineshop' => cmx_artikel_import_bool_from_csv($variant_row['onlineshop'] ?? '') ? 1 : 0,
			'pos' => cmx_artikel_import_bool_from_csv($variant_row['pos'] ?? '') ? 1 : 0,
			'nicht_mehr_lieferbar' => cmx_artikel_import_bool_from_csv($variant_row['nicht_mehr_lieferbar'] ?? '') ? 1 : 0,
			'archiviert' => cmx_artikel_import_bool_from_csv($variant_row['archiviert'] ?? '') ? 1 : 0,
