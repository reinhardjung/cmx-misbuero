...
if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_variant_flag_is_active')) {
	function cmx_artikel_variant_flag_is_active($value): bool {
		if (\is_bool($value)) {
			return $value;
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_variant_row_is_selectable')) {
	function cmx_artikel_variant_row_is_selectable(array $row): bool {
		return !cmx_artikel_variant_flag_is_active($row['nicht_mehr_lieferbar'] ?? 0)
			&& !cmx_artikel_variant_flag_is_active($row['archiviert'] ?? 0);
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_variant_entry_is_selectable')) {
	function cmx_artikel_variant_entry_is_selectable(array $entry): bool {
		return cmx_artikel_variant_row_is_selectable($entry);
	}
}
	...
	function cmx_artikel_admin_variant_entries(int $post_id): array {
		...
		foreach (\array_values($stored) as $index => $row) {
			...
			$entries[] = [
				...
				'nicht_mehr_lieferbar' => !empty($row['nicht_mehr_lieferbar']),
				'archiviert' => !empty($row['archiviert']),
				'search'     => $search_blob,
			];
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_variant_vk')) {
	function cmx_artikel_variant_vk(int $post_id, $variant_index, bool $fallback_to_article = true): string {
		if ($post_id <= 0 || $variant_index === '' || $variant_index === null || !\is_numeric((string) $variant_index)) {
			$variant_vk = '';
		} else {
			$variant_vk = '';
			$variant_index = (int) $variant_index;
			foreach (cmx_artikel_admin_variant_entries($post_id) as $entry) {
				if ((int) ($entry['index'] ?? -1) !== $variant_index) {
					continue;
				}
				$variant_vk = \trim((string) ($entry['vk'] ?? ''));
				break;
			}
		}
	...
	function cmx_artikel_admin_variant_search_ids(string $term): array {
		...
		foreach ((array) $post_ids as $post_id) {
			foreach (cmx_artikel_admin_variant_entries((int) $post_id) as $entry) {
				if (!cmx_artikel_variant_entry_is_selectable((array) $entry)) {
					continue;
				}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_admin_image_url_exists')) {
	function cmx_artikel_admin_image_url_exists(string $url): bool {
		...
		if ($baseurl !== '' && $basedir !== '' && \strpos($url, $baseurl . '/') === 0) {
			...
			return $rel !== '' && cmx_artikel_admin_upload_file_exists($basedir . '/' . $rel);
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_admin_upload_file_exists')) {
	function cmx_artikel_admin_upload_file_exists(string $path): bool {
		$path = \trim($path);
		if ($path === '') {
			return false;
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_admin_image_src')) {
	function cmx_artikel_admin_image_src(int $post_id): string {
		...
		if ($local_path !== '' && cmx_artikel_admin_upload_file_exists($local_path) && $local_img !== '') {
			return $local_img;
		}
		...
		if ($thumb_id > 0) {
			...
			if ($attached_file !== '' && cmx_artikel_admin_upload_file_exists($attached_file)) {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_katalog_link_url')) {
	function cmx_artikel_katalog_link_url(int $post_id): string {
		$post_id = (int) $post_id;
		if ($post_id <= 0 || !cmx_artikel_is_katalog_sichtbar($post_id)) {
			return '';
		}

		if (\function_exists(__NAMESPACE__ . '\\cmx_artikel_detail_url')) {
			$url = (string) cmx_artikel_detail_url($post_id);
			if ($url !== '') {
				return $url;
			}
		}
		...
		if (\function_exists(__NAMESPACE__ . '\\cmx_artikel_liste_url')) {
			return (string) cmx_artikel_liste_url();
		}
