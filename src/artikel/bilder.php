...
if (!\function_exists(__NAMESPACE__.'\\cmx_li_base_path')) {
	function cmx_li_base_path(string $post_type = 'artikel'): string {
		$u = wp_get_upload_dir();
		$subdir = $post_type === 'zugangsdaten' ? 'zugangsdaten' : 'artikel';
		return wp_normalize_path($u['basedir'] . CMX_LOCAL_IMG_SUBDIR . '/' . $subdir);
	}
}
if (!\function_exists(__NAMESPACE__.'\\cmx_li_base_url')) {
	function cmx_li_base_url(string $post_type = 'artikel'): string {
		$u = wp_get_upload_dir();
		$subdir = $post_type === 'zugangsdaten' ? 'zugangsdaten' : 'artikel';
		return rtrim($u['baseurl'], '/') . CMX_LOCAL_IMG_SUBDIR . '/' . $subdir;
	}
}
...
if (!\function_exists(__NAMESPACE__.'\\cmx_li_basename_for_post')) {
	function cmx_li_basename_for_post(\WP_Post $post): string {
		$fallback = $post->post_type === 'zugangsdaten' ? 'zugangsdaten' : 'artikel';
		$title_slug = strtolower((string) sanitize_title(get_the_title($post) ?: $fallback));
		return $title_slug !== '' ? $title_slug : $fallback;
	}
}

if (!\function_exists(__NAMESPACE__.'\\cmx_li_gallery_meta_key')) {
	function cmx_li_gallery_meta_key(string $post_type = 'artikel'): string {
		return $post_type === 'zugangsdaten'
			? '_cmx_local_image_zugangsdaten_gallery'
...
if (!\function_exists(__NAMESPACE__.'\\cmx_li_gallery_get')) {
	function cmx_li_gallery_get(int $post_id, string $meta_base): array {
		$items = \get_post_meta($post_id, cmx_li_gallery_meta_key((string) \get_post_type($post_id)), true);
...
if (!\function_exists(__NAMESPACE__.'\\cmx_li_gallery_update')) {
	function cmx_li_gallery_update(int $post_id, string $meta_base, array $items): void {
		...
		$gallery_meta_key = cmx_li_gallery_meta_key((string) \get_post_type($post_id));
		...
		\update_post_meta($post_id, $gallery_meta_key, $items);
...
if (!\function_exists(__NAMESPACE__.'\\cmx_li_sync_filename_with_title')) {
	function cmx_li_sync_filename_with_title(int $post_id, \WP_Post $post, string $meta_base): void {
		...
		$base_dir = cmx_li_base_path((string) $post->post_type);
		$base_url = cmx_li_base_url((string) $post->post_type);
		if (!is_dir($base_dir)) {
			wp_mkdir_p($base_dir);
		}
...
/** Bild-Metabox für Artikel und Zugangsdaten. */
...
if (!\function_exists(__NAMESPACE__.'\\cmx_li_render_box_artikel')) {
	function cmx_li_render_box_artikel(\WP_Post $post): void {
		...
		$is_zugangsdaten = $post->post_type === 'zugangsdaten';
		$image_label = $is_zugangsdaten ? 'Foto' : 'Artikelbild';
		$save_label = $is_zugangsdaten ? 'Zugangsdaten speichern' : 'Artikel speichern';
		$meta_base = $is_zugangsdaten ? '_cmx_local_image_zugangsdaten' : '_cmx_local_image_artikel';
		...
		echo '<div class="cmx-li-preview-empty" style="' . ($has_image ? 'display:none;' : 'display:block;') . 'color:#646970;font-style:italic;text-align:center;">Kein ' . esc_html($image_label) . ' vorhanden.<br>Dateien hier ablegen oder klicken.<br>PNG-Format: 1920x1080px</div>';
		...
		foreach ($gallery as $index => $item) {
			...
			echo '<button type="button" class="button-link cmx-local-image-original-preview" data-preview-url="' . esc_url($item_url) . '" aria-label="Bild in Originalgrösse anzeigen" title="Bild in Originalgrösse anzeigen" style="text-decoration:none;color:#8c8f94;"><span class="dashicons dashicons-visibility" style="font-size:16px;width:16px;height:16px;line-height:16px;"></span></button>';
			if ($item_url !== '') {
		...
		echo '<p class="cmx-li-save-wrap" style="margin:10px 0 0 0;display:none;"><button type="submit" class="button button-primary cmx-li-save-button">' . esc_html($save_label) . '</button></p>';
		echo '</div>';
		...
		(function(){
			...
			var imageLabel = ' . wp_json_encode($image_label) . ';
			...
			function updateStatusFromRows() {
				var rows = visibleRows();
				if (!rows.length) {
					setStatus("Kein " + imageLabel + " vorhanden.");
					return;
				}
...
/** Save-Handler für Artikel und Zugangsdaten. */
function cmx_li_save_gallery_post($post_id, $post, $update): void {
	if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
	if (!is_object($post) || !in_array((string) $post->post_type, ['artikel', 'zugangsdaten'], true)) return;
	if (!current_user_can('edit_post', $post_id)) return;
	if (empty($_POST['cmx_li_artikel_nonce']) || !wp_verify_nonce($_POST['cmx_li_artikel_nonce'], 'cmx_li_artikel_nonce')) return;

	$meta_base = $post->post_type === 'zugangsdaten'
		? '_cmx_local_image_zugangsdaten'
		: '_cmx_local_image_artikel';
...
\add_action('save_post_artikel', __NAMESPACE__ . '\\cmx_li_save_gallery_post', 10, 3);
\add_action('save_post_zugangsdaten', __NAMESPACE__ . '\\cmx_li_save_gallery_post', 10, 3);
