...
if (!\function_exists(__NAMESPACE__.'\\cmx_li_render_box_artikel')) {
	function cmx_li_render_box_artikel(\WP_Post $post): void {
		...
		echo '<div class="cmx-li-preview-frame" style="position:relative;margin-bottom:10px;border:1px dashed #c3c4c7;background:#fff;padding:10px;min-height:180px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:border-color .15s ease, background-color .15s ease;">';
		echo '<img class="cmx-li-preview-image" src="' . ($has_image ? esc_url($url) : '') . '" alt="" style="' . ($has_image ? 'display:block;' : 'display:none;') . 'max-width:100%;max-height:380px;height:auto;width:auto;">';
		...
		(function(){
			...
			function updatePreview() {
				...
				if (previewImage) {
					previewImage.src = rows[0].dataset.url || "";
