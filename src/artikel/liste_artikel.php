...
if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_liste_variant_is_catalog_visible')) {
	function cmx_artikel_liste_variant_is_catalog_visible(array $entry): bool {
		if (\function_exists(__NAMESPACE__ . '\\cmx_artikel_variant_entry_is_selectable') && !cmx_artikel_variant_entry_is_selectable($entry)) {
			return false;
		}

		return !empty($entry['katalog']);
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_liste_catalog_variant_entries')) {
	function cmx_artikel_liste_catalog_variant_entries(int $artikel_id): array {
		if ($artikel_id <= 0 || !\function_exists(__NAMESPACE__ . '\\cmx_artikel_admin_variant_entries')) {
			return [];
		}
		...
		return \array_values(\array_filter($entries, static function ($entry): bool {
			return \is_array($entry) && cmx_artikel_liste_variant_is_catalog_visible((array) $entry);
		}));
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_liste_is_catalog_visible')) {
	function cmx_artikel_liste_is_catalog_visible(int $artikel_id): bool {
		if ($artikel_id <= 0) {
			return false;
		}
		...
		return cmx_artikel_liste_catalog_variant_entries($artikel_id) !== [];
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_artikel_liste_rows')) {
	function cmx_artikel_liste_rows(): array {
		...
		foreach ((array) $q->posts as $artikel_id) {
			...
			if (!cmx_artikel_liste_is_catalog_visible($artikel_id)) {
				continue;
			}
	...
	function cmx_render_artikel_liste_page(): void {
		...
		echo '<style>
			...
			*{box-sizing:border-box}
			...
			.cmx-artikel-head-brand{flex:0 0 auto;display:flex;align-items:flex-start;justify-content:flex-end;gap:16px;min-height:96px}
			.cmx-artikel-head-logo{display:block;width:171px;height:96px;object-fit:contain;object-position:right top}
			...
			.cmx-artikel-table-wrap{padding:18px 28px 28px;overflow-x:auto}
				.cmx-artikel-table{width:100%;border-collapse:collapse;min-width:980px}
			...
			.cmx-artikel-desc{min-width:320px}
				.cmx-artikel-table th:first-child,.cmx-artikel-thumb-wrap{width:148px}
				.cmx-artikel-thumb{display:block;width:128px;height:72px;object-fit:contain;border-radius:10px;border:1px solid #e0e0e0;background:#fff}
				.cmx-artikel-thumb-placeholder{display:block;width:128px;height:72px;border-radius:10px;border:1px dashed #d4d4d4;background:#f4f4f4}
			...
			@media (max-width:720px){
				...
				.cmx-artikel-cart-flyout{left:0;right:auto;width:min(340px,calc(100vw - 32px))}
					.cmx-artikel-table th:first-child,.cmx-artikel-thumb-wrap{width:118px}
					.cmx-artikel-thumb,.cmx-artikel-thumb-placeholder{width:104px;height:59px}
				}
		</style>';
		echo '</head><body>';
