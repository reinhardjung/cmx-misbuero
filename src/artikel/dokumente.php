...
\add_action('add_meta_boxes', function() {
	...
	\add_meta_box(
		'cmx_artikel_dokumente',
		$title,
		__NAMESPACE__ . '\\cmx_render_artikel_dokumente_metabox',
		'artikel',
		'side',
		'default'
	);
...
function cmx_render_artikel_dokumente_metabox(\WP_Post $post): void {
	$meta_key = defined(__NAMESPACE__ . '\\CMX_DOK_REL_META')
		? (CMX_DOK_REL_META['artikel'] ?? 'cmx_dokumente_artikel')
	...
	foreach ($docs as $doc_id) {
		// Double-check: Nur wenn Zuordnung wirklich die aktuelle Artikel-ID enthält.
