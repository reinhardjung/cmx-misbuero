...
if (!\class_exists(__NAMESPACE__ . '\\CMX_Kontakt_Auto_Mails')) {
	final class CMX_Kontakt_Auto_Mails {
		...
		private static function build_mail_payload(string $type, int $post_id, array $row, string $event_date): array {
			...
			$body = self::replace_html_template_tokens($body, $type);
		...
		private static function send_contact_mail(string $to, array $payload, string $context) {
			...
			$embedded_logo_listener = static function ($phpmailer) use ($context): void {
				if (!\function_exists(__NAMESPACE__ . '\\cmx_email_embed_self_logo_for_phpmailer')) {
					self::embed_contact_mail_logos_for_phpmailer($phpmailer, $context);
					return;
				}
		...
		private static function replace_html_template_tokens(string $message, string $type = ''): string {
			$thank_you_logo_html = $type === 'thanks' ? self::thank_you_logo_html() : '';
			$birthday_logo_html = $type === 'birthday' ? self::birthday_logo_html() : '';
			$company_birthday_logo_html = $type === 'anniversary' ? self::company_birthday_logo_html() : '';
			...
			if ($type === 'thanks') {
				$message = self::remove_non_thanks_logo_images($message);
			}
		...
		private static function embed_contact_mail_logos_for_phpmailer($phpmailer, string $context): void {
			if ($context === 'kontakt_danke') {
				self::embed_thank_you_logo_for_phpmailer($phpmailer);
				return;
			}

			if ($context === 'kontakt_geburtstag') {
				self::embed_birthday_logo_for_phpmailer($phpmailer);
				return;
			}

			if ($context === 'kontakt_firmenjubilaeum') {
				self::embed_company_birthday_logo_for_phpmailer($phpmailer);
			}
		}

		private static function remove_non_thanks_logo_images(string $message): string {
			return (string) \preg_replace(
				'~<img\b[^>]+(?:cmx-birthday-logo|cmx-company-birthday-logo|assets/birthday(?:_company)?\.png)[^>]*>~i',
				'',
				$message
			);
		}
