...
\add_action('add_meta_boxes', function() {
	...
	\add_meta_box(
		'cmx_kontakte_dokumente',
		$title,
		__NAMESPACE__ . '\\cmx_render_kontakte_dokumente_metabox',
		'kontakte',
		'side',
		'default'
	);
...
function cmx_render_kontakte_dokumente_metabox(\WP_Post $post): void {
	$meta_key = defined(__NAMESPACE__ . '\\CMX_DOK_REL_META')
		? (CMX_DOK_REL_META['kontakte'] ?? 'cmx_dokumente_kontakte')
		: 'cmx_dokumente_kontakte';
	...
	foreach ($docs as $doc_id) {
		// Double-check: Nur wenn Zuordnung wirklich die aktuelle kontakte-ID enthält.
