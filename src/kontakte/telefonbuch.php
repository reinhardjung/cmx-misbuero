	...
	function cmx_telefonbuch_contact_row(int $post_id): array {
		...
		return [
			...
			'detail_url' => \function_exists(__NAMESPACE__ . '\\cmx_telefonbuch_detail_url')
				...
				: '',
			'call_url' => \function_exists(__NAMESPACE__ . '\\cmx_telefonbuch_call_url')
				? (string) cmx_telefonbuch_call_url($post_id)
				: '',
	...
	function cmx_render_telefonbuch_page(): void {
		...
		echo '<style>
			...
			*{box-sizing:border-box}
			...
			.cmx-telefonbuch-head-brand{flex:0 0 auto;display:flex;align-items:flex-start;justify-content:flex-end;min-height:96px}
			.cmx-telefonbuch-head-logo{display:block;width:171px;height:96px;object-fit:contain;object-position:right top}
			...
			.cmx-telefonbuch-call-link{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:999px;border:1px solid #c8d9ec;background:#eef6ff;color:#135e96;text-decoration:none;flex:0 0 auto}
			.cmx-telefonbuch-call-link:hover{background:#ddecfb;color:#0a4b79;border-color:#9fc3e6}
			.cmx-telefonbuch-call-link svg{display:block;width:15px;height:15px;fill:currentColor}
		...
		foreach ($rows as $row) {
			...
			if ($call_url !== '') {
				echo '<a class="cmx-telefonbuch-call-link" href="' . \esc_url($call_url) . '" title="Call-Dossier öffnen" aria-label="Call-Dossier öffnen">';
...
\add_action('template_redirect', function (): void {
	$is_call_request = \function_exists(__NAMESPACE__ . '\\cmx_is_call_request') && cmx_is_call_request();
	if (!cmx_is_telefonbuch_request() && !$is_call_request) {
		return;
	}
