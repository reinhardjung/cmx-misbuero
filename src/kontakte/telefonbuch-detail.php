...
if (!\function_exists(__NAMESPACE__ . '\\cmx_render_telefonbuch_detail_not_found_page')) {
	function cmx_render_telefonbuch_detail_not_found_page(): void {
		...
		$local_ch_url = $number !== ''
			? (string) \add_query_arg('what', $number, 'https://www.local.ch/de/q')
			: 'https://www.local.ch/de/telefonnummernsuche';
		...
		echo '<p><a href="' . \esc_url($back_url) . '">Zurück zum Telefonbuch</a></p>';
		echo '<p><a href="' . \esc_url($local_ch_url) . '" target="_blank" rel="noopener noreferrer">Telefonnummer auf local.ch suchen</a></p>';
		echo '</div></div></div>';
		echo '</body></html>';
		exit;
	}
