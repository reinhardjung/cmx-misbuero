...
require_once __DIR__ . '/telefonbuch.php';
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_kontakte_primary_category_taxonomy')) {
	function cmx_kontakte_primary_category_taxonomy(): string {
		$taxonomies = \function_exists(__NAMESPACE__ . '\\cmx_kontakte_notice_category_taxonomies')
			? cmx_kontakte_notice_category_taxonomies()
			: [];
		if (!empty($taxonomies[0])) {
			return (string) $taxonomies[0];
		}
		return \function_exists(__NAMESPACE__ . '\\cmx_tax_key')
			? (string) cmx_tax_key(basename(__DIR__), 'Kategorien')
			: basename(__DIR__) . '_kategorien';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_kontakte_requested_category_term_id')) {
	function cmx_kontakte_requested_category_term_id(string $taxonomy): int {
		if ($taxonomy === '' || !\taxonomy_exists($taxonomy) || !isset($_GET['cmx_kontakt_kategorie'])) {
			return 0;
		}

		$slug = \sanitize_key((string) \wp_unslash($_GET['cmx_kontakt_kategorie']));
		if ($slug === '') {
			return 0;
		}
...
\add_filter('wp_terms_checklist_args', function (array $args, int $post_id): array {
	$screen = \function_exists('get_current_screen') ? \get_current_screen() : null;
	if (!$screen || (string) ($screen->base ?? '') !== 'post' || (string) ($screen->post_type ?? '') !== basename(__DIR__)) {
		return $args;
	}

	$taxonomy = isset($args['taxonomy']) ? (string) $args['taxonomy'] : '';
	if ($taxonomy === '' || $taxonomy !== cmx_kontakte_primary_category_taxonomy()) {
		return $args;
	}

	$term_id = cmx_kontakte_requested_category_term_id($taxonomy);
	if ($term_id <= 0) {
		return $args;
	}

	$selected = isset($args['selected_cats']) && \is_array($args['selected_cats'])
...
cmx_require_files(__DIR__,'stammdaten,status,zuordnung_kontakt,kommunikation,auto_mails,adressen,bilder,admincolumns,doppelte,stufen,exports,imports,sichern,vcards,carddav,umsatz');
