...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_slot_interval')) {
	function cmx_buchungen_frontend_slot_interval(): int {
		$option_name = \defined(__NAMESPACE__ . '\\CMX_SETTINGS_MAIN')
			? (string) \constant(__NAMESPACE__ . '\\CMX_SETTINGS_MAIN')
			: 'cmx_einstellungen';
		$options = (array) \get_option($option_name, []);
		$value = (int) ($options['buchungen_slot_interval'] ?? 15);
		return \in_array($value, [15, 30, 60], true) ? $value : 15;
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_agb_link')) {
	function cmx_buchungen_frontend_agb_link(): string {
		$options = (array) \get_option(\defined(__NAMESPACE__ . '\\CMX_SETTINGS_MAIN') ? CMX_SETTINGS_MAIN : 'cmx_einstellungen', []);
		$website = isset($options['website']) && \is_array($options['website']) ? $options['website'] : [];
		$legal = isset($website['legal']) && \is_array($website['legal']) ? $website['legal'] : [];

		return \trim((string) ($legal['agb_url'] ?? ''));
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_services')) {
	function cmx_buchungen_frontend_services(): array {
		...
		foreach ($templates as $index => $template) {
			...
			$duration = \max(1, (int) ($template['duration'] ?? ($unit === 'days' ? 1 : 60)));
			$duration_minutes = cmx_buchungen_frontend_duration_minutes($unit, $duration);
			$date_from = \function_exists(__NAMESPACE__ . '\\cmx_buchungen_template_sanitize_date') ? cmx_buchungen_template_sanitize_date($template['date_from'] ?? '') : cmx_buchungen_sanitize_date((string) ($template['date_from'] ?? ''));
			$date_to = \function_exists(__NAMESPACE__ . '\\cmx_buchungen_template_sanitize_date') ? cmx_buchungen_template_sanitize_date($template['date_to'] ?? '') : cmx_buchungen_sanitize_date((string) ($template['date_to'] ?? ''));
			...
			if ($kontakt_id > 0 && \get_post_type($kontakt_id) === 'kontakte') {
				...
				$contact_image = cmx_buchungen_contact_image_url($kontakt_id);
			}
			$image = $contact_image !== '' ? $contact_image : cmx_buchungen_article_image_url($artikel_id);
			...
			$services[] = [
				...
				'kontakt_id' => $kontakt_id,
					...
					'cr' => \in_array($template['cr'] ?? false, [true, 1, '1'], true),
				'image' => $image,
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_date_range_is_free')) {
	function cmx_buchungen_frontend_date_range_is_free(string $start_date, int $days, int $artikel_id = 0): bool {
		static $busy_ranges_by_article = [];
		...
		if (!\array_key_exists($artikel_id, $busy_ranges_by_article)) {
			$busy_ranges = [];
			$meta_query = [
				'relation' => 'AND',
			...
			$busy_ranges_by_article[$artikel_id] = $busy_ranges;
		}

		foreach ($busy_ranges_by_article[$artikel_id] as [$existing_start, $existing_end]) {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_date_allowed_by_weekdays')) {
	function cmx_buchungen_frontend_date_allowed_by_weekdays(string $date, array $weekdays = [], string $date_from = '', string $date_to = ''): bool {
		$date = cmx_buchungen_sanitize_date($date);
		if ($date === '') {
			return false;
		}
		$date_from = cmx_buchungen_sanitize_date($date_from);
		$date_to = cmx_buchungen_sanitize_date($date_to);
		...
		return $weekday > 0 && \in_array($weekday, $weekdays, true);
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_day_amount_is_available')) {
	function cmx_buchungen_frontend_day_amount_is_available(string $start_date, int $days, array $weekdays = [], int $artikel_id = 0, string $date_from = '', string $date_to = ''): bool {
		$start_date = cmx_buchungen_sanitize_date($start_date);
		$days = \max(1, \min(60, $days));
		if ($start_date === '') {
			return false;
		...
		for ($offset = 0; $offset < $days; $offset++) {
			$date = \wp_date('Y-m-d', \strtotime($start_date . ' +' . $offset . ' days'));
			if (!cmx_buchungen_frontend_date_allowed_by_weekdays($date, $weekdays, $date_from, $date_to)) {
				return false;
			}
		}

		return cmx_buchungen_frontend_date_range_is_free($start_date, $days, $artikel_id);
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_day_dates')) {
	function cmx_buchungen_frontend_day_dates(int $days, int $range_days = 90, array $weekdays = [], int $artikel_id = 0, string $date_from = '', string $date_to = ''): array {
		$days = \max(1, \min(60, $days));
		...
		for ($i = 0; $i < $range_days; $i++) {
			...
			if (cmx_buchungen_frontend_day_amount_is_available($date, $days, $weekdays, $artikel_id, $date_from, $date_to)) {
				$dates[] = $date;
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_hour_amount_options')) {
	function cmx_buchungen_frontend_hour_amount_options(string $date, string $time, int $max_hours = 60, string $period = 'all', array $weekdays = [], int $artikel_id = 0, string $date_from = '', string $date_to = ''): array {
		$date = cmx_buchungen_sanitize_date($date);
		$time = cmx_buchungen_sanitize_time($time);
		$max_hours = \max(1, \min(60, $max_hours));
		$artikel_id = \max(0, $artikel_id);
		if ($date === '' || $time === '' || !cmx_buchungen_frontend_date_allowed_by_weekdays($date, $weekdays, $date_from, $date_to)) {
			return [];
		}

		$start = \function_exists(__NAMESPACE__ . '\\cmx_buchungen_time_to_minutes') ? cmx_buchungen_time_to_minutes($time) : -1;
		if ($start < 0) {
			return [];
		}

		$period = \in_array($period, ['all', 'morning', 'afternoon'], true) ? $period : 'all';
		$period_limit = 1440;
		if ($period === 'morning') {
			$period_limit = 720;
		...
		foreach (cmx_buchungen_frontend_opening_hours_for_weekday($weekday, $weekdays) as $range) {
			...
			if ($from >= 0 && $to > $from && $start >= $from && $start < $to) {
				$range_limit = \max($range_limit, \min($to, $period_limit, 1440));
			}
		}
		if ($range_limit <= $start) {
			return [];
		}

		$options = [];
		for ($hours = 1; $hours <= $max_hours; $hours++) {
			$duration = $hours * 60;
			if ($start + $duration > $range_limit) {
				break;
			}
			if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_slot_is_free') || cmx_buchungen_slot_is_free($date, $time, $duration, 0, 0, 0, $artikel_id)) {
				$options[] = $hours;
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_hour_amount_options_for_slots')) {
	function cmx_buchungen_frontend_hour_amount_options_for_slots(array $slots_by_date, int $max_hours = 60, string $period = 'all', array $weekdays = [], int $artikel_id = 0, string $date_from = '', string $date_to = ''): array {
		$out = [];
		foreach ($slots_by_date as $date => $times) {
			$date = cmx_buchungen_sanitize_date((string) $date);
			if ($date === '' || !\is_array($times)) {
				continue;
			}
			foreach ($times as $time) {
				$time = cmx_buchungen_sanitize_time((string) $time);
				if ($time === '') {
					continue;
				}
				$options = cmx_buchungen_frontend_hour_amount_options($date, $time, $max_hours, $period, $weekdays, $artikel_id, $date_from, $date_to);
				if ($options !== []) {
					$out[$date][$time] = $options;
				}
			}
		}
		return $out;
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_slots')) {
	function cmx_buchungen_frontend_slots(int $duration, int $days = 42, array $weekdays = [], int $artikel_id = 0, int $interval = 15, string $date_from = '', string $date_to = ''): array {
		$duration = \max(1, $duration);
		$days = \max(1, \min(90, $days));
		$interval = \in_array($interval, [15, 30, 60], true) ? $interval : cmx_buchungen_frontend_slot_interval();
		$weekdays = \function_exists(__NAMESPACE__ . '\\cmx_buchungen_template_sanitize_weekdays') ? cmx_buchungen_template_sanitize_weekdays($weekdays) : $weekdays;
		$weekdays = $weekdays !== [] ? $weekdays : [1, 2, 3, 4, 5];
		$artikel_id = \max(0, $artikel_id);
		$today = \wp_date('Y-m-d');
		...
		for ($i = 0; $i < $days; $i++) {
			...
			foreach (cmx_buchungen_frontend_opening_hours_for_weekday($weekday, $weekdays) as $range) {
				...
				for ($minute = $from; $minute + $duration <= $to; $minute += $interval) {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_find_or_create_contact')) {
	function cmx_buchungen_frontend_find_or_create_contact(string $name, string $email, string $phone, array $address = []) {
		...
		$zip = isset($address['zip']) ? \sanitize_text_field((string) $address['zip']) : '';
		...
		if (\post_type_exists('kontakte')) {
			...
			if ($contact_id > 0) {
				...
				cmx_buchungen_frontend_update_contact_address($contact_id, $street, $zip, $city);
				return $contact_id;
			}
			...
			$contact_id = \wp_insert_post([
				...
				'meta_input' => [
					...
					'_cmx_rechnung_strasse' => $street,
					'_cmx_rechnung_plz' => $zip,
					'_cmx_rechnung_ort' => $city,
				],
			], true);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_update_contact_address')) {
	function cmx_buchungen_frontend_update_contact_address(int $contact_id, string $street, string $zip, string $city): void {
		...
		foreach ([
			...
			if ($value !== '') {
				\update_post_meta($contact_id, $meta_key, $value);
			}
		}
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_upload_license')) {
	function cmx_buchungen_frontend_upload_license(int $booking_id, int $contact_id): int {
		if ($booking_id <= 0 || $contact_id <= 0 || empty($_FILES['license_file']) || empty($_FILES['license_file']['tmp_name'])) {
			return 0;
		}

		$file_type = \wp_check_filetype_and_ext((string) $_FILES['license_file']['tmp_name'], (string) $_FILES['license_file']['name']);
		...
		$attachment_id = \media_handle_upload('license_file', $booking_id);
		if (\is_wp_error($attachment_id) || (int) $attachment_id <= 0) {
			return 0;
		}

		$attachment_id = (int) $attachment_id;
		\update_post_meta($booking_id, '_cmx_buchung_fuehrerausweis_attachment_id', $attachment_id);
		if (\function_exists(__NAMESPACE__ . '\\cmx_vermietung_sync_license_document')) {
			$doc_id = (int) cmx_vermietung_sync_license_document($attachment_id, $contact_id);
			if ($doc_id > 0) {
				\update_post_meta($booking_id, '_cmx_buchung_fuehrerausweis_document_id', $doc_id);
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_error_message')) {
	function cmx_buchungen_frontend_error_message(string $code): string {
		return match ($code) {
			...
			'agb' => 'Bitte die AGB akzeptieren.',
			'upload' => 'Der Führerausweis konnte nicht gespeichert werden. Bitte eine Bilddatei wählen oder ohne Datei fortfahren.',
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_handle_submit')) {
	function cmx_buchungen_frontend_handle_submit(): void {
		...
		$street = isset($_POST['street']) ? \sanitize_text_field((string) \wp_unslash($_POST['street'])) : '';
		$zip = isset($_POST['zip']) ? \sanitize_text_field((string) \wp_unslash($_POST['zip'])) : '';
		...
		$is_cr_booking = $selected_service !== null && !empty($selected_service['cr']);
		$agb_link = cmx_buchungen_frontend_agb_link();
		$raw_duration = $selected_service !== null ? \max(1, (int) ($selected_service['duration'] ?? 60)) : 60;
		$booking_amount = isset($_POST['booking_days']) ? \max(1, \min(999, (int) \wp_unslash($_POST['booking_days']))) : $raw_duration;
		$duration = cmx_buchungen_frontend_duration_minutes($unit, $raw_duration);
		if (\in_array($unit, ['days', 'hours'], true)) {
			$raw_duration = \min(60, $booking_amount);
			$duration = cmx_buchungen_frontend_duration_minutes($unit, $raw_duration);
		}
		if ($unit === 'days') {
			$time = '00:00';
		...
		if ($agb_link !== '' && empty($_POST['agb_accepted'])) {
			\wp_safe_redirect(cmx_buchungen_frontend_url(['cmx_booking_error' => 'agb']));
			exit;
		}
		...
		if ($unit === 'days' && !cmx_buchungen_frontend_day_amount_is_available($date, $raw_duration, (array) ($selected_service['weekdays'] ?? []), $service_id, (string) ($selected_service['date_from'] ?? ''), (string) ($selected_service['date_to'] ?? ''))) {
			\wp_safe_redirect(cmx_buchungen_frontend_url(['cmx_booking_error' => 'slot']));
			exit;
		}
		if ($unit === 'hours' && !\in_array($raw_duration, cmx_buchungen_frontend_hour_amount_options($date, $time, 60, (string) ($selected_service['period'] ?? 'all'), (array) ($selected_service['weekdays'] ?? []), $service_id, (string) ($selected_service['date_from'] ?? ''), (string) ($selected_service['date_to'] ?? '')), true)) {
			\wp_safe_redirect(cmx_buchungen_frontend_url(['cmx_booking_error' => 'slot']));
			exit;
		}
		if ($unit !== 'days') {
			...
			if (!cmx_buchungen_slot_is_free($date, $time, $duration, 0, 0, 0, $service_id)) {
				\wp_safe_redirect(cmx_buchungen_frontend_url(['cmx_booking_error' => 'slot']));
				exit;
			}
		}
		...
		$booking_id = \wp_insert_post([
			...
			'meta_input' => [
				...
				'_cmx_buchung_quantity' => \in_array($unit, ['days', 'hours'], true) ? (string) $raw_duration : '',
				'_cmx_buchung_cr' => $is_cr_booking ? '1' : '0',
				'_cmx_buchung_strasse' => $street,
				'_cmx_buchung_plz' => $zip,
				'_cmx_buchung_ort' => $city,
				'_cmx_buchung_agb_accepted' => !empty($_POST['agb_accepted']) ? '1' : '0',
				'_cmx_buchung_agb_link' => $agb_link !== '' ? $agb_link : '',
				CMX_BUCHUNGEN_META_STATUS => 'angefragt',
		...
		if ($is_cr_booking && !empty($_FILES['license_file']) && !empty($_FILES['license_file']['tmp_name']) && cmx_buchungen_frontend_upload_license($booking_id, (int) $contact_id) <= 0) {
			\wp_safe_redirect(cmx_buchungen_frontend_url(['cmx_booking_error' => 'upload']));
			exit;
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_calendar_dates')) {
	function cmx_buchungen_frontend_calendar_dates(string $date, string $time, int $duration, string $unit = ''): array {
		...
		$end = $start->modify('+' . \max(1, $duration) . ' minutes');
		return [
			'all_day' => false,
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_render_page')) {
	function cmx_buchungen_frontend_render_page(): void {
		...
		$day_dates = [];
		$amount_options = [];
		$slot_interval = cmx_buchungen_frontend_slot_interval();
		foreach ($services as $service) {
			if ((string) ($service['unit'] ?? 'minutes') === 'days') {
				$service_day_dates = [];
				$service_article_id = \max(0, (int) ($service['artikel_id'] ?? 0));
				$service_date_from = (string) ($service['date_from'] ?? '');
				$service_date_to = (string) ($service['date_to'] ?? '');
				for ($days = 1; $days <= 60; $days++) {
					$service_day_dates[(string) $days] = cmx_buchungen_frontend_day_dates($days, 90, (array) ($service['weekdays'] ?? [1, 2, 3, 4, 5]), $service_article_id, $service_date_from, $service_date_to);
				}
			...
			$slot_cache_key = $duration_key . ':' . $slot_interval . ':' . $service_article_id . ':' . \implode(',', \array_map('intval', $service_weekdays)) . ':' . $service_date_from . ':' . $service_date_to;
			if (!isset($slots[$duration_key])) {
				$slots[$duration_key] = cmx_buchungen_frontend_slots((int) ($service['duration_minutes'] ?? $service['duration']), 42, [1, 2, 3, 4, 5], 0, $slot_interval);
			}
			if (!isset($slots[$slot_cache_key])) {
				$slots[$slot_cache_key] = cmx_buchungen_frontend_slots((int) ($service['duration_minutes'] ?? $service['duration']), 42, $service_weekdays, $service_article_id, $slot_interval, $service_date_from, $service_date_to);
			}
			$slots[(string) ($service['id'] ?? '')] = $slots[$slot_cache_key];
			if ((string) ($service['unit'] ?? 'minutes') === 'hours') {
				$amount_options[(string) ($service['id'] ?? '')] = cmx_buchungen_frontend_hour_amount_options_for_slots(
					$slots[$slot_cache_key],
					60,
		...
		$booking_quantity = $confirmed_id > 0 ? \max(1, (int) \get_post_meta($confirmed_id, '_cmx_buchung_quantity', true)) : 1;
		...
		echo '<style>
			...
			*{box-sizing:border-box}
			...
			.cmx-booking-head-brand{flex:0 0 auto;display:flex;align-items:flex-start;justify-content:flex-end;min-height:96px}
			.cmx-booking-head-logo{display:block;width:171px;height:96px;object-fit:contain;object-position:right top}
			...
			.cmx-booking-avatar{width:58px;height:58px;border-radius:999px;background:#eef5ff;display:flex;align-items:center;justify-content:center;font-weight:800;color:#475569;flex:0 0 auto;overflow:hidden}
			img.cmx-booking-avatar{box-sizing:border-box;padding:7px;object-fit:contain}
			...
			.cmx-booking-scheduler,.cmx-booking-form{scroll-margin-top:90px}
			...
			.cmx-booking-slots{display:flex;flex-direction:column;gap:12px;padding-top:0;max-height:500px;overflow:auto}
			...
			.cmx-booking-slot.is-selected{border-color:#2563eb;background:#eff6ff;padding-right:8px}
			.cmx-booking-next-controls{display:inline-flex;align-items:center;gap:6px;margin-left:auto}
			.cmx-booking-next-controls input{box-sizing:border-box;width:46px;border:1px solid #cfd5df;border-radius:6px;padding:7px 4px;font:inherit;font-weight:700;text-align:center;background:#fff}
			.cmx-booking-count-label{flex:0 0 60px;color:#4b5563;font-size:13px;font-weight:700;text-align:left;white-space:nowrap}
			.cmx-booking-next{border:0;border-radius:4px;background:#2563eb;color:#fff;padding:8px 14px;font-weight:700;cursor:pointer;transform:translateX(2px)}
			...
			.cmx-booking-form{max-width:780px;margin:98px auto 0;border:1px solid #d6dbe3;border-radius:18px;padding:28px;background:#fff}
			.cmx-booking-form [hidden]{display:none !important}
			...
			.cmx-booking-form input[type=file]{padding:9px 12px;background:#fff}
			.cmx-booking-agb-label{grid-column:1/-1;display:flex !important;flex-direction:row !important;align-items:center;gap:8px !important;font-weight:600 !important;color:#4b5563 !important;white-space:nowrap}
			.cmx-booking-agb-label span{white-space:nowrap}
			.cmx-booking-agb-label input{width:auto !important;flex:0 0 auto}
			.cmx-booking-agb-label a{color:#2563eb;text-decoration:none}
			.cmx-booking-agb-label a:hover{text-decoration:underline}
			...
			.cmx-booking-confirm-cal a{display:inline-flex;align-items:center;gap:9px;color:#1e3a8a;text-decoration:none}
			.cmx-booking-confirm-cal a:hover{text-decoration:underline}
			.cmx-booking-cal-icon{display:block;width:20px;height:20px;object-fit:contain;flex:0 0 20px}
		...
		echo '</head><body><main class="cmx-booking-page"><section class="' . \esc_attr($shell_classes) . '" data-cmx-booking-app data-services="' . \esc_attr(cmx_buchungen_frontend_json($services)) . '" data-slots="' . \esc_attr(cmx_buchungen_frontend_json($slots)) . '" data-day-dates="' . \esc_attr(cmx_buchungen_frontend_json($day_dates)) . '" data-amount-options="' . \esc_attr(cmx_buchungen_frontend_json($amount_options)) . '">';
		...
		if ($confirmed_id > 0 && $confirmed_service !== null) {
			...
			if ($booking_date !== '' && $booking_time !== '') {
				$start_label = $is_day_booking
					...
					: \wp_date('H:i', \strtotime($booking_date . ' ' . $booking_time)) . ' - ' . \wp_date('H:i', \strtotime($booking_date . ' ' . $booking_time . ' +' . \max(1, $booking_duration) . ' minutes')) . ', ' . \wp_date('l, d. F Y', \strtotime($booking_date));
			}
			...
			$confirmed_amount_label = cmx_buchungen_frontend_duration_label($booking_duration, $confirmed_unit);
			echo '<div class="cmx-booking-confirm-row"><span class="cmx-booking-icon">●</span><span>' . \esc_html($confirmed_amount_label) . '</span></div>';
			echo '<div class="cmx-booking-confirm-cal">';
			$apple_icon_url = \defined('CMX_PLUGIN_URL') ? (string) \constant('CMX_PLUGIN_URL') . 'assets/cal_apple.png' : '';
			$google_icon_url = \defined('CMX_PLUGIN_URL') ? (string) \constant('CMX_PLUGIN_URL') . 'assets/cal_google.png' : '';
			if ($ics_url !== '') {
				echo '<a href="' . \esc_url($ics_url) . '">' . ($apple_icon_url !== '' ? '<img class="cmx-booking-cal-icon" src="' . \esc_url($apple_icon_url) . '" alt="" aria-hidden="true">' : '') . '<span>ICS-Datei herunterladen</span></a>';
			}
			if ($google_url !== '') {
				echo '<a href="' . \esc_url($google_url) . '" target="_blank" rel="noopener noreferrer">' . ($google_icon_url !== '' ? '<img class="cmx-booking-cal-icon" src="' . \esc_url($google_icon_url) . '" alt="" aria-hidden="true">' : '') . '<span>In Google Kalender eintragen</span></a>';
			}
			echo '</div>';
			...
			if ($services === []) {
				...
				foreach ($services as $service) {
					...
					$service_unit = (string) ($service['unit'] ?? 'minutes');
					$service_duration_label = cmx_buchungen_frontend_duration_label((int) ($service['duration_minutes'] ?? $service['duration']), $service_unit);
					echo '<span class="cmx-booking-duration">' . \esc_html($service_duration_label) . '</span>';
			...
			echo '<div class="cmx-booking-form-grid"><label>Name<input name="name" autocomplete="name" required></label><label class="cmx-booking-field-email">E-Mail<input type="email" name="email" autocomplete="email" required></label><label class="cmx-booking-field-phone">Telefon<input name="phone" autocomplete="tel"></label><label class="cmx-booking-field-street">Strasse<input name="street" autocomplete="street-address"></label><label class="cmx-booking-field-zip">PLZ<input name="zip" autocomplete="postal-code"></label><label class="cmx-booking-field-city">Ort<input name="city" autocomplete="address-level2"></label><label class="is-wide">Termin<input data-form-summary readonly tabindex="-1" aria-readonly="true"></label><label class="is-wide" data-cr-extra hidden>Führerausweis<input type="file" name="license_file" accept="image/*" data-cr-field disabled></label>';
			if ($agb_link !== '') {
				echo '<label class="cmx-booking-agb-label"><input type="checkbox" name="agb_accepted" value="1" required> <span>Ich habe die <a href="' . \esc_url($agb_link) . '" target="_blank" rel="noopener noreferrer">AGB gelesen</a>, verstanden und akzeptiert.</span></label>';
			}
			echo '<label class="is-wide">Notiz<textarea name="note"></textarea></label><p class="cmx-booking-form-note">Du erhältst sofort eine Eingangsbestätigung per E-Mail.<br/>Die Buchung wird erst nach zusätzlicher manueller Bestätigung verbindlich.</p></div><div class="cmx-booking-form-actions"><button type="button" class="cmx-booking-link" data-back-schedule>Zurück</button><button type="submit" class="cmx-booking-button">buchen</button></div></form>';
		}
		...
		(function(){
			...
			function bookingScrollOffset(extra){ return extra || 0; }
			...
			function isAmountService(){ return selectedService && ["days","hours"].indexOf(String(selectedService.unit || "minutes")) !== -1; }
			...
			function serviceDayDateSource(){ return selectedService ? (dayDates[String(selectedService.id)] || {}) : {}; }
			...
			function serviceAmountOptions(){ return selectedService ? (amountOptions[String(selectedService.id)] || {}) : {}; }
			function dayAmountOptions(dateKey){
				...
				if(source && !Array.isArray(source)){
					Object.keys(source).forEach(function(count){
						...
						if(dates.indexOf(String(dateKey)) !== -1) options.push(parseInt(count, 10));
					});
				}
				return options.filter(function(value){ return Number.isFinite(value) && value > 0; }).sort(function(a, b){ return a - b; });
			}
			function hourAmountOptions(dateKey, time){
				...
				return options.map(function(value){ return parseInt(value, 10); }).filter(function(value){ return Number.isFinite(value) && value > 0; }).sort(function(a, b){ return a - b; });
			}
				function availableAmounts(){
					if(!selectedService) return [];
					if(isDayService()) return dayAmountOptions(selectedDate);
					if(String(selectedService.unit || "minutes") === "hours") return selectedTime ? hourAmountOptions(selectedDate, selectedTime) : [];
					return [];
				}
				function normalizeAmount(value){
					...
					if(options.indexOf(requested) !== -1) return requested;
					for(var i = options.length - 1; i >= 0; i--){
						if(options[i] <= requested) return options[i];
					}
					return options[0];
				}
				...
				var options = availableAmounts();
					...
					if(options.indexOf(selectedDayCount) === -1){
						selectedDayCount = normalizeAmount(selectedService.duration || 1);
					}
				}
			function durationLabel(){
				if(!selectedService) return "";
				var unit = String(selectedService.unit || "minutes");
				var duration = isAmountService() ? selectedDayCount : parseInt(selectedService.duration || 60, 10);
				if(isDayService()) return String(duration) + (duration === 1 ? " Tag" : " Tage");
				if(unit === "hours") return String(duration) + (duration === 1 ? " Stunde" : " Stunden");
				return String(duration) + " Minuten";
			}
			function amountLabel(){
				if(!selectedService) return "";
				var unit = String(selectedService.unit || "minutes");
				if(unit === "days") return selectedDayCount === 1 ? "Tag" : "Tage";
				if(unit === "hours") return selectedDayCount === 1 ? "Stunde" : "Stunden";
				return "";
			}
			function bookingModeLabel(){
				if(!selectedService) return "";
				var unit = String(selectedService.unit || "minutes");
				if(unit === "days") return "Tagesbuchung";
				if(unit === "hours") return "Auf Stundenbasis";
				return "Fixe Zeit";
			}
			...
			function appendNextControls(row){
				var controls = document.createElement("span");
					controls.className = "cmx-booking-next-controls";
					if(isAmountService()){
						syncSelectedAmount();
						var options = availableAmounts();
						...
						input.addEventListener("change", function(event){
							...
							label.textContent = amountLabel();
							updateSide();
							if(isDayService()) renderCalendar();
							renderSlots();
						});
					var label = document.createElement("span");
					label.className = "cmx-booking-count-label";
					label.textContent = amountLabel();
			...
			function openSchedule(serviceId, dayCount){
				...
				if(!selectedService) return;
				selectedDayCount = isAmountService() ? Math.max(1, Math.min(60, parseInt(dayCount || selectedService.duration || 1, 10))) : 1;
				...
				renderSlots();
				showStep("schedule");
				scrollBookingElementIntoView(qs(".cmx-booking-scheduler") || qs("[data-step=\"schedule\"]"), 68);
			}
				...
				function scrollToBookingForm(){
					...
					scrollBookingElementIntoView(form, 80);
				}
				function openForm(){
				...
				qs("[data-form-summary]").value = (isDayService() ? summaryDate + ", " + durationLabel() : fmtTime(selectedTime) + ", " + selectedDate + (isAmountService() ? ", " + durationLabel() : "")) + " - " + selectedService.title;
					...
					if(name){
						try{ name.focus({preventScroll:true}); }catch(err){ name.focus(); }
					}
					scrollToBookingForm();
				}
