...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_agb_link')) {
	function cmx_buchungen_frontend_agb_link(): string {
		if (\function_exists(__NAMESPACE__ . '\\cmx_carent_agb_link')) {
			return \trim((string) cmx_carent_agb_link());
		}
		if (\function_exists(__NAMESPACE__ . '\\cmx_email_agb_link')) {
			$email_link = \trim((string) cmx_email_agb_link());
			if ($email_link !== '') {
				return $email_link;
			}
		}
		...
		return $attachment_id > 0 ? \trim((string) \wp_get_attachment_url($attachment_id)) : '';
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_day_dates')) {
	function cmx_buchungen_frontend_day_dates(int $days, int $range_days = 90, array $weekdays = [], int $artikel_id = 0): array {
		$days = \max(1, \min(60, $days));
		...
		for ($i = 0; $i < $range_days; $i++) {
			...
			if (cmx_buchungen_frontend_date_range_is_free($date, $days, $artikel_id)) {
				$dates[] = $date;
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_slots')) {
	function cmx_buchungen_frontend_slots(int $duration, int $days = 42, array $weekdays = [], int $artikel_id = 0): array {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_buchungen_frontend_handle_submit')) {
	function cmx_buchungen_frontend_handle_submit(): void {
		...
		if ($is_cr_booking && $agb_link !== '' && empty($_POST['agb_accepted'])) {
		...
		if ($unit === 'days' && !cmx_buchungen_frontend_date_range_is_free($date, $raw_duration, $service_id)) {
			\wp_safe_redirect(cmx_buchungen_frontend_url(['cmx_booking_error' => 'slot']));
			exit;
		}
		if ($unit !== 'days') {
		...
		$contact_id = cmx_buchungen_frontend_find_or_create_contact($name, $email, $phone, [
			'street' => $is_cr_booking ? $street : '',
			'zip' => $is_cr_booking ? $zip : '',
			'city' => $is_cr_booking ? $city : '',
		]);
		if (\is_wp_error($contact_id)) {
		...
		$booking_id = \wp_insert_post([
			...
			'meta_input' => [
				...
				'_cmx_buchung_strasse' => $is_cr_booking ? $street : '',
				'_cmx_buchung_plz' => $is_cr_booking ? $zip : '',
				'_cmx_buchung_ort' => $is_cr_booking ? $city : '',
				'_cmx_buchung_agb_accepted' => ($is_cr_booking && $agb_link !== '') ? '1' : '0',
				'_cmx_buchung_agb_link' => ($is_cr_booking && $agb_link !== '') ? $agb_link : '',
	...
	function cmx_buchungen_frontend_render_page(): void {
		...
		foreach ($services as $service) {
			if ((string) ($service['unit'] ?? 'minutes') === 'days') {
				...
				for ($days = 1; $days <= 60; $days++) {
					$service_day_dates[(string) $days] = cmx_buchungen_frontend_day_dates($days, 90, (array) ($service['weekdays'] ?? [1, 2, 3, 4, 5]), $service_article_id);
				}
			...
			$slot_cache_key = $duration_key . ':' . $service_article_id . ':' . \implode(',', \array_map('intval', $service_weekdays));
			...
			if (!isset($slots[$slot_cache_key])) {
				$slots[$slot_cache_key] = cmx_buchungen_frontend_slots((int) ($service['duration_minutes'] ?? $service['duration']), 42, $service_weekdays, $service_article_id);
			}
		...
		echo '<style>
			...
			*{box-sizing:border-box}
			...
			.cmx-booking-agb-label{grid-column:1/-1;display:flex !important;flex-direction:row !important;align-items:center;gap:8px !important;font-weight:600 !important;color:#4b5563 !important}
