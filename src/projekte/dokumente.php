...
\add_action('add_meta_boxes', function() {
	...
	\add_meta_box(
		'cmx_projekte_dokumente',
		$title,
		__NAMESPACE__ . '\\cmx_render_projekte_dokumente_metabox',
		'projekte',
		'side',
		'default'
	);
...
function cmx_render_projekte_dokumente_metabox(\WP_Post $post): void {
	$meta_key = defined(__NAMESPACE__ . '\\CMX_DOK_REL_META')
		? (CMX_DOK_REL_META['projekte'] ?? 'cmx_dokumente_projekte')
	...
	foreach ($docs as $doc_id) {
		// Double-check: Nur wenn Zuordnung wirklich die aktuelle projekte-ID enthält.
