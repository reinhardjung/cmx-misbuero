...
function cmx_get_beleg_field_label_html(string $label, string $key): string {
	...
	$title = cmx_starts_with($key, 'mail_') ? 'Vorlage laden' : 'Text aus globales.ini laden';
	...
	if ($is_mail_field || $is_belegfuss_field) {
		...
		\wp_editor($display, $editor_id, [
			...
			'tinymce' => [
				...
				if ($is_mail_field) {
					cmx_render_beleg_placeholder_buttons($editor_id, ['{anrede}', '{tageszeit}', '{vorname}', '{nachname}', '{firma}', '{bezeichnung}', '{beleg}', '{beleg_datum}', '{faellig_bis}', '{betrag}', '{logo}']);
				}
				return;
			}
