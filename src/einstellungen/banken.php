...
if (!\function_exists(__NAMESPACE__ . '\\cmx_get_payrexx_api_key')) {
	function cmx_get_payrexx_api_key(): string {
		$options = (array) \get_option(CMX_SETTINGS_MAIN, []);
		$value = $options['payrexx_api_key'] ?? '';
		return \is_scalar($value) ? (string) $value : '';
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_render_payrexx_api_key_field')) {
	function cmx_render_payrexx_api_key_field(): void {
		$value = \esc_attr(cmx_get_payrexx_api_key());
		echo '<input type="password" class="regular-text" name="' . \esc_attr(CMX_SETTINGS_MAIN) . '[payrexx_api_key]" value="' . $value . '" placeholder="API-Key" autocomplete="off">';
		echo '<p class="description">Den <code>API-Key</code> findest Du in Payrexx im Bereich API / Schnittstellen. Wird für spätere Payrexx-API-Funktionen gespeichert.</p>';
	}
}
...
function cmx_register_banken_tab(): void {
	...
	\add_settings_field(
		'payrexx_api_key',
		'API-Key',
		__NAMESPACE__ . '\\cmx_render_payrexx_api_key_field',
		'cmx_tab_banken',
		'cmx_sec_payrexx'
	);
...
\add_filter('pre_update_option_' . CMX_SETTINGS_MAIN, static function ($new, $old) {
	...
	if (\array_key_exists('payrexx_api_key', $new)) {
		$new['payrexx_api_key'] = \sanitize_text_field((string) $new['payrexx_api_key']);
	}
	return $new;
}, 20, 2);
