...
function cmx_register_vorgaben_tab(): void {
	...
	\add_settings_section(
		...
		\add_settings_field(
			...
			function () {
				...
				echo '<p class="description">Standard: PDF-Beleg herunterladen. Das PDF-Icon wird in der E-Mail immer angezeigt.</p>';
			},
			'cmx_tab_vorgaben__email',
			'cmx_sec_vorgaben_email'
		);
		...
		\add_settings_field(
			'belege_kontakt_ansprechpartner_uebernehmen',
			'Kontaktadresse',
			function () {
				$opts = (array) \get_option(\CLOUDMEISTER\CMX\Buero\CMX_SETTINGS_MAIN, []);
				$enabled = \array_key_exists('belege_kontakt_ansprechpartner_uebernehmen', $opts)
					? !empty($opts['belege_kontakt_ansprechpartner_uebernehmen'])
					: true;

				echo '<label>';
				echo '<input type="hidden" name="'.\CLOUDMEISTER\CMX\Buero\CMX_SETTINGS_MAIN.'[belege_kontakt_ansprechpartner_uebernehmen]" value="0">';
				echo '<input type="checkbox" name="'.\CLOUDMEISTER\CMX\Buero\CMX_SETTINGS_MAIN.'[belege_kontakt_ansprechpartner_uebernehmen]" value="1" '.\checked($enabled, true, false).'> ';
				echo 'Ansprechpartner bei der Kontakt-Auswahl mit übernehmen';
				echo '</label>';
				echo '<p class="description">Wenn deaktiviert, wird beim Übernehmen eines Kontakts die Firmenadresse ohne Ansprechpartner in den Beleg geschrieben. Bei Privatkontakten ohne Firma bleibt der Name erhalten.</p>';
			},
			'cmx_tab_vorgaben__belege',
			'cmx_sec_vorgaben_belege'
		);
		...
		\add_settings_field(
			'belege_kontakt_land_kennung',
			'Länderkennung',
			function () {
				$opts = (array) \get_option(\CLOUDMEISTER\CMX\Buero\CMX_SETTINGS_MAIN, []);
				$current = isset($opts['belege_kontakt_land_kennung']) ? \sanitize_key((string) $opts['belege_kontakt_land_kennung']) : 'immer';
				if (!\in_array($current, ['immer', 'ausland', 'nie'], true)) {
					$current = 'immer';
				}
				$options = [
					'immer'   => 'Immer anzeigen, z.B. CH-8000 Zürich',
					'ausland' => 'Nur bei Ausland anzeigen',
					'nie'     => 'Nie anzeigen',
				];

				echo '<select name="'.\CLOUDMEISTER\CMX\Buero\CMX_SETTINGS_MAIN.'[belege_kontakt_land_kennung]" style="min-width:320px;">';
				foreach ($options as $value => $label) {
					echo '<option value="'.\esc_attr($value).'" '.\selected($current, $value, false).'>'.\esc_html($label).'</option>';
				}
				echo '</select>';
				echo '<p class="description">Steuert die Länderkennung beim Übernehmen der Kontaktadresse in einen Beleg.</p>';
			},
			'cmx_tab_vorgaben__belege',
			'cmx_sec_vorgaben_belege'
		);
	...
	\register_setting(
		...
		[
			...
			'sanitize_callback' => function ($val) {
				if (!\array_key_exists('cmx_katalog_online', $_POST)) {
					return (string) \get_option('cmx_katalog_online', '0');
				}
				return $val ? '1' : '0';
			},
		]
	...
	\add_settings_field(
		...
		function () {
			...
			echo '<input type="hidden" name="cmx_katalog_online" value="0">';
