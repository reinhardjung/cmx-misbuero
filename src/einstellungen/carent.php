...
if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_mail_transfer_template_key')) {
	function cmx_carent_mail_transfer_template_key(): string {
		return 'carent_mail_transfer_template';
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_spatzig_key')) {
	function cmx_carent_spatzig_key(): string {
		return 'carent_spatzig';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_utilization_mode_key')) {
	function cmx_carent_utilization_mode_key(): string {
		return 'carent_utilization_mode';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_target_hours_per_day_key')) {
	function cmx_carent_target_hours_per_day_key(): string {
		return 'carent_target_hours_per_day';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_utilization_mode_options')) {
	function cmx_carent_utilization_mode_options(): array {
		return [
			'calendar_days'       => 'Kalendertage',
			'twentyfour_hour_days'=> '24h-basiert',
			'hourly'              => 'stundenbasiert',
		];
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_normalize_target_hours_per_day')) {
	function cmx_carent_normalize_target_hours_per_day(mixed $value): float {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_current_utilization_mode')) {
	function cmx_carent_current_utilization_mode(): string {
		$options = (array) \get_option(cmx_carent_settings_option_name(), []);
		$value = \sanitize_key((string) ($options[cmx_carent_utilization_mode_key()] ?? ''));
		return \array_key_exists($value, cmx_carent_utilization_mode_options()) ? $value : 'twentyfour_hour_days';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_current_target_hours_per_day')) {
	function cmx_carent_current_target_hours_per_day(): float {
		$options = (array) \get_option(cmx_carent_settings_option_name(), []);
		return cmx_carent_normalize_target_hours_per_day($options[cmx_carent_target_hours_per_day_key()] ?? 24);
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_mail_template_default')) {
	function cmx_carent_mail_template_default(string $key): string {
		if ($key === cmx_carent_mail_transfer_template_key()) {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_current_spatzig')) {
	function cmx_carent_current_spatzig(): int {
		$options = (array) \get_option(cmx_carent_settings_option_name(), []);
		return isset($options[cmx_carent_spatzig_key()]) ? \max(0, (int) $options[cmx_carent_spatzig_key()]) : 0;
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_mail_placeholders')) {
	function cmx_carent_mail_placeholders(): array {
		return ['{anrede}', '{tageszeit}', '{vorname}', '{nachname}', '{firma}', '{vertrag}', '{vertrags_id}', '{fahrzeug}', '{kennzeichen}', '{uebernahme}', '{rueckgabe}', '{betrag}', '{besondere_abmachungen}', '{logo}'];
	}
}
...
\add_action('admin_init', function (): void {
	...
	\add_settings_field(
		'cmx_carent_mail_transfer_template',
		'Übernahme',
		static function (): void {
			cmx_carent_render_mail_editor(cmx_carent_mail_transfer_template_key(), 'cmx_carent_mail_transfer_template');
		},
		$page,
		'cmx_sec_carent'
	);
	...
	\add_settings_field(
		'cmx_carent_utilization_mode',
		'Auslastung berechnen',
		static function (): void {
			$option_name = cmx_carent_settings_option_name();
			$key = cmx_carent_utilization_mode_key();
			$current = cmx_carent_current_utilization_mode();
			echo '<select id="cmx-carent-utilization-mode" name="' . \esc_attr($option_name . '[' . $key . ']') . '" style="height:40px;min-height:40px;line-height:38px;padding-top:0;padding-bottom:0;">';
			foreach (cmx_carent_utilization_mode_options() as $value => $label) {
				echo '<option value="' . \esc_attr((string) $value) . '"' . \selected($current, (string) $value, false) . '>' . \esc_html((string) $label) . '</option>';
			}
			echo '</select>';
			echo '<p class="description">Steuert die Berechnung der Auslastung im CaRent-Dashboard.</p>';
		},
		$page,
		'cmx_sec_carent'
	);
	...
	\add_settings_field(
		'cmx_carent_target_hours_per_day',
		'Zielstunden pro Tag',
		static function (): void {
			$option_name = cmx_carent_settings_option_name();
			$key = cmx_carent_target_hours_per_day_key();
			$current = cmx_carent_current_target_hours_per_day();
			echo '<input type="number" min="0.25" max="24" step="0.25" class="small-text" id="cmx-carent-target-hours-per-day" name="' . \esc_attr($option_name . '[' . $key . ']') . '" value="' . \esc_attr((string) $current) . '">';
			echo '<p class="description">Wird für die stundenbasierte Auslastung im CaRent-Dashboard verwendet.</p>';
		},
		$page,
		'cmx_sec_carent'
	);
	...
	\add_settings_field(
		...
		static function (): void {
			$option_name = cmx_carent_settings_option_name();
			$key = cmx_carent_spatzig_key();
			echo '<input type="number" min="0" step="1" class="small-text" id="cmx-carent-spatzig" name="' . \esc_attr($option_name . '[' . $key . ']') . '" value="' . \esc_attr((string) cmx_carent_current_spatzig()) . '" style="width:95px;">';
			echo '<p class="description">Anzahl Minuten Toleranzzeit für die 24h-basierte Auslastung.</p>';
		},
		$page,
		'cmx_sec_carent'
	);
...
\add_filter('pre_update_option_' . CMX_SETTINGS_MAIN, function ($value, $old_value) {
	...
	$mail_transfer_key = cmx_carent_mail_transfer_template_key();
	$mail_return_key = cmx_carent_mail_return_template_key();
	$mail_return_days_key = cmx_carent_mail_return_days_key();
	$spatzig_key = cmx_carent_spatzig_key();
	$utilization_mode_key = cmx_carent_utilization_mode_key();
	$target_hours_per_day_key = cmx_carent_target_hours_per_day_key();
	...
	foreach ([$mail_contract_key, $mail_transfer_key, $mail_return_key] as $mail_key) {
	...
	if (\array_key_exists($spatzig_key, $value)) {
		$value[$spatzig_key] = \max(0, (int) $value[$spatzig_key]);
	} elseif (isset($old_value[$spatzig_key])) {
		$value[$spatzig_key] = \max(0, (int) $old_value[$spatzig_key]);
	}

	if (\array_key_exists($utilization_mode_key, $value)) {
		$mode = \sanitize_key((string) $value[$utilization_mode_key]);
		$value[$utilization_mode_key] = \array_key_exists($mode, cmx_carent_utilization_mode_options()) ? $mode : 'twentyfour_hour_days';
	} elseif (isset($old_value[$utilization_mode_key])) {
		$mode = \sanitize_key((string) $old_value[$utilization_mode_key]);
		$value[$utilization_mode_key] = \array_key_exists($mode, cmx_carent_utilization_mode_options()) ? $mode : 'twentyfour_hour_days';
	}

	if (\array_key_exists($target_hours_per_day_key, $value)) {
		$value[$target_hours_per_day_key] = cmx_carent_normalize_target_hours_per_day($value[$target_hours_per_day_key]);
	} elseif (isset($old_value[$target_hours_per_day_key])) {
		$value[$target_hours_per_day_key] = cmx_carent_normalize_target_hours_per_day($old_value[$target_hours_per_day_key]);
	}
