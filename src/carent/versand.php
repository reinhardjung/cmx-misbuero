...
if (
	(!\function_exists(__NAMESPACE__ . '\\cmx_mail_outlook_head_html') || !\function_exists(__NAMESPACE__ . '\\cmx_mail_button_html'))
	&& \is_file(\dirname(__DIR__) . '/belege/vorlage_mail.php')
) {
	require_once \dirname(__DIR__) . '/belege/vorlage_mail.php';
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_versand_contract_number')) {
	function cmx_carent_versand_contract_number(int $post_id, array $data = []): string {
		$post_id = (int) $post_id;
		$number = $post_id > 0 ? \trim((string) \get_post_meta($post_id, '_cmx_carent_nummer', true)) : '';
		if ($number === '' && $post_id > 0 && \function_exists(__NAMESPACE__ . '\\cmx_carent_ensure_nummer')) {
			$number = \trim((string) cmx_carent_ensure_nummer($post_id));
		}
		if ($number === '') {
			$number = \trim((string) ($data['number'] ?? $data['nummer'] ?? ''));
		}
		if ($number === '' && $post_id > 0) {
			$number = \trim((string) \get_the_title($post_id));
		}

		return $number !== '' ? $number : ('#' . $post_id);
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_versand_message_html')) {
	function cmx_carent_versand_message_html(int $post_id, array $data, array $pdf = [], string $template_key = 'carent_mail_contract_template'): string {
		...
		if ($contract_title === '') {
			...
			$contract_number = cmx_carent_versand_contract_number($post_id, $data);
		...
		$body = cmx_carent_versand_render_mail_template($template, [
			...
			$formatted_body = cmx_carent_versand_format_template_html($body);
			$is_return_mail = $template_key === 'carent_mail_return_template';
			$title = $is_return_mail ? 'Rückgabe ' . $contract_number : 'Mietvertrag ' . $contract_number;
			$preheader = $is_return_mail
			? 'Unterlagen zur Rückgabe als PDF im Anhang.'
			: 'Mietvertrag inkl. Übergabeprotokoll und AGB als PDF im Anhang.';
		$pdf_url = \trim((string) (($pdf['public_url'] ?? '') ?: ($pdf['url'] ?? '')));

		if (\function_exists(__NAMESPACE__ . '\\cmx_carent_versand_wrap_mail_html')) {
			return cmx_carent_versand_wrap_mail_html([
				'title' => $title,
				'kicker' => $self_name !== '' ? $self_name : \get_bloginfo('name'),
				'date' => $uebernahme_when,
				'preheader' => $preheader,
				'body_html' => $formatted_body,
				'download_url' => $pdf_url,
				'download_label' => $is_return_mail ? 'Rückgabe-PDF herunterladen' : 'Mietvertrag herunterladen',
				'kontakt_id' => (int) ($contact['id'] ?? 0),
			]);
		}

		return '<div style="font-family:Arial,sans-serif;font-size:15px;line-height:1.6;color:#1d2327;">' . $formatted_body . '</div>';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_versand_wrap_mail_html')) {
	function cmx_carent_versand_wrap_mail_html(array $data): string {
		$title = \trim((string) ($data['title'] ?? 'Mietvertrag'));
		$kicker = \trim((string) ($data['kicker'] ?? \get_bloginfo('name')));
		$date = \trim((string) ($data['date'] ?? ''));
		$preheader = \trim((string) ($data['preheader'] ?? ''));
		$body_html = (string) ($data['body_html'] ?? '');
		$download_url = \trim((string) ($data['download_url'] ?? ''));
		$download_label = \trim((string) ($data['download_label'] ?? 'PDF herunterladen'));
		$kontakt_id = (int) ($data['kontakt_id'] ?? 0);
		$site_name = \get_bloginfo('name');
		$mail_head_html = \function_exists(__NAMESPACE__ . '\\cmx_mail_outlook_head_html') ? (string) cmx_mail_outlook_head_html() : '';
		$mail_theme = \function_exists(__NAMESPACE__ . '\\cmx_email_theme_palette') ? (array) cmx_email_theme_palette() : [];
		...
		$button_link_color = (string) ($mail_theme['link_color'] ?? $button_background);
		...
		$header_content_html = \function_exists(__NAMESPACE__ . '\\cmx_email_header_content_html')
			? (string) cmx_email_header_content_html(\esc_html($kicker), \esc_html($title), \esc_html($date), \esc_html($preheader), $header_logo_html)
			: '<div style="font-family:Segoe UI,Roboto,Arial,sans-serif;font-size:14px;letter-spacing:0.08em;text-transform:uppercase;opacity:0.9;color:' . \esc_attr($header_text) . ';">' . \esc_html($kicker) . '</div>'
				. '<div style="font-family:Segoe UI,Roboto,Arial,sans-serif;font-size:26px;line-height:1.2;margin-top:6px;font-weight:600;color:' . \esc_attr($header_text) . ';">' . \esc_html($title) . '</div>'
				. ($date !== '' ? '<div style="font-family:Segoe UI,Roboto,Arial,sans-serif;font-size:13px;line-height:1.4;margin-top:4px;opacity:0.9;color:' . \esc_attr($header_text) . ';">' . \esc_html($date) . '</div>' : '')
				. ($preheader !== '' ? '<div style="font-family:Segoe UI,Roboto,Arial,sans-serif;font-size:12px;opacity:0.85;margin-top:4px;color:' . \esc_attr($header_text) . ';">' . \esc_html($preheader) . '</div>' : '');
		$button_html = '';
		if ($download_url !== '' && \function_exists(__NAMESPACE__ . '\\cmx_mail_button_html')) {
			$button_icon_fill = (string) ($mail_theme['button_mode'] ?? 'button') === 'link' ? $button_link_color : '#ffffff';
			$button_icon_html = '<span style="vertical-align:middle;display:inline-block;' . ($download_label !== '' ? 'margin-right:8px;' : '') . '">'
				. '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" style="vertical-align:middle;display:inline-block;">'
				. '<path d="M6 2h9l5 5v15a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2z" fill="' . \esc_attr($button_icon_fill) . '"/>'
				. '<path d="M15 2v6h6" fill="#e7e9ef"/>'
				. '<rect x="6.8" y="13.3" width="10.4" height="5.2" rx="1" fill="' . \esc_attr($button_accent) . '"/>'
				. '</svg></span>';
			$button_html = '<table role="presentation" cellpadding="0" cellspacing="0" style="margin:18px 0 24px 0;"><tr><td>'
				. cmx_mail_button_html($download_url, $download_label, $button_background, $button_text, 240, $button_icon_html, 2)
				. '</td></tr></table>';
		}
		...
		<tr>
			<td align="center" style="padding:24px 12px;">
				<table role="presentation" cellpadding="0" cellspacing="0" width="600" style="width:600px;max-width:600px;background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;box-shadow:0 8px 24px rgba(0,0,0,0.08);mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;">
					<tr><td style="' . $header_style . '">' . $header_content_html . '</td></tr>
					<tr><td style="padding:24px 24px 8px 24px;font-family:Segoe UI,Roboto,Arial,sans-serif;color:#1f2933;font-size:16px;line-height:1.6;">' . $body_html . $button_html . '</td></tr>
					<tr>
						<td style="padding:0 24px 24px 24px;">
							...
							' . ($show_powered_by ? '<p style="margin:0;font-family:Segoe UI,Roboto,Arial,sans-serif;font-size:12px;color:#8b98a5;line-height:1.5;">Erstellt mit <a href="https://misbuero.ch/" style="color:#8b98a5;text-decoration:underline;">MisBüro</a> – der einfachen Bürosoftware für Selbständige in der Schweiz.</p>' : '') . '
						</td>
					</tr>
				</table>
			</td>
