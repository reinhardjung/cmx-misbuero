...
define(__NAMESPACE__ . '\\CMX_TAX_'.strtoupper(basename(__DIR__)),'Fotos,Schaden'); // 'Kategorien,Fotos'
...
\add_action('init', function () {
	cmx_create_taxo(basename(__DIR__), 'Foto', 'Fotos');
	cmx_create_taxo(basename(__DIR__), 'Schaden', 'Schaden');
}, 15);
...
cmx_require_files(__DIR__,'stammdaten,admincolumns,kontakt,status,fahrzeug,fotos,schadenprotokoll,uebernahme_rueckgabe,ausweis_fahrer,ausweis_id,vertrag,beleg,versand,dashboard,tablet');

\add_action('save_post_carent', function (int $post_id, \WP_Post $post): void {
	if ((string) $post->post_type !== 'carent' || \wp_is_post_autosave($post_id) || \wp_is_post_revision($post_id)) {
		return;
	}
	if (\function_exists(__NAMESPACE__ . '\\cmx_carent_ensure_contract_archive_dir')) {
		cmx_carent_ensure_contract_archive_dir($post_id);
	}
}, 100, 2);
