...
if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_fotos_rows_from_webdav')) {
	function cmx_carent_fotos_rows_from_webdav(int $post_id): array {
		...
		$rows = [];
		$dirs = [
			['module' => 'carent', 'rel_dir' => 'fotos/' . $post_id, 'meta_prefix' => 'fotos/' . $post_id],
		];
		if (\function_exists(__NAMESPACE__ . '\\cmx_carent_contract_archive_rel_dir')) {
			$archive_rel_dir = (string) cmx_carent_contract_archive_rel_dir($post_id);
			$dirs[] = ['module' => 'archiv', 'rel_dir' => $archive_rel_dir, 'meta_prefix' => 'misbuero/archiv/' . $archive_rel_dir];
		}

		foreach ($dirs as $dir_config) {
			$module = (string) ($dir_config['module'] ?? 'carent');
			$rel_dir = (string) ($dir_config['rel_dir'] ?? '');
			$meta_prefix = \trim((string) ($dir_config['meta_prefix'] ?? $rel_dir), '/');
			$is_archive_dir = \str_starts_with($meta_prefix, 'misbuero/archiv/');
			$dir = (string) cmx_dav_module_file_path($module, $rel_dir);
			...
			foreach ($entries as $entry) {
				...
				if ($is_archive_dir && !\preg_match('/^foto-/i', $entry)) {
					continue;
				}
				...
				$rows[] = [
					'term_id' => 0,
					'attachment_id' => \trim($meta_prefix . '/' . $entry, '/'),
				];
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_carent_fotos_rows')) {
	function cmx_carent_fotos_rows(int $post_id, ?string $meta_key = null, string $legacy_meta_key = ''): array {
		...
		$has_resolved_meta = \metadata_exists('post', $post_id, $resolved_meta_key);
		$rows = cmx_carent_fotos_normalize_rows(\get_post_meta($post_id, $resolved_meta_key, true));
		if ($rows !== []) {
			return $rows;
		}
		if ($has_resolved_meta) {
			return [];
		}

		if ($legacy_meta_key !== '') {
			$has_legacy_meta = \metadata_exists('post', $post_id, $legacy_meta_key);
		...
		if ($resolved_meta_key !== CMX_CARENT_FOTOS_ROWS_META) {
			return [];
		}
	...
	function cmx_carent_fotos_attachment_payload($attachment_id): array {
		...
		if ($value !== '' && !\ctype_digit($value) && \function_exists(__NAMESPACE__ . '\\cmx_dav_module_file_url')) {
			if (\str_starts_with($value, 'misbuero/archiv/') && \function_exists(__NAMESPACE__ . '\\cmx_dav_archive_file_url')) {
				$url = (string) cmx_dav_archive_file_url($value);
			} else {
...
\add_action('wp_ajax_cmx_carent_fotos_upload', function (): void {
	...
	$uploaded = \function_exists(__NAMESPACE__ . '\\cmx_carent_store_contract_uploaded_file')
		? cmx_carent_store_contract_uploaded_file($post_id, (array) $_FILES['file'], 'foto')
		: cmx_dav_store_uploaded_file('carent', (array) $_FILES['file'], 'fotos' . ($post_id > 0 ? '/' . $post_id : ''));
