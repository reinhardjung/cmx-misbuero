...
\add_action('wp_ajax_cmx_carent_transfer_upload', function (): void {
	...
	$uploaded = \function_exists(__NAMESPACE__ . '\\cmx_carent_store_contract_uploaded_file')
		? cmx_carent_store_contract_uploaded_file($post_id, (array) $_FILES['file'], 'uebergabe')
		: cmx_dav_store_uploaded_file('carent', (array) $_FILES['file'], 'uebergabe-rueckgabe' . ($post_id > 0 ? '/' . $post_id : ''));
...
\add_action('wp_ajax_cmx_carent_transfer_video_upload', function (): void {
	...
	$uploaded = \function_exists(__NAMESPACE__ . '\\cmx_carent_store_contract_uploaded_file')
		? cmx_carent_store_contract_uploaded_file($post_id, (array) $_FILES['file'], 'video')
		: cmx_dav_store_uploaded_file('carent', (array) $_FILES['file'], 'videos' . ($post_id > 0 ? '/' . $post_id : ''));
	if (\is_wp_error($uploaded)) {
		\wp_send_json_error(['message' => (string) $uploaded->get_error_message()], 500);
	}
	$file_path = (string) ($uploaded['rel_path'] ?? '');
