...
if (!\function_exists(__NAMESPACE__ . '\\cmx_is_vermietung_request')) {
	function cmx_is_vermietung_request(): bool {
		...
		$query_route = isset($_GET['cmx_vermietung'])
			? (string) \wp_unslash($_GET['cmx_vermietung'])
			: '';
		if ($query_route === '1') {
			return true;
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_contact_document_base_name')) {
	function cmx_vermietung_contact_document_base_name(int $contact_id, string $document_type = 'fuehrerausweis'): string {
		$name = $contact_id > 0 ? \trim((string) \get_the_title($contact_id)) : '';
		$name = \preg_replace('/\s+/', '', \wp_strip_all_tags($name));
		$prefix = $document_type === 'identitaetskarte' ? 'ID_' : 'FA_';
		$name = \sanitize_file_name($prefix . ($name !== '' ? $name : ('Kontakt' . $contact_id)));
		$name = (string) \pathinfo($name, \PATHINFO_FILENAME);

		return $name !== '' ? $name : ($prefix . 'Kontakt' . $contact_id);
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_license_document_base_name')) {
	function cmx_vermietung_license_document_base_name(int $contact_id): string {
		return cmx_vermietung_contact_document_base_name($contact_id, 'fuehrerausweis');
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_find_license_document')) {
	function cmx_vermietung_find_license_document(int $attachment_id, int $contact_id, string $rel_path): int {
		if (($attachment_id <= 0 && $rel_path === '') || $contact_id <= 0 || !\post_type_exists('dokumente')) {
			return 0;
		}
		...
		foreach ((array) $query->posts as $doc_id) {
			...
			if ($attachment_id > 0 && (int) \get_post_meta($doc_id, '_cmx_dokumente_attachment_id', true) === $attachment_id) {
				return $doc_id;
			}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_sync_contact_document')) {
	function cmx_vermietung_sync_contact_document($file_ref, int $contact_id, string $document_type): int {
		if ($contact_id <= 0 || !\post_type_exists('dokumente')) {
			return 0;
		}

		$document_type = $document_type === 'identitaetskarte' ? 'identitaetskarte' : 'fuehrerausweis';
		$attachment_id = 0;
		$rel_path = '';
		$file_ref = \trim((string) $file_ref);
		if ($file_ref !== '' && \ctype_digit($file_ref)) {
			$attachment_id = (int) $file_ref;
			if ($attachment_id <= 0 || (string) \get_post_type($attachment_id) !== 'attachment' || !\str_starts_with((string) \get_post_mime_type($attachment_id), 'image/')) {
				return 0;
			}
			$rel_path = $document_type === 'fuehrerausweis'
				? cmx_vermietung_rename_license_attachment($attachment_id, $contact_id)
				: \ltrim((string) \get_post_meta($attachment_id, '_wp_attached_file', true), '/');
		} elseif ($file_ref !== '') {
			$rel_path = \function_exists(__NAMESPACE__ . '\\cmx_dav_normalize_rel_path')
				? (string) cmx_dav_normalize_rel_path($file_ref)
				: \ltrim(\str_replace('\\', '/', $file_ref), '/');
		}
		if ($attachment_id <= 0 && $rel_path === '') {
			return 0;
		}

		$title = cmx_vermietung_contact_document_base_name($contact_id, $document_type);
		...
		if ($attachment_id > 0) {
			...
			\delete_post_meta($doc_id, '_cmx_dokumente_attachment_id');
		}
		if ($rel_path !== '') {
		...
		$category = $document_type === 'identitaetskarte' ? 'Identitätskarte' : 'Führerausweis';
		if (\function_exists(__NAMESPACE__ . '\\cmx_carent_vertrag_assign_dokument_category')) {
			cmx_carent_vertrag_assign_dokument_category($doc_id, $category);
		} elseif (\function_exists(__NAMESPACE__ . '\\cmx_tax_key')) {
			$taxonomy = (string) cmx_tax_key('dokumente', 'Kategorien');
			if ($taxonomy !== '' && \taxonomy_exists($taxonomy)) {
				\wp_set_object_terms($doc_id, [$category], $taxonomy, false);
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_sync_license_document')) {
	function cmx_vermietung_sync_license_document($file_ref, int $contact_id): int {
		return cmx_vermietung_sync_contact_document($file_ref, $contact_id, 'fuehrerausweis');
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_sync_identity_document')) {
	function cmx_vermietung_sync_identity_document($file_ref, int $contact_id): int {
		return cmx_vermietung_sync_contact_document($file_ref, $contact_id, 'identitaetskarte');
	}
}
		...
		foreach ((array) $query->posts as $post_id) {
			...
			if ($chassi !== '') {
				...
				$variant_entries = \function_exists(__NAMESPACE__ . '\\cmx_carent_fahrzeug_variant_entries')
					...
					: [];
				$all_variant_entries = \function_exists(__NAMESPACE__ . '\\cmx_artikel_admin_variant_entries')
					? (array) cmx_artikel_admin_variant_entries($post_id)
					: [];
				if ($variant_entries === []) {
					if ($all_variant_entries !== []) {
						continue;
					}
					$rows[] = [
						'id'                  => $post_id,
						...
						'mietpreis'           => $mietpreis,
						'einheit'             => cmx_vermietung_billing_unit_label($post_id, 0),
					];
					continue;
				}
			...
			foreach ($variant_entries as $entry) {
				...
				$rows[] = [
					...
					'einheit'             => cmx_vermietung_billing_unit_label($post_id, $variant_index),
				];
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_billing_unit_label')) {
	function cmx_vermietung_billing_unit_label(int $vehicle_id, $variant_index): string {
		if (\function_exists(__NAMESPACE__ . '\\cmx_carent_abrechnung_unit_label')) {
			return (string) cmx_carent_abrechnung_unit_label($vehicle_id, $variant_index);
		}

		if ($vehicle_id > 0 && $variant_index !== '' && \is_numeric((string) $variant_index) && \function_exists(__NAMESPACE__ . '\\cmx_carent_fahrzeug_variant_entry')) {
			$entry = (array) cmx_carent_fahrzeug_variant_entry($vehicle_id, (int) $variant_index);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_calculate_total')) {
	function cmx_vermietung_calculate_total(mixed $anzahl, mixed $mietpreis): string {
		...
		return \number_format(\round(\max(0, $anzahl_value) * \max(0, $mietpreis_value), 0), 2, '.', '');
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_parse_amount')) {
	function cmx_vermietung_parse_amount(mixed $value): float {
		...
		return \is_finite($number) ? \max(0.0, (float) $number) : 0.0;
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_format_amount')) {
	function cmx_vermietung_format_amount(mixed $value): string {
		return \number_format(cmx_vermietung_parse_amount($value), 2, '.', '');
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_damage_cost_total')) {
	function cmx_vermietung_damage_cost_total(int $post_id): string {
		if ($post_id <= 0) {
			return '';
		}

		$stored_key = \defined(__NAMESPACE__ . '\\CMX_CARENT_ABRECHNUNG_SCHADENKOSTEN_META')
			? (string) \constant(__NAMESPACE__ . '\\CMX_CARENT_ABRECHNUNG_SCHADENKOSTEN_META')
			: '_cmx_carent_abrechnung_schadenkosten';
		$stored = \trim((string) \get_post_meta($post_id, $stored_key, true));
		if ($stored !== '') {
			return \function_exists(__NAMESPACE__ . '\\cmx_carent_fahrzeug_normalize_decimal')
				? cmx_carent_fahrzeug_normalize_decimal($stored)
				: \number_format(cmx_vermietung_parse_amount($stored), 2, '.', '');
		}

		$rows = \function_exists(__NAMESPACE__ . '\\cmx_vermietung_schaden_rows')
			? (array) cmx_vermietung_schaden_rows($post_id)
			: [];
		$total = 0.0;
		foreach ($rows as $row) {
			$row = (array) $row;
			$total += cmx_vermietung_parse_amount((string) ($row['kosten'] ?? ''));
		}
		...
		return \function_exists(__NAMESPACE__ . '\\cmx_carent_fahrzeug_normalize_decimal')
			? cmx_carent_fahrzeug_normalize_decimal((string) $total)
			: \number_format($total, 2, '.', '');
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_save_abrechnung_values')) {
	function cmx_vermietung_save_abrechnung_values(int $post_id): void {
		$mietdauer = isset($_POST['cmx_vermietung_abrechnung_mietdauer'])
			? cmx_carent_fahrzeug_normalize_int(\wp_unslash($_POST['cmx_vermietung_abrechnung_mietdauer']))
			: '';
		$mietpreis = isset($_POST['cmx_vermietung_abrechnung_mietpreis'])
			? cmx_carent_fahrzeug_normalize_decimal(\wp_unslash($_POST['cmx_vermietung_abrechnung_mietpreis']))
			: '';
		$mehrkilometer = isset($_POST['cmx_vermietung_abrechnung_mehrkilometer'])
			? cmx_carent_fahrzeug_normalize_int(\wp_unslash($_POST['cmx_vermietung_abrechnung_mehrkilometer']))
			: '';
		$mehrpreis = isset($_POST['cmx_vermietung_abrechnung_mehrpreis'])
			? cmx_carent_fahrzeug_normalize_decimal(\wp_unslash($_POST['cmx_vermietung_abrechnung_mehrpreis']))
			: '';
		$schadenkosten = isset($_POST['cmx_vermietung_abrechnung_schadenkosten'])
			? cmx_carent_fahrzeug_normalize_decimal(\wp_unslash($_POST['cmx_vermietung_abrechnung_schadenkosten']))
			: '';

		$mietdauer_key = \defined(__NAMESPACE__ . '\\CMX_CARENT_FAHRZEUG_ANZAHL_META')
		...
		if ($mietdauer === '') {
			\delete_post_meta($post_id, $mietdauer_key);
		} else {
			\update_post_meta($post_id, $mietdauer_key, $mietdauer);
		}
		if ($mietpreis === '') {
			\delete_post_meta($post_id, $mietpreis_key);
		} else {
			\update_post_meta($post_id, $mietpreis_key, $mietpreis);
		}
		if ($mehrkilometer === '') {
			\delete_post_meta($post_id, $mehrkilometer_key);
		} else {
			\update_post_meta($post_id, $mehrkilometer_key, $mehrkilometer);
		}
		if ($mehrpreis === '') {
			\delete_post_meta($post_id, $mehrpreis_key);
		} else {
			\update_post_meta($post_id, $mehrpreis_key, $mehrpreis);
		}
		if ($schadenkosten === '') {
			\delete_post_meta($post_id, $schadenkosten_key);
		} else {
			\update_post_meta($post_id, $schadenkosten_key, $schadenkosten);
		}
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_vehicle_detail_values')) {
	function cmx_vermietung_vehicle_detail_values(int $vehicle_id, int $carent_id = 0, ?int $variant_index = null): array {
		...
		foreach (['mehrpreis', 'kasko_min', 'kasko_max', 'mietpreis'] as $decimal_key) {
			$decimal_value = \trim((string) ($values[$decimal_key] ?? ''));
			if ($decimal_value !== '') {
				$values[$decimal_key] = cmx_carent_fahrzeug_normalize_decimal($decimal_value);
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_signature_attachment_url')) {
	function cmx_vermietung_signature_attachment_url($attachment_id): string {
		...
		if (!\ctype_digit($value)) {
			if (\str_starts_with($value, 'misbuero/archiv/') && \function_exists(__NAMESPACE__ . '\\cmx_dav_archive_file_url')) {
				return (string) cmx_dav_archive_file_url($value);
			}
			return \function_exists(__NAMESPACE__ . '\\cmx_dav_module_file_url') ? (string) cmx_dav_module_file_url('carent', $value) : '';
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_store_signature_attachment')) {
	function cmx_vermietung_store_signature_attachment(int $post_id, string $transfer_key, string $role, string $data_url, string $existing_attachment_id = '') {
		...
		$filename = 'unterschrift-' . \sanitize_file_name($transfer_key . '-' . $role) . '-' . (\function_exists('current_time') ? (string) \current_time('Ymd-His') : \gmdate('Ymd-His')) . '.png';
		...
		$subdir = \function_exists(__NAMESPACE__ . '\\cmx_carent_contract_archive_rel_dir')
			? (string) cmx_carent_contract_archive_rel_dir($post_id)
			: 'signaturen/' . $post_id;
		if (\function_exists(__NAMESPACE__ . '\\cmx_dav_archiv_share_path')) {
			$share_path = (string) cmx_dav_archiv_share_path();
		}
		...
		@chmod($file, 0666);
		return \function_exists(__NAMESPACE__ . '\\cmx_dav_archive_rel_path')
			? 'misbuero/archiv/' . cmx_dav_archive_rel_path($subdir . '/' . $filename)
			: $subdir . '/' . $filename;
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_render_signature_pad')) {
	function cmx_vermietung_render_signature_pad(string $transfer_key, string $role, $attachment_id, bool $enabled, bool $acceptance_checked = false): void {
		...
		echo '<div class="cmx-vermietung-signature-meta-row">';
		...
		echo '<button type="button" class="cmx-vermietung-signature-lock" data-signature-lock="' . \esc_attr($prefix) . '" aria-pressed="false" title="Unterschrift sperren" aria-label="Unterschrift sperren"' . ($enabled ? '' : ' disabled') . '>' . cmx_vermietung_signature_lock_icon() . '</button>';
		echo '</div>';
		echo '</div>';
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_signature_lock_icon')) {
	function cmx_vermietung_signature_lock_icon(): string {
		return '<span class="cmx-vermietung-signature-lock-icon is-open" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path fill="currentColor" d="M17 9h-1V7a4 4 0 0 0-7.9-.9l2 .44A2 2 0 0 1 14 7v2H7a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-8a2 2 0 0 0-2-2Zm0 10H7v-8h10v8Zm-5-2a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z"/></svg></span><span class="cmx-vermietung-signature-lock-icon is-closed" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path fill="currentColor" d="M17 9h-1V7a4 4 0 0 0-8 0v2H7a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-8a2 2 0 0 0-2-2Zm-7-2a2 2 0 0 1 4 0v2h-4V7Zm7 12H7v-8h10v8Zm-5-2a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z"/></svg></span>';
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_schaden_row_markup')) {
	function cmx_vermietung_schaden_row_markup(string $index, array $row, array $term_options, string $field_name, bool $enabled): string {
		...
		$kosten_id = 'cmx-vermietung-schaden-kosten-' . $index;
		...
		<div class="cmx-vermietung-schaden-row" data-index="<?php echo \esc_attr($index); ?>">
			...
			<div class="cmx-vermietung-schaden-row-grid">
				<div class="cmx-vermietung-schaden-field">
					...
					<label class="cmx-vermietung-schaden-field-label" for="<?php echo \esc_attr($kosten_id); ?>"><?php echo \esc_html__('Kosten', 'cmx-misbuero'); ?></label>
					<input type="number" min="0" step="0.01" class="cmx-vermietung-schaden-select cmx-vermietung-schaden-kosten" id="<?php echo \esc_attr($kosten_id); ?>" name="<?php echo \esc_attr($field_name); ?>[<?php echo \esc_attr($index); ?>][kosten]" value="<?php echo \esc_attr($kosten); ?>"<?php echo $disabled_attr; ?>>
				</div>
				<div class="cmx-vermietung-schaden-field">
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_render_schaden_section')) {
	function cmx_vermietung_render_schaden_section(int $post_id, bool $enabled): void {
		...
		if ($rows === []) {
			$rows = [['term_id' => 0, 'note' => '', 'fotos_gemacht' => false, 'kosten' => '']];
		}

		$term_options = cmx_vermietung_schaden_term_options();
		$host_id = 'cmx-vermietung-schadenprotokoll';
		$template_markup = cmx_vermietung_schaden_row_markup('__INDEX__', ['term_id' => 0, 'note' => '', 'fotos_gemacht' => false, 'kosten' => ''], $term_options, $field_name, $enabled);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_vermietung_save_schaden_rows_values')) {
	function cmx_vermietung_save_schaden_rows_values(int $post_id): void {
		...
		foreach ((array) $raw_rows as $raw_row) {
			...
			$current_kosten = \function_exists(__NAMESPACE__ . '\\cmx_carent_schadenprotokoll_sanitize_cost')
				? cmx_carent_schadenprotokoll_sanitize_cost($raw_row['kosten'] ?? '')
				: \trim((string) ($raw_row['kosten'] ?? ''));
			...
			if ($current_term_id <= 0 && $current_note === '' && !$current_fotos_gemacht && $current_kosten === '') {
				continue;
			}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_handle_vermietung_create')) {
	function cmx_handle_vermietung_create(): void {
		...
		cmx_vermietung_save_abrechnung_values($post_id);
		...
		if ($has_license_upload) {
			$uploaded = \function_exists(__NAMESPACE__ . '\\cmx_dav_store_uploaded_file')
				? (\function_exists(__NAMESPACE__ . '\\cmx_carent_store_contract_uploaded_file')
					? cmx_carent_store_contract_uploaded_file($post_id, (array) $_FILES['cmx_vermietung_fuehrerausweis_file'], 'fuehrerausweis')
		...
		$final_license_file_ref = \trim((string) \get_post_meta($post_id, $license_meta_key, true));
		if ($final_license_file_ref !== '' && $kontakt_id > 0) {
			cmx_vermietung_sync_license_document($final_license_file_ref, $kontakt_id);
		}
		if ($has_identity_upload) {
			$uploaded = \function_exists(__NAMESPACE__ . '\\cmx_dav_store_uploaded_file')
				? (\function_exists(__NAMESPACE__ . '\\cmx_carent_store_contract_uploaded_file')
					? cmx_carent_store_contract_uploaded_file($post_id, (array) $_FILES['cmx_vermietung_identitaetskarte_file'], 'identitaetskarte')
			...
			cmx_vermietung_sync_identity_document((string) $uploaded['rel_path'], $kontakt_id);
		}
		...
		if (\function_exists(__NAMESPACE__ . '\\cmx_carent_vertrag_generate_pdf')) {
			$pdf_result = cmx_carent_vertrag_generate_pdf($post_id);
			if (\is_wp_error($pdf_result)) {
				\error_log('[CMX Carent] Mietvertrag-PDF konnte nach dem Speichern nicht erzeugt werden: ' . $pdf_result->get_error_message());
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_render_vermietung_page')) {
	function cmx_render_vermietung_page(): void {
		...
		$selected_vehicle_details['einheit'] = cmx_vermietung_billing_unit_label($selected_vehicle_id, $selected_vehicle_variant_index);
		$selected_billing_more_km_key = \defined(__NAMESPACE__ . '\\CMX_CARENT_ABRECHNUNG_MEHRKILOMETER_META')
			? (string) \constant(__NAMESPACE__ . '\\CMX_CARENT_ABRECHNUNG_MEHRKILOMETER_META')
			: '_cmx_carent_abrechnung_mehrkilometer';
		$selected_billing_more_km = $current_post_id > 0
			? \trim((string) \get_post_meta($current_post_id, $selected_billing_more_km_key, true))
			: '';
		$selected_billing_more_km_manual = $selected_billing_more_km !== '';
		if ($selected_billing_more_km === '') {
			$selected_billing_more_km = '0';
		}
		$selected_billing_damage_cost = cmx_vermietung_damage_cost_total($current_post_id);
		if ($selected_billing_damage_cost === '') {
			$selected_billing_damage_cost = '0.00';
		}
		$selected_billing_rental_total = \round(cmx_vermietung_parse_amount($selected_vehicle_details['anzahl'] ?? '') * cmx_vermietung_parse_amount($selected_vehicle_details['mietpreis'] ?? ''), 0);
		$selected_billing_more_km_total = cmx_vermietung_parse_amount($selected_billing_more_km) * cmx_vermietung_parse_amount($selected_vehicle_details['mehrpreis'] ?? '');
		$selected_billing_total = $selected_billing_rental_total + $selected_billing_more_km_total + cmx_vermietung_parse_amount($selected_billing_damage_cost);
		$selected_billing_show_damage = cmx_vermietung_parse_amount($selected_billing_damage_cost) > 0;
		$selected_billing_unit_label = (string) ($selected_vehicle_details['einheit'] ?? 'Tag');
		...
		\wp_print_styles(['dashicons']);
			...
			*{box-sizing:border-box}
			...
			.cmx-vermietung-head-brand{flex:0 0 auto;display:flex;align-items:flex-start;justify-content:flex-end;min-height:96px}
			.cmx-vermietung-head-logo{display:block;width:171px;height:96px;object-fit:contain;object-position:right top}
			...
			.cmx-vermietung-billing-wrap{display:grid;gap:12px;padding:16px 18px 18px}
			.cmx-vermietung-billing-row{display:grid;grid-template-columns:minmax(170px,1.1fr) minmax(170px,1fr) minmax(190px,1.1fr) minmax(140px,.8fr);gap:12px;align-items:stretch}
			.cmx-vermietung-billing-row.is-hidden{display:none}
			.cmx-vermietung-billing-cell{min-width:0;padding:12px 14px;border:1px solid #e4e7ec;border-radius:12px;background:#fafafa}
			.cmx-vermietung-billing-label{display:block;font-size:11px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:#98a2b3}
			.cmx-vermietung-billing-value{display:flex;align-items:center;gap:8px;margin-top:6px;font-size:15px;font-weight:700;color:#1d2327}
			.cmx-vermietung-billing-value input{width:120px;max-width:100%;min-height:38px;padding:8px 10px;border:1px solid #c8c8c8;border-radius:10px;background:#fff;font:inherit;font-weight:700;text-align:right}
			.cmx-vermietung-billing-value input:disabled{background:#f8fafc;color:#98a2b3;cursor:not-allowed}
			.cmx-vermietung-billing-row--total{grid-template-columns:minmax(0,1fr)}
			.cmx-vermietung-billing-total{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:14px 16px;border:1px solid #e4e7ec;border-radius:12px;background:#fafafa;font-size:16px;font-weight:800;color:#1d2327}
			...
			.cmx-vermietung-schaden-kosten{width:50%;min-width:90px}
			...
			.cmx-vermietung-signature-item.is-signature-locked .cmx-vermietung-signature-canvas{cursor:not-allowed;background:#fbfcfe}
			.cmx-vermietung-signature-meta-row{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:10px}
			.cmx-vermietung-signature-meta{margin:0;font-size:13px;color:#667085}
			.cmx-vermietung-signature-lock{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;padding:0;border:1px solid #d0d5dd;border-radius:8px;background:#fff;color:#667085;cursor:pointer;flex:0 0 auto}
			.cmx-vermietung-signature-lock:hover,.cmx-vermietung-signature-lock:focus{border-color:#9aa4b2;color:#1d2327}
			.cmx-vermietung-signature-lock:disabled{opacity:.5;cursor:not-allowed}
			.cmx-vermietung-signature-lock svg{display:block;width:16px;height:16px}
			.cmx-vermietung-signature-lock .is-closed{display:none}
			.cmx-vermietung-signature-lock.is-locked{background:#f2f4f7;color:#1d2327;border-color:#98a2b3}
			.cmx-vermietung-signature-lock.is-locked .is-open{display:none}
			.cmx-vermietung-signature-lock.is-locked .is-closed{display:inline-flex}
		...
		$info_fields = [
			...
			'mehrpreis'   => ['label' => 'Mehrpreis', 'name' => 'cmx_vermietung_fahrzeug_mehrpreis', 'type' => 'text', 'step' => '', 'inputmode' => 'decimal'],
			'kasko_min'   => ['label' => 'Kasko min', 'name' => 'cmx_vermietung_fahrzeug_kasko_min', 'type' => 'text', 'step' => '', 'inputmode' => 'decimal'],
			'kasko_max'   => ['label' => 'Kasko max', 'name' => 'cmx_vermietung_fahrzeug_kasko_max', 'type' => 'text', 'step' => '', 'inputmode' => 'decimal'],
			'anzahl'      => ['label' => 'Anzahl', 'name' => 'cmx_vermietung_fahrzeug_anzahl', 'type' => 'number', 'step' => '1'],
			'mietpreis'   => ['label' => 'Mietpreis', 'name' => 'cmx_vermietung_fahrzeug_mietpreis', 'type' => 'text', 'step' => '', 'inputmode' => 'decimal'],
			'summe'       => ['label' => 'Summe', 'name' => '', 'type' => 'text', 'step' => '', 'inputmode' => 'decimal', 'readonly' => true],
		];
		foreach ($info_fields as $field_key => $field_config) {
			...
			$inputmode_attr = ((string) ($field_config['inputmode'] ?? '')) !== ''
				? ' inputmode="' . \esc_attr((string) $field_config['inputmode']) . '"'
				: '';
			...
			echo '<input class="cmx-vermietung-info-value" id="cmx-vermietung-info-' . \esc_attr($field_key) . '"' . $name_attr . ' type="' . \esc_attr((string) $field_config['type']) . '" value="' . \esc_attr($field_value) . '"' . (((string) $field_config['step']) !== '' ? ' step="' . \esc_attr((string) $field_config['step']) . '"' : '') . $inputmode_attr . $readonly_attr . ($selected_vehicle_id > 0 ? '' : ' disabled') . '>';
			echo '</div>';
		}
		echo '</div>';
		echo '</section>';
		...
		echo '</div>';
		echo '</section>';
		echo '<section class="cmx-vermietung-transfer-panel' . ($selected_vehicle_id > 0 ? '' : ' is-hidden') . '" id="cmx-vermietung-abrechnung-panel">';
		echo '<div class="cmx-vermietung-panel-head">';
		echo '<h2 class="cmx-vermietung-panel-title">Abrechnung</h2>';
		echo '<p class="cmx-vermietung-panel-sub" style="padding-bottom:20px;">Kostenübersicht zum gewählten Fahrzeug.</p>';
		echo '</div>';
		echo '<div class="cmx-vermietung-billing-wrap">';
		echo '<div class="cmx-vermietung-billing-row">';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Position</span><span class="cmx-vermietung-billing-value">Mietdauer</span></div>';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Menge</span><span class="cmx-vermietung-billing-value"><input type="number" min="0" step="1" name="cmx_vermietung_abrechnung_mietdauer" id="cmx-vermietung-abrechnung-mietdauer" value="' . \esc_attr((string) ($selected_vehicle_details['anzahl'] ?? '')) . '"' . ($selected_vehicle_id > 0 ? '' : ' disabled') . '> <span id="cmx-vermietung-abrechnung-mietdauer-unit">' . \esc_html($selected_billing_unit_label) . '</span></span></div>';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Einheitspreis</span><span class="cmx-vermietung-billing-value"><input type="text" inputmode="decimal" name="cmx_vermietung_abrechnung_mietpreis" id="cmx-vermietung-abrechnung-mietpreis" value="' . \esc_attr((string) ($selected_vehicle_details['mietpreis'] ?? '')) . '"' . ($selected_vehicle_id > 0 ? '' : ' disabled') . '> CHF</span></div>';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Total</span><span class="cmx-vermietung-billing-value"><input type="text" inputmode="decimal" id="cmx-vermietung-abrechnung-mietdauer-total" value="' . \esc_attr(\number_format(cmx_vermietung_parse_amount($selected_billing_rental_total), 2, '.', '')) . '"' . ($selected_vehicle_id > 0 ? '' : ' disabled') . '> CHF</span></div>';
		echo '</div>';
		echo '<div class="cmx-vermietung-billing-row">';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Position</span><span class="cmx-vermietung-billing-value">Mehrkilometer</span></div>';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Menge</span><span class="cmx-vermietung-billing-value"><input type="number" min="0" step="1" name="cmx_vermietung_abrechnung_mehrkilometer" id="cmx-vermietung-abrechnung-mehrkilometer" value="' . \esc_attr($selected_billing_more_km) . '" data-manual="' . ($selected_billing_more_km_manual ? '1' : '0') . '"' . ($selected_vehicle_id > 0 ? '' : ' disabled') . '> km</span></div>';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Einheitspreis</span><span class="cmx-vermietung-billing-value"><input type="text" inputmode="decimal" name="cmx_vermietung_abrechnung_mehrpreis" id="cmx-vermietung-abrechnung-mehrpreis" value="' . \esc_attr((string) ($selected_vehicle_details['mehrpreis'] ?? '')) . '"' . ($selected_vehicle_id > 0 ? '' : ' disabled') . '> CHF / km</span></div>';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Total</span><span class="cmx-vermietung-billing-value"><input type="text" inputmode="decimal" id="cmx-vermietung-abrechnung-mehrkilometer-total" value="' . \esc_attr(\number_format(cmx_vermietung_parse_amount($selected_billing_more_km_total), 2, '.', '')) . '"' . ($selected_vehicle_id > 0 ? '' : ' disabled') . '> CHF</span></div>';
		echo '</div>';
		echo '<div class="cmx-vermietung-billing-row' . ($selected_billing_show_damage ? '' : ' is-hidden') . '" id="cmx-vermietung-abrechnung-schaden-row">';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Position</span><span class="cmx-vermietung-billing-value">Zusatzkosten (Schaden)</span></div>';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Menge</span><span class="cmx-vermietung-billing-value">-</span></div>';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Einheitspreis</span><span class="cmx-vermietung-billing-value">-</span></div>';
		echo '<div class="cmx-vermietung-billing-cell"><span class="cmx-vermietung-billing-label">Total</span><span class="cmx-vermietung-billing-value"><input type="text" inputmode="decimal" name="cmx_vermietung_abrechnung_schadenkosten" id="cmx-vermietung-abrechnung-schadenkosten" value="' . \esc_attr($selected_billing_damage_cost) . '"' . ($selected_vehicle_id > 0 ? '' : ' disabled') . '> CHF</span></div>';
		echo '</div>';
		echo '<div class="cmx-vermietung-billing-row cmx-vermietung-billing-row--total">';
		echo '<div class="cmx-vermietung-billing-total"><span>TOTAL</span><span><span id="cmx-vermietung-abrechnung-total">' . \esc_html(cmx_vermietung_format_amount((string) $selected_billing_total)) . '</span> CHF</span></div>';
		echo '</div>';
		echo '</div>';
		echo '</section>';
		...
		echo '<script>
			(function(){
				...
				var billingPanel=document.getElementById("cmx-vermietung-abrechnung-panel");
				...
				var billingNodes={
					mietdauer:document.getElementById("cmx-vermietung-abrechnung-mietdauer"),
					mietdauerUnit:document.getElementById("cmx-vermietung-abrechnung-mietdauer-unit"),
					mietpreis:document.getElementById("cmx-vermietung-abrechnung-mietpreis"),
					mehrkilometer:document.getElementById("cmx-vermietung-abrechnung-mehrkilometer"),
					mehrpreis:document.getElementById("cmx-vermietung-abrechnung-mehrpreis"),
					schadenkosten:document.getElementById("cmx-vermietung-abrechnung-schadenkosten"),
					schadenkostenRow:document.getElementById("cmx-vermietung-abrechnung-schaden-row"),
					mietdauerTotal:document.getElementById("cmx-vermietung-abrechnung-mietdauer-total"),
					mehrkilometerTotal:document.getElementById("cmx-vermietung-abrechnung-mehrkilometer-total"),
					total:document.getElementById("cmx-vermietung-abrechnung-total")
				};
				var transferNodes={
				...
				function formatRentalTotalValue(value){
					return isFinite(value) ? Math.round(value).toFixed(2) : "";
				}
				function refreshVehicleAmountSummary(){
					if(!vehicleInfoNodes.summe){return;}
					if(billingNodes.mietdauer && vehicleInfoNodes.anzahl){
						billingNodes.mietdauer.value=vehicleInfoNodes.anzahl.value;
					}
					if(billingNodes.mietpreis && vehicleInfoNodes.mietpreis){
						billingNodes.mietpreis.value=vehicleInfoNodes.mietpreis.value;
					}
					if(billingNodes.mehrpreis && vehicleInfoNodes.mehrpreis){
						billingNodes.mehrpreis.value=vehicleInfoNodes.mehrpreis.value;
					}
					...
					vehicleInfoNodes.summe.value=formatRentalTotalValue(Math.max(0,anzahl)*Math.max(0,mietpreis));
					refreshBillingSummary();
				}
				function refreshBillingSummary(){
					var mietdauer=billingNodes.mietdauer ? parseNumericValue(billingNodes.mietdauer.value) : NaN;
					var mietpreis=billingNodes.mietpreis ? parseNumericValue(billingNodes.mietpreis.value) : NaN;
					var mehrkilometer=billingNodes.mehrkilometer ? parseNumericValue(billingNodes.mehrkilometer.value) : NaN;
					var mehrpreis=billingNodes.mehrpreis ? parseNumericValue(billingNodes.mehrpreis.value) : NaN;
					var schadenkosten=billingNodes.schadenkosten ? parseNumericValue(billingNodes.schadenkosten.value) : NaN;
					var mietdauerTotal=Math.round((isFinite(mietdauer) ? Math.max(0,mietdauer) : 0)*(isFinite(mietpreis) ? Math.max(0,mietpreis) : 0));
					var mehrkilometerTotal=(isFinite(mehrkilometer) ? Math.max(0,mehrkilometer) : 0)*(isFinite(mehrpreis) ? Math.max(0,mehrpreis) : 0);
					var schadenTotal=isFinite(schadenkosten) ? Math.max(0,schadenkosten) : 0;
					if(billingNodes.schadenkostenRow){billingNodes.schadenkostenRow.classList.toggle("is-hidden", !(schadenTotal>0));}
					if(billingNodes.mietdauerTotal && document.activeElement!==billingNodes.mietdauerTotal){billingNodes.mietdauerTotal.value=formatMoneyValue(mietdauerTotal);}
					if(billingNodes.mehrkilometerTotal && document.activeElement!==billingNodes.mehrkilometerTotal){billingNodes.mehrkilometerTotal.value=formatMoneyValue(mehrkilometerTotal);}
					if(billingNodes.total){billingNodes.total.textContent=formatMoneyValue(mietdauerTotal+mehrkilometerTotal+schadenTotal);}
				}
				function syncVehicleInfoFromBilling(){
					if(vehicleInfoNodes.anzahl && billingNodes.mietdauer){
						vehicleInfoNodes.anzahl.value=billingNodes.mietdauer.value;
					}
					if(vehicleInfoNodes.mietpreis && billingNodes.mietpreis){
						vehicleInfoNodes.mietpreis.value=billingNodes.mietpreis.value;
					}
					if(vehicleInfoNodes.mehrpreis && billingNodes.mehrpreis){
						vehicleInfoNodes.mehrpreis.value=billingNodes.mehrpreis.value;
					}
					if(vehicleInfoNodes.summe && billingNodes.mietdauerTotal){
						vehicleInfoNodes.summe.value=formatRentalTotalValue(parseNumericValue(billingNodes.mietdauerTotal.value) || 0);
					}
				}
				function syncBillingUnit(unit){
					if(billingNodes.mietdauerUnit){
						billingNodes.mietdauerUnit.textContent=String(unit||"").trim() || "Tag";
					}
				}
				function refreshBillingFromEditableTotal(totalInput, quantityInput, priceInput){
					var total=totalInput ? parseNumericValue(totalInput.value) : NaN;
					var quantity=quantityInput ? parseNumericValue(quantityInput.value) : NaN;
					if(!priceInput || !isFinite(total) || !isFinite(quantity) || quantity<=0){return;}
					priceInput.value=(Math.max(0,total)/Math.max(0,quantity)).toFixed(2);
				}
				function calculateIncludedKilometerOverage(){
					var limit=vehicleInfoNodes.begrenzung ? parseNumericValue(vehicleInfoNodes.begrenzung.value) : NaN;
					var pickup=transferNodes.km_stand ? parseNumericValue(transferNodes.km_stand.value) : NaN;
					var returned=returnNodes.km_stand ? parseNumericValue(returnNodes.km_stand.value) : NaN;
					var amount;
					if(!billingNodes.mehrkilometer || String(billingNodes.mehrkilometer.dataset.manual||"0")==="1"){return;}
					if(!isFinite(limit) || limit<=0 || !isFinite(pickup) || !isFinite(returned)){return;}
					amount=Math.round(Math.max(0, returned-pickup-limit));
					billingNodes.mehrkilometer.value=String(amount);
					refreshBillingSummary();
				}
				function refreshDamageCostFromRows(){
					var fields=document.querySelectorAll(".cmx-vermietung-schaden-kosten");
					var total=0;
					fields.forEach(function(input){
						var value=parseNumericValue(input.value);
						if(isFinite(value)){total+=Math.max(0,value);}
					});
					if(billingNodes.schadenkosten){
						billingNodes.schadenkosten.value=formatMoneyValue(total);
					}
					refreshBillingSummary();
				}
				...
				function paymentDefaultAmount(){
					var value="";
					if(billingNodes.total && String(billingNodes.total.textContent||"").trim()!==""){
						value=String(billingNodes.total.textContent||"").trim();
					}else if(vehicleInfoNodes.summe && String(vehicleInfoNodes.summe.value||"").trim()!==""){
				...
				function initDamageSection(sectionId){
					...
					function updateRowEnabled(row){
						var fields=row.querySelectorAll(".cmx-vermietung-schaden-select");
				...
				function setSignaturePadEnabled(enabled){
					signaturePads.forEach(function(pad){
						...
						if(pad.clearButton){
							pad.clearButton.disabled=!enabled || !!pad.locked;
						}
						if(pad.lockButton){
							pad.lockButton.disabled=!enabled;
						}
				...
				function setSignaturePadLocked(pad, locked){
					if(!pad){return;}
					pad.locked=!!locked;
					pad.drawing=false;
					pad.item.classList.toggle("is-signature-locked", pad.locked);
					if(pad.lockButton){
						pad.lockButton.classList.toggle("is-locked", pad.locked);
						pad.lockButton.setAttribute("aria-pressed", pad.locked ? "true" : "false");
						pad.lockButton.setAttribute("title", pad.locked ? "Unterschrift entsperren" : "Unterschrift sperren");
						pad.lockButton.setAttribute("aria-label", pad.locked ? "Unterschrift entsperren" : "Unterschrift sperren");
					}
					if(pad.clearButton){
						pad.clearButton.disabled=!!pad.disabled || pad.locked;
					}
					if(pad.locked){
						pad.canvas.setAttribute("data-readonly","1");
					}else{
						pad.canvas.removeAttribute("data-readonly");
					}
				}
				function initSignaturePad(item){
					...
					var lockButton=item.querySelector("[data-signature-lock]");
					...
					canvas.addEventListener("pointerdown", function(event){
						if(pad.disabled || pad.locked){return;}
						event.preventDefault();
					...
					canvas.addEventListener("pointermove", function(event){
						if(!pad.drawing || pad.disabled || pad.locked){return;}
						event.preventDefault();
						var point=getSignaturePoint(canvas, event);
					...
					if(lockButton){
						lockButton.addEventListener("click", function(event){
							...
							setSignaturePadLocked(pad, !pad.locked);
						});
					}
					setSignaturePadLocked(pad, false);
				...
				function updateVehicleInfo(item){
					...
					var values=item ? {
						...
						einheit:String(item.getAttribute("data-einheit")||"Tag").trim() || "Tag",
						summe:""
					} : {
						kennzeichen:"",
						km_stand_uebernahme:"",
					...
					if(billingPanel){
						billingPanel.classList.toggle("is-hidden", !hasVehicle);
					}
					...
					if(billingNodes.mietdauer){
						billingNodes.mietdauer.value=values.anzahl;
						billingNodes.mietdauer.disabled=!hasVehicle;
					}
					syncBillingUnit(values.einheit);
					if(billingNodes.mietpreis){
						billingNodes.mietpreis.value=values.mietpreis;
						billingNodes.mietpreis.disabled=!hasVehicle;
					}
					if(billingNodes.mehrpreis){
						billingNodes.mehrpreis.value=values.mehrpreis;
						billingNodes.mehrpreis.disabled=!hasVehicle;
					}
					if(billingNodes.mietdauerTotal){
						billingNodes.mietdauerTotal.disabled=!hasVehicle;
						if(!hasVehicle){billingNodes.mietdauerTotal.value="";}
					}
					if(billingNodes.mehrkilometerTotal){
						billingNodes.mehrkilometerTotal.disabled=!hasVehicle;
						if(!hasVehicle){billingNodes.mehrkilometerTotal.value="";}
					}
					if(billingNodes.mehrkilometer){
						billingNodes.mehrkilometer.disabled=!hasVehicle;
						if(!hasVehicle){
							billingNodes.mehrkilometer.value="";
						}
					}
					if(billingNodes.schadenkosten){
						billingNodes.schadenkosten.disabled=!hasVehicle;
						if(!hasVehicle){billingNodes.schadenkosten.value="";}
					}
					...
					calculateIncludedKilometerOverage();
					refreshKmSyncButtonStates();
				}
				...
				if(vehicleInfoNodes.mehrpreis){
					vehicleInfoNodes.mehrpreis.addEventListener("input", refreshVehicleAmountSummary);
				}
				if(vehicleInfoNodes.begrenzung){
					vehicleInfoNodes.begrenzung.addEventListener("input", function(){
						calculateIncludedKilometerOverage();
					});
				}
				if(billingNodes.mehrkilometer){
					billingNodes.mehrkilometer.addEventListener("input", function(){
						billingNodes.mehrkilometer.dataset.manual="1";
						refreshBillingSummary();
					});
				}
				[billingNodes.mietdauer,billingNodes.mietpreis,billingNodes.mehrpreis].forEach(function(input){
					if(!input){return;}
					input.addEventListener("input", function(){
						syncVehicleInfoFromBilling();
						refreshBillingSummary();
					});
				});
				if(billingNodes.mietdauerTotal){
					billingNodes.mietdauerTotal.addEventListener("input", function(){
						refreshBillingFromEditableTotal(billingNodes.mietdauerTotal, billingNodes.mietdauer, billingNodes.mietpreis);
						syncVehicleInfoFromBilling();
						refreshBillingSummary();
					});
					billingNodes.mietdauerTotal.addEventListener("blur", refreshBillingSummary);
				}
				if(billingNodes.mehrkilometerTotal){
					billingNodes.mehrkilometerTotal.addEventListener("input", function(){
						refreshBillingFromEditableTotal(billingNodes.mehrkilometerTotal, billingNodes.mehrkilometer, billingNodes.mehrpreis);
						refreshBillingSummary();
					});
				}
				if(billingNodes.schadenkosten){
					billingNodes.schadenkosten.addEventListener("input", refreshBillingSummary);
				}
				document.addEventListener("input", function(event){
					var target=event.target;
					if(target && target.matches && target.matches(".cmx-vermietung-schaden-kosten")){
						refreshDamageCostFromRows();
					}
				});
				...
				function chooseBooking(item){
					...
					if(billingNodes.mehrkilometer){billingNodes.mehrkilometer.dataset.manual="0";}
				...
				if(transferNodes.km_stand){
					["input","change"].forEach(function(eventName){
						...
						transferNodes.km_stand.addEventListener(eventName, calculateIncludedKilometerOverage);
					});
				}
				if(returnNodes.km_stand){
					["input","change"].forEach(function(eventName){
						returnNodes.km_stand.addEventListener(eventName, refreshKmSyncButtonStates);
						returnNodes.km_stand.addEventListener(eventName, calculateIncludedKilometerOverage);
					});
				}
				...
				document.addEventListener("click", function(event){
					...
					form.addEventListener("submit", function(event){
						...
						var hasExistingLicense=licenseAttachmentInput && String(licenseAttachmentInput.value||"").trim()!=="" && String(licenseAttachmentInput.value||"").trim()!=="0";
