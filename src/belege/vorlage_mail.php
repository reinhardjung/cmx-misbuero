...
function cmxbu_render_belegmail_template(array $data = []): string {
	...
	$header_content_html = \function_exists(__NAMESPACE__ . '\\cmx_email_header_content_html')
		...
		if ($button_label === '') {
			$button_label = 'PDF-Beleg herunterladen';
		}
		$button_icon_fill = (string) ($mail_theme['button_mode'] ?? 'button') === 'link' ? $link_color : '#ffffff';
	...
	$salutation_line_html = $anrede_esc !== ''
		...
		$default_body_html = $salutation_line_html
			. '<p style="margin:0 0 16px 0;font-size:16px;line-height:1.6;">Der Beleg wurde erfolgreich erstellt und kann jetzt bequem herunter geladen werden.</p>'
			...
			. '<p style="margin:0 0 ' . $thank_you_margin_bottom . ' 0;font-size:16px;line-height:1.6;">Vielen Dank für das Vertrauen in meine Diersntleitungen.</p>';
