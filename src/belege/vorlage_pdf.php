...
if (!defined(__NAMESPACE__.'\\CMX_LOGO_WIDTH_MM')) define(__NAMESPACE__.'\\CMX_LOGO_WIDTH_MM', 56.0);
...
if (!function_exists(__NAMESPACE__ . '\\cmxbu_strip_png_iccp_chunks')) {
	/**
	 * Entfernt PNG-Metadaten, die Dompdf/GD beim PDF-Rendering stoeren koennen.
	 */
	function cmxbu_strip_png_iccp_chunks(string $png_data, bool &$removed = false): string {
		...
		while ($offset + 12 <= $len) {
			...
			if ($chunk_type === 'iCCP' || $chunk_type === 'pHYs') {
				$removed = true;
			} else {
				$out .= $chunk_bin;
			}
...
function cmx_get_contact_logo(int $post_id): string {
	...
	// Die Kontaktverwaltung kann Galerie-Logos auch dann per URL anzeigen, wenn
	// ein gespeicherter lokaler Pfad nach einem Umzug nicht mehr gültig ist.
	// Dompdf darf Remote-URLs laden, deshalb dieselbe sichtbare URL übernehmen.
	if (\function_exists(__NAMESPACE__ . '\\cmx_kl_active_gallery_item')) {
		...
		$active_url = \is_array($active_item)
			? \esc_url_raw(\trim((string) ($active_item['url'] ?? '')))
			: '';
		if ($active_url !== '') {
			return $active_url;
		}
	...
	$local_url = \esc_url_raw(\trim($local_url));
	if ($local_url !== '') {
		return $local_url;
	}
...
function cmx_get_branding_logo(): string {
	$post_id = \function_exists(__NAMESPACE__ . '\\cmx_email_self_contact_id')
		...
		: 0;
	if ($post_id > 0 && \function_exists(__NAMESPACE__ . '\\cmx_get_contact_logo')) {
		$logo = (string) cmx_get_contact_logo($post_id);
		if ($logo !== '') {
			return $logo;
		}
	}
		...
		foreach ($rows as $i=>$r) {
			...
			if ($artikel_id) {
				...
				if ($artikel_id > 0 && \function_exists(__NAMESPACE__ . '\\cmxbu_resolve_position_variant_entry')) {
					...
					if (\is_array($variant_entry)) {
						$variant_sku = \trim((string) ($variant_entry['sku'] ?? ''));
						if ($variant_sku !== '') {
							$artnr = $variant_sku;
						}
...
add_action('save_post_belege', __NAMESPACE__.'\\cmxbu_generate_document_on_save', 999, 3);
	function cmxbu_generate_document_on_save(int $post_id, \WP_Post $post, bool $update): void {
		...
		$bank_iban = trim((string)($bank['iban'] ?? ''));
			...
			$payrexx_instance_slug = \function_exists(__NAMESPACE__ . '\\cmx_get_payrexx_instanz_slug')
				? \trim((string) cmx_get_payrexx_instanz_slug())
				: '';
			if (
				$beleg_type === 'rechnung'
			&& $beleg_richtung === 'ausgang'
			&& $payment_amount > 0.0
			&& $payrexx_instance_slug !== ''
...
// var_dump($dates['currency']); exit;
		...
		$tpl = [
			...
			'document' => [
				...
				'date' => $doc_date,
					...
					'paid_amount' => (float) ($payment_amounts['paid_amount'] ?? 0.0),
						...
						'payrexx_enabled' => ($payrexx_instance_slug !== ''),
			...
			'attachments' => \function_exists(__NAMESPACE__ . '\\cmx_beleg_pdf_anhaenge')
				? cmx_beleg_pdf_anhaenge($post_id)
				: [],
		...
		try {
			...
			if (\function_exists(__NAMESPACE__ . '\\cmx_pdf_signature_sign_file')) {
				$signature_result = cmx_pdf_signature_sign_file($pdf_path, [
					'type' => 'beleg',
					'post_id' => $post_id,
					'document_type' => (string) $beleg_type,
					'title' => (string) ($tpl['document']['title'] ?? 'Beleg'),
					'payrexx_vpos_url' => (string) ($tpl['document']['payrexx_vpos_url'] ?? ''),
				]);
				if (\is_wp_error($signature_result)) {
					cmxbu_log('PDF-Signatur fehlgeschlagen', ['pdf' => $pdf_path, 'error' => $signature_result->get_error_message()]);
				}
			}
		} catch (\Throwable $e) {
