...
if (!\defined(__NAMESPACE__ . '\\CMX_EXT_CHROME_PROCESS_EVENT')) {
	\define(__NAMESPACE__ . '\\CMX_EXT_CHROME_PROCESS_EVENT', 'cmx_ext_chrome_process_pdf_import');
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_request_user_id')) {
	function cmx_ext_chrome_request_user_id(): int {
		...
		if ($current_user_id > 0) {
			$expected = \trim((string) \get_user_meta($current_user_id, CMX_EXT_CHROME_USER_TOKEN_META, true));
		...
		$user_ids = \get_users([
			'fields' => 'ID',
			'number' => 10,
			'meta_key' => CMX_EXT_CHROME_USER_TOKEN_META,
			'meta_value' => $provided,
		]);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_validate_current_user_token')) {
	function cmx_ext_chrome_validate_current_user_token(): bool {
		return cmx_ext_chrome_request_user_id() > 0;
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_manifest_json')) {
	function cmx_ext_chrome_manifest_json(array $config): string {
		$manifest = [
			...
			'description' => 'Importiert das aktuell angezeigte PDF direkt als Beleg oder Dokument in Mis Büro.',
...
.choice{display:none;margin-top:12px;gap:8px}
.choice.is-visible{display:grid;grid-template-columns:1fr 1fr}
...
.hint{margin-top:10px;color:#646970;font-size:12px;display:flex;justify-content:space-between;gap:12px}
a{color:#2271b1}
</style>
</head>
<body>
<div class="wrap">
  ...
  <div id="choice" class="choice" aria-label="Ablage auswählen">
    <button id="as-beleg" type="button">als Beleg</button>
    <button id="as-dokument" class="secondary" type="button">als Dokument</button>
  </div>
  <div class="actions">
    <button id="retry" class="secondary" type="button" hidden>Nochmals versuchen</button>
  </div>
  <div class="hint"><span id="hint">PDF wird nach Deinen Einstellungen importiert.</span><a href="#" id="settings">Einstellungen</a></div>
</div>
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_options_html')) {
	function cmx_ext_chrome_options_html(): string {
		return '<!doctype html>
<html lang="de">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Mis Büro - PDF Import Einstellungen</title>
<style>
body{margin:0;font:14px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f6f7fb;color:#1d2327;min-width:360px}
.wrap{max-width:680px;margin:0 auto;padding:28px}
.panel{background:#fff;border:1px solid #dcdcde;border-radius:12px;padding:22px;box-shadow:0 18px 44px rgba(15,23,42,.08)}
.head{display:flex;align-items:center;gap:12px;margin-bottom:20px}
.icon{width:48px;height:48px;display:flex;align-items:center;justify-content:center;background:#fff;border:1px solid #dcdcde;border-radius:14px}
h1{font-size:22px;line-height:1.2;margin:0}
.sub{color:#646970;margin-top:3px}
.field{display:grid;gap:7px;margin-top:18px}
label{font-weight:700}
select{font:inherit;border:1px solid #c3c4c7;border-radius:8px;padding:8px 10px;max-width:320px;background:#fff}
.check{display:flex;gap:10px;align-items:flex-start;margin-top:18px}
.check input{margin-top:4px}
.desc{color:#646970;font-size:13px}
.actions{display:flex;align-items:center;gap:12px;margin-top:22px}
button{appearance:none;border:1px solid #2271b1;background:#2271b1;color:#fff;border-radius:8px;padding:8px 12px;cursor:pointer}
.status{color:#00a32a;font-weight:600}
</style>
</head>
<body>
<div class="wrap">
  <div class="panel">
    <div class="head">
      <div class="icon"><img src="icon48.png" alt="" width="42" height="42"></div>
      <div>
        <h1>PDF Import</h1>
        <div class="sub">Festlegen, wie PDFs aus Chrome in Mis Büro abgelegt werden.</div>
      </div>
    </div>
    <div class="field">
      <label for="default-mode">Standard-Ablage</label>
      <select id="default-mode">
        <option value="beleg">als Beleg / Lieferantenrechnung</option>
        <option value="dokument">als normales Dokument</option>
      </select>
      <div class="desc">Beleg ist das bisherige Verhalten. Dokument erstellt einen Eintrag im CPT Dokumente ohne automatische Zuordnung.</div>
    </div>
    <label class="check">
      <input id="always-use" type="checkbox">
      <span>
        <strong>Diese Auswahl immer direkt verwenden</strong>
        <span class="desc" style="display:block;">Wenn deaktiviert, fragt das Popup bei jedem PDF, ob als Beleg oder Dokument importiert werden soll.</span>
      </span>
    </label>
    <div class="actions">
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_options_js')) {
	function cmx_ext_chrome_options_js(): string {
		return <<<'JS'
...
const alwaysUseEl = document.getElementById('always-use');
const saveBtn = document.getElementById('save');
const statusEl = document.getElementById('status');

function normalizeMode(value) {
  return value === 'dokument' ? 'dokument' : 'beleg';
}

async function loadSettings() {
  const data = await chrome.storage.local.get(DEFAULT_SETTINGS);
  defaultModeEl.value = normalizeMode(data.defaultMode);
  alwaysUseEl.checked = data.alwaysUseDefault !== false;
}

async function saveSettings() {
  await chrome.storage.local.set({
    defaultMode: normalizeMode(defaultModeEl.value),
    alwaysUseDefault: !!alwaysUseEl.checked,
  });
...
const asBelegBtn = document.getElementById('as-beleg');
const asDokumentBtn = document.getElementById('as-dokument');
...
function setChoiceVisible(visible) {
  if (choiceEl) choiceEl.classList.toggle('is-visible', !!visible);
}

async function loadSettings() {
  const data = await chrome.storage.local.get(DEFAULT_SETTINGS);
  return {
    defaultMode: normalizeMode(data.defaultMode),
    alwaysUseDefault: data.alwaysUseDefault !== false,
  };
}

function modeLabel(mode) {
  return normalizeMode(mode) === 'dokument' ? 'Dokument' : 'Beleg';
}

async function runImport(mode = 'beleg') {
  lastMode = normalizeMode(mode);
  retryBtn.hidden = true;
  setChoiceVisible(false);
  setStatus('PDF wird als ' + modeLabel(lastMode) + ' an Mis Büro gesendet...');
  let response;
  try {
    response = await chrome.runtime.sendMessage({ type: 'cmx-import-active-pdf', payload: { importMode: lastMode } });
  } catch (error) {
  ...
  const resultMode = normalizeMode(result.mode || lastMode);
  setStatus(modeLabel(resultMode) + ' wurde erstellt. Öffne Übersicht...', 'success');
  const listUrl = result.listUrl || result.list_url || result.editUrl || result.edit_url || '';
  if (listUrl) {
    try {
      ...
      setStatus(modeLabel(resultMode) + ' wurde erstellt, konnte aber nicht automatisch geöffnet werden.', 'success');
    }
  }
  retryBtn.hidden = false;
}
...
async function init() {
  ...
  lastMode = settings.defaultMode;
  if (hintEl) {
    hintEl.textContent = settings.alwaysUseDefault
      ? 'Standard: Import als ' + modeLabel(settings.defaultMode) + '.'
      : 'Bitte wähle für dieses PDF die Ablage.';
  }
  if (settings.alwaysUseDefault) {
    runImport(settings.defaultMode);
    return;
  }
  setStatus('Wie soll dieses PDF in Mis Büro abgelegt werden?');
  setChoiceVisible(true);
}

retryBtn.addEventListener('click', () => runImport(lastMode));
if (asBelegBtn) asBelegBtn.addEventListener('click', () => runImport('beleg'));
if (asDokumentBtn) asDokumentBtn.addEventListener('click', () => runImport('dokument'));
if (settingsLink) {
...
async function ensureDefaultSettings() {
  const data = await chrome.storage.local.get(['defaultMode', 'alwaysUseDefault']).catch(() => ({}));
  const updates = {};
  if (data.defaultMode !== 'beleg' && data.defaultMode !== 'dokument') {
    updates.defaultMode = DEFAULT_SETTINGS.defaultMode;
  }
  if (typeof data.alwaysUseDefault !== 'boolean') {
    updates.alwaysUseDefault = DEFAULT_SETTINGS.alwaysUseDefault;
  }
  if (Object.keys(updates).length > 0) {
    await chrome.storage.local.set(updates).catch(() => undefined);
  }
}
...
async function uploadToMisBuero(payload) {
  ...
  formData.append('import_mode', normalizeImportMode(payload.importMode));
...
async function importActivePdf(importMode = 'beleg') {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'cmx-import-active-pdf') {
    ...
    importActivePdf(normalizeImportMode(payload.importMode))
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_readme_txt')) {
	function cmx_ext_chrome_readme_txt(): string {
		return "Mis Büro - Google Chrome Erweiterung\n\n"
			...
			. "- Mis Büro legt je nach Erweiterungs-Einstellung einen neuen Beleg oder ein neues Dokument an.\n"
			. "- Die Einstellungen erreichst du über den Link im Popup oder über die Erweiterungsdetails in Chrome.\n";
	}
}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_render_zip')) {
	function cmx_ext_chrome_render_zip(array $config): void {
		...
		$zip->addFromString('options.html', cmx_ext_chrome_options_html());
		$zip->addFromString('options.js', cmx_ext_chrome_options_js());
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_write_extension_dir')) {
	function cmx_ext_chrome_write_extension_dir(string $dir, array $config): bool {
		...
		$files = [
			...
			'options.html'     => cmx_ext_chrome_options_html(),
			'options.js'       => cmx_ext_chrome_options_js(),
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_document_self_meta_key')) {
	function cmx_ext_chrome_document_self_meta_key(): string {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_document_upload_dir_parts')) {
	function cmx_ext_chrome_document_upload_dir_parts(int $year): array {
		if (\function_exists(__NAMESPACE__ . '\\cmx_dok_upload_target_dir')) {
			return (array) cmx_dok_upload_target_dir('dokumente', $year);
		}
		$base = \defined('WP_CONTENT_DIR')
			? \wp_normalize_path((string) \constant('WP_CONTENT_DIR') . '/uploads/misbuero/archiv/' . $year . '/dokumente')
			: '';
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_document_title_from_source')) {
	function cmx_ext_chrome_document_title_from_source(string $source_title = ''): string {
		$title = \trim(\preg_replace('/\s+/', ' ', \wp_strip_all_tags($source_title)));
		$title = (string) \preg_replace('/\.pdf$/i', '', $title);
		$title = \trim($title);
		return $title !== '' ? $title : 'PDF Import ' . \wp_date('Y-m-d H:i');
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_create_document')) {
	function cmx_ext_chrome_create_document(string $source_title = ''): int {
		if (!\post_type_exists('dokumente')) {
			return 0;
		}

		$title = cmx_ext_chrome_document_title_from_source($source_title);
		$post_id = (int) \wp_insert_post([
			'post_type' => 'dokumente',
			'post_status' => 'publish',
			'post_title' => $title,
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_attach_pdf_to_document')) {
	function cmx_ext_chrome_attach_pdf_to_document(int $post_id, array $file): array {
		if ($post_id <= 0 || \get_post_type($post_id) !== 'dokumente') {
			throw new \RuntimeException('Ungültiges Dokument.');
		}
		if (empty($file['tmp_name']) || !\is_uploaded_file((string) $file['tmp_name'])) {
			throw new \RuntimeException('Keine hochgeladene PDF-Datei empfangen.');
		}
		...
		if ($file_error !== \UPLOAD_ERR_OK) {
			throw new \RuntimeException('PDF-Upload fehlgeschlagen.');
		}
		...
		$year = (int) \wp_date('Y');
		[$dir_base, $base_url] = cmx_ext_chrome_document_upload_dir_parts($year);
		if ($dir_base === '' || $base_url === '') {
			throw new \RuntimeException('Dokumente-Upload-Verzeichnis konnte nicht vorbereitet werden.');
		}
		...
		if (!\is_array($uploaded) || empty($uploaded['file'])) {
			throw new \RuntimeException('Dokument-Upload fehlgeschlagen.');
		}

		$post_title = cmx_ext_chrome_document_title_from_source((string) $post->post_title);
		$base_dir = \dirname((string) $uploaded['file']);
		$desired_base = \sanitize_file_name($post_title . '.pdf');
		if ($desired_base === '' || \strtolower((string) \pathinfo($desired_base, \PATHINFO_EXTENSION)) !== 'pdf') {
			$desired_base = 'pdf-import.pdf';
		}
		...
		if ($new_path !== $uploaded['file'] && @\rename((string) $uploaded['file'], $new_path)) {
			$uploaded['file'] = $new_path;
		}

		$rel = \ltrim((string) \str_replace(\trailingslashit((string) (\WP_CONTENT_DIR . '/uploads')), '', (string) $uploaded['file']), '/');
		$self_meta_key = cmx_ext_chrome_document_self_meta_key();
		$existing = (array) \get_post_meta($post_id, $self_meta_key, true);
		$existing = \array_values(\array_filter($existing, static function ($value): bool {
			return $value !== '' && $value !== null;
		}));
		$existing[] = $rel;
		$existing = \array_values(\array_unique($existing));
		\update_post_meta($post_id, $self_meta_key, $existing);
		\update_post_meta($post_id, '_cmx_dokumente_file_path', $rel);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_apply_pdf_extraction')) {
	function cmx_ext_chrome_apply_pdf_extraction(int $post_id, array $upload): array {
		$file_path = isset($upload['file']) ? (string) $upload['file'] : '';
		$file_rel = isset($upload['path']) ? (string) $upload['path'] : '';
		$extraction = [
			'success' => false,
			'message' => 'PDF-Auswertung ist nicht verfügbar.',
			'data' => [],
			'applied' => [],
		];

		if ($post_id <= 0 || $file_path === '' || !\is_readable($file_path)) {
			$extraction['message'] = 'PDF konnte nicht gelesen werden.';
		} elseif (\class_exists(__NAMESPACE__ . '\\MIS_BUERO_BELEG_UPLOAD') && \method_exists(__NAMESPACE__ . '\\MIS_BUERO_BELEG_UPLOAD', 'apply_pdf_extraction_to_beleg')) {
			$extraction = MIS_BUERO_BELEG_UPLOAD::apply_pdf_extraction_to_beleg($post_id, $file_path);
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_schedule_pdf_extraction')) {
	function cmx_ext_chrome_schedule_pdf_extraction(int $post_id, array $upload): bool {
		$file_path = isset($upload['file']) ? (string) $upload['file'] : '';
		$file_rel = isset($upload['path']) ? (string) $upload['path'] : '';
		if ($post_id <= 0 || $file_path === '' || !\is_readable($file_path)) {
			\update_post_meta($post_id, '_cmx_beleg_pdf_extraction_last', [
				...
				'applied' => [],
				'source' => '',
				'ocr_error' => 'PDF konnte nicht gelesen werden.',
				'services_status' => '',
				'services_endpoint' => '',
				'services_body_excerpt' => '',
			]);
			return false;
		}

		$args = [$post_id, $file_path, $file_rel];
		if (!\wp_next_scheduled(CMX_EXT_CHROME_PROCESS_EVENT, $args)) {
			$scheduled = \wp_schedule_single_event(\time() + 1, CMX_EXT_CHROME_PROCESS_EVENT, $args);
			if (!$scheduled) {
				\update_post_meta($post_id, '_cmx_beleg_pdf_extraction_last', [
					'time' => \current_time('mysql'),
					'file' => $file_rel,
					'success' => 0,
					'message' => 'PDF-Auswertung konnte nicht geplant werden.',
					'applied' => [],
					'source' => '',
					'ocr_error' => 'PDF-Auswertung konnte nicht geplant werden.',
					'services_status' => '',
					'services_endpoint' => '',
					'services_body_excerpt' => '',
				]);
		...
		\update_post_meta($post_id, '_cmx_beleg_pdf_extraction_last', [
			...
			'file' => $file_rel,
			'success' => 0,
			'message' => 'PDF-Auswertung wurde geplant.',
			'applied' => [],
			'source' => '',
			'ocr_error' => '',
			'services_status' => '',
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_process_pdf_import')) {
	function cmx_ext_chrome_process_pdf_import(int $post_id, string $file_path, string $file_rel = ''): void {
		try {
			cmx_ext_chrome_apply_pdf_extraction($post_id, [
				'file' => $file_path,
				'path' => $file_rel,
			]);
		} catch (\Throwable $exception) {
	...
	\add_action(CMX_EXT_CHROME_PROCESS_EVENT, __NAMESPACE__ . '\\cmx_ext_chrome_process_pdf_import', 10, 3);
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_send_json_success')) {
	function cmx_ext_chrome_send_json_success(array $data): void {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_import_mode_from_request')) {
	function cmx_ext_chrome_import_mode_from_request(): string {
		$mode = isset($_POST['import_mode']) ? \sanitize_key((string) \wp_unslash($_POST['import_mode'])) : '';
		return $mode === 'dokument' ? 'dokument' : 'beleg';
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_ext_chrome_upload_response')) {
	function cmx_ext_chrome_upload_response(): void {
		$user_id = cmx_ext_chrome_request_user_id();
		if ($user_id <= 0) {
		...
		if (!\current_user_can('upload_files')) {
			\wp_send_json_error(['message' => 'Dieser Benutzer darf keine Dateien hochladen.'], 403);
		}
		...
		$import_mode = cmx_ext_chrome_import_mode_from_request();

		if ($import_mode === 'dokument') {
			$doc_id = cmx_ext_chrome_create_document((string) $title_base);
			if ($doc_id <= 0) {
				\wp_send_json_error(['message' => 'Neues Dokument konnte nicht angelegt werden.'], 500);
			}

			try {
				$upload = cmx_ext_chrome_attach_pdf_to_document($doc_id, (array) $_FILES['file']);
			} catch (\Throwable $exception) {
		...
		try {
			$upload = cmx_ext_chrome_attach_pdf_to_beleg($beleg_id, (array) $_FILES['file']);
		} catch (\Throwable $exception) {
		...
		$response_data = [
			...
			'extracted' => false,
			'extraction_message' => 'PDF-Auswertung läuft im Hintergrund.',
			'applied' => [],
			'processing_scheduled' => true,
		];

		if (\function_exists('fastcgi_finish_request')) {
			\update_post_meta($beleg_id, '_cmx_beleg_pdf_extraction_last', [
				'time' => \current_time('mysql'),
				'file' => (string) ($upload['path'] ?? ''),
				'success' => 0,
				'message' => 'PDF-Auswertung läuft im Hintergrund.',
				'applied' => [],
				'source' => '',
				'ocr_error' => '',
				'services_status' => '',
			...
			cmx_ext_chrome_send_json_success($response_data);
			\fastcgi_finish_request();
			cmx_ext_chrome_apply_pdf_extraction($beleg_id, $upload);
			exit;
		}

		$processing_scheduled = cmx_ext_chrome_schedule_pdf_extraction($beleg_id, $upload);
		$response_data['extraction_message'] = $processing_scheduled ? 'PDF-Auswertung wurde geplant.' : 'PDF-Auswertung konnte nicht geplant werden.';
		$response_data['processing_scheduled'] = $processing_scheduled;
		\wp_send_json_success($response_data);
	}
	\add_action('wp_ajax_' . CMX_EXT_CHROME_UPLOAD_ACTION, __NAMESPACE__ . '\\cmx_ext_chrome_upload_response');
	\add_action('wp_ajax_nopriv_' . CMX_EXT_CHROME_UPLOAD_ACTION, __NAMESPACE__ . '\\cmx_ext_chrome_upload_response');
}
