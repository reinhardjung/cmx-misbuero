...
add_action('pre_get_posts', function(\WP_Query $q) {
	...
	$import_only  = isset($_GET['cmx_import_filter']) && (string) \wp_unslash($_GET['cmx_import_filter']) === '1';
	...
	if ($import_only) {
		$meta_query[] = [
			'relation' => 'OR',
			[
				'key'     => '_cmx_beleg_import_flag',
				'value'   => '1',
				'compare' => '=',
			],
			[
				'key'     => '_cmx_beleg_pdf_extraction_last',
				'compare' => 'EXISTS',
			],
		];
	}
...
\add_action('restrict_manage_posts', function ($post_type = '', $which = ''): void {
	...
	$import_checked = isset($_GET['cmx_import_filter']) && (string) \wp_unslash($_GET['cmx_import_filter']) === '1';
	$import_classes = 'cmx-belege-woo-filter cmx-belege-import-filter' . ($import_checked ? ' is-checked' : '');
	echo '<label for="cmx_import_filter" class="' . \esc_attr($import_classes) . '">';
	echo '<input type="checkbox" name="cmx_import_filter" id="cmx_import_filter" value="1" ' . \checked($import_checked, true, false) . '>';
	echo '<span>' . \esc_html__('Import', 'cmx-misbuero') . '</span>';
	echo '</label>';
}, 999, 2);
...
\add_filter('views_edit-belege', function(array $views): array {
	...
	$is_current = isset($_GET['cmx_import_filter']) && (string) \wp_unslash($_GET['cmx_import_filter']) === '1';
	$views['cmx_import_filter_belege'] = '<a href="' . \esc_url($url) . '"' . ($is_current ? ' class="current" aria-current="page"' : '') . '>Import</a>';
