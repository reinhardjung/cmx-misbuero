	...
	(function(){
		...
		function sumAll(){
			...
			if (sumInput) {
				...
				sumInput.style.display = hasPos ? 'none' : '';
				if (out) out.style.display = hasPos ? '' : 'none';
				if (hasPos) {
					sumInput.dataset.manual = '';
				} else if (sumInput.dataset.manual === '1') {
			...
			if (sumInput && !sumInput.dataset.manual && hasPos) {
		...
		function bindLive(){
			...
			if (sumInput) {
				...
				sumInput.addEventListener('blur', function(){
					if (sumInput.dataset.manual === '1' && !sumInput.readOnly) {
						sumInput.value = formatCH(toNumber(sumInput.value || '0'));
					}
				});
...
add_action('save_post_belege', function ($post_id) {
	...
	$raw = trim($raw);
	$existing_raw = (string) \get_post_meta($post_id, '_cmx_beleg_summe_override', true);
	$services_document = \get_post_meta($post_id, '_cmx_beleg_services_document', true);
	$has_services_total = \is_array($services_document) && $existing_raw !== '' && (float) cmx_norm_decimal($existing_raw) > 0.0;
	if ($raw === '') {
		if ($has_services_total) {
			return;
		}
		delete_post_meta($post_id, '_cmx_beleg_summe_override');
		return;
