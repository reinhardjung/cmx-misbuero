...
if (!\function_exists(__NAMESPACE__ . '\\cmx_belegeingang_positive_amount')) {
	function cmx_belegeingang_positive_amount($value): float {
		if (\is_string($value)) {
			if (\function_exists(__NAMESPACE__ . '\\cmx_norm_decimal')) {
				$value = cmx_norm_decimal($value);
			} else {
				$value = \str_replace(["\xc2\xa0", ' ', "'"], '', \trim($value));
				$value = \str_replace(',', '.', (string) \preg_replace('/[^\d,\.\+\-]/u', '', $value));
			}
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_belegeingang_total_from_facturx')) {
	function cmx_belegeingang_total_from_facturx(array $facturx): float {
		foreach (['gross_total', 'grand_total', 'due_payable_total', 'total_amount', 'amount'] as $key) {
			$amount = cmx_belegeingang_positive_amount($facturx[$key] ?? 0);
			if ($amount > 0.0) {
				return $amount;
			}
		}

		$net = cmx_belegeingang_positive_amount($facturx['net_total'] ?? ($facturx['tax_basis_total'] ?? 0));
		$tax = cmx_belegeingang_positive_amount($facturx['tax_total'] ?? 0);
		if ($net > 0.0 && $tax > 0.0) {
		...
		$position_total = 0.0;
		foreach ((array) ($facturx['positions'] ?? []) as $position) {
			if (!\is_array($position)) {
				continue;
			}
			$line_total = cmx_belegeingang_positive_amount($position['line_total'] ?? ($position['betrag'] ?? 0));
			if ($line_total <= 0.0) {
				$qty = cmx_belegeingang_positive_amount($position['menge'] ?? 1);
				$price = cmx_belegeingang_positive_amount($position['preis'] ?? 0);
				$line_total = $qty > 0.0 ? $qty * $price : $price;
			}
			$position_total += $line_total;
		}

		return $position_total > 0.0 ? \round($position_total, 2) : 0.0;
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_belegeingang_apply_amount_meta')) {
	function cmx_belegeingang_apply_amount_meta(int $post_id, array $facturx): void {
		$total = cmx_belegeingang_total_from_facturx($facturx);
		if ($post_id <= 0 || $total <= 0.0) {
			return;
		}

		$formatted = (string) \number_format($total, 2, '.', '');
		\update_post_meta($post_id, '_cmx_beleg_summe_override', $formatted);
		\update_post_meta($post_id, 'betrag', $formatted);
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_belegeingang_format_note_amount')) {
	function cmx_belegeingang_format_note_amount(float $amount, string $currency): string {
		if ($amount <= 0.0) {
			return '';
		}
		$formatted = \function_exists(__NAMESPACE__ . '\\cmx_format_swiss_number')
			? (string) cmx_format_swiss_number($amount, 2)
			: (string) \number_format($amount, 2, '.', "'");
		$currency = \trim($currency);
		return \trim(($currency !== '' ? $currency . ' ' : '') . $formatted);
	}
}

if (!\function_exists(__NAMESPACE__ . '\\cmx_belegeingang_positions_note_text')) {
	function cmx_belegeingang_positions_note_text(array $facturx): string {
		$positions = \array_values(\array_filter((array) ($facturx['positions'] ?? []), static fn($row): bool => \is_array($row)));
		if ($positions === []) {
			return '';
		}
		...
		$title = 'PDF-Positionen aus Factur-X/ZUGFeRD' . ($document_no !== '' ? ' (' . \esc_html($document_no) . ')' : '');
		$html = '<h4>' . $title . '</h4>';
		...
		foreach ($positions as $index => $position) {
			$title = \trim((string) ($position['artikel_name'] ?? ($position['title'] ?? ($position['name'] ?? ''))));
			$description = \trim((string) ($position['beschreibung'] ?? ($position['description'] ?? '')));
			$qty = cmx_belegeingang_positive_amount($position['menge'] ?? ($position['qty'] ?? 0));
			$unit = \trim((string) ($position['einheit'] ?? ($position['unit'] ?? '')));
			$price = cmx_belegeingang_positive_amount($position['preis'] ?? ($position['unit_price'] ?? 0));
			$line_total = cmx_belegeingang_positive_amount($position['line_total'] ?? ($position['betrag'] ?? 0));
			if ($line_total <= 0.0 && $qty > 0.0 && $price > 0.0) {
				$line_total = \round($qty * $price, 2);
			}

			$qty_text = '';
			if ($qty > 0.0) {
				$qty_text = \function_exists(__NAMESPACE__ . '\\cmx_format_swiss_number')
					? (string) cmx_format_swiss_number($qty, 2)
					: (string) \number_format($qty, 2, '.', "'");
			}
			$price_text = cmx_belegeingang_format_note_amount($price, $currency);
			$line_total_text = cmx_belegeingang_format_note_amount($line_total, $currency);

			$html .= '<tr>';
			$html .= '<td>' . ((int) $index + 1) . '</td>';
			$html .= '<td>' . \esc_html($title !== '' ? $title : 'Position') . '</td>';
			$html .= '<td>' . \esc_html($description !== $title ? $description : '') . '</td>';
			$html .= '<td>' . \esc_html(\trim($qty_text . ' ' . $unit)) . '</td>';
			$html .= '<td>' . \esc_html($price_text) . '</td>';
			$html .= '<td>' . \esc_html($line_total_text) . '</td>';
			$html .= '</tr>';
		}
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_belegeingang_append_positions_note')) {
	function cmx_belegeingang_append_positions_note(int $post_id, array $facturx): void {
		if ($post_id <= 0) {
			return;
		}
		$text = cmx_belegeingang_positions_note_text($facturx);
		if ($text === '') {
			return;
		}

		$hash = \md5($text);
		if ((string) \get_post_meta($post_id, '_cmx_belegeingang_positions_note_hash', true) === $hash) {
			return;
		}
		...
		\update_post_meta($post_id, '_cmx_belegeingang_positions_note_hash', $hash);
	}
}
	...
	function cmx_belegeingang_parse_facturx_pdf(string $pdf_path): array {
		...
		try {
			...
			if ($reader->firstDocumentPosition()) {
				do {
					...
					$positions[] = [
						...
						'line_total' => (string) \number_format((float) ($line_amount ?: 0), 2, '.', ''),
						'rabatt' => '',
			...
			$summary_data = [
				...
				'due_payable_total' => (float) ($due_payable ?: 0),
				'line_total' => (float) ($line_total ?: 0),
				'charge_total' => (float) ($charge_total ?: 0),
				'allowance_total' => (float) ($allowance_total ?: 0),
				'tax_basis_total' => (float) ($tax_basis ?: 0),
				'tax_total' => (float) ($tax_total ?: 0),
				'rounding_total' => (float) ($rounding ?: 0),
				'prepaid_total' => (float) ($prepaid ?: 0),
				'net_total' => (float) ($tax_basis ?: $line_total ?: 0),
				'positions' => $positions,
			];
			$summary_data['gross_total'] = cmx_belegeingang_total_from_facturx($summary_data);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_belegeingang_apply_facturx_meta')) {
	function cmx_belegeingang_apply_facturx_meta(int $post_id, array $facturx): void {
		...
		cmx_belegeingang_apply_amount_meta($post_id, $facturx);
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_belegeingang_contact_id_for_party')) {
	function cmx_belegeingang_contact_id_for_party(string $name, string $address, array $emails, string $phone = '', bool $match_by_name = true): int {
		$name = \trim($name);
		$emails = cmx_belegeingang_normalize_emails($emails);
		if ($name === '' && $emails !== []) {
		...
		if ($kontakt_id <= 0 && $match_by_name && $name !== '' && \function_exists(__NAMESPACE__ . '\\cmx_beleg_find_existing_kontakt_id_from_label')) {
...
if (!\function_exists(__NAMESPACE__ . '\\cmx_belegeingang_import_as_supplier_invoice')) {
	function cmx_belegeingang_import_as_supplier_invoice(int $post_id): bool {
		...
		$kontakt_id = cmx_belegeingang_contact_id_for_party($seller, (string) ($data['seller_address'] ?? ''), $seller_emails, (string) ($data['seller_phone'] ?? ''), false);
		$recipient_kontakt_id = cmx_belegeingang_contact_id_for_party($buyer, (string) ($data['buyer_address'] ?? ''), $buyer_emails, (string) ($data['buyer_phone'] ?? ''), false);
		...
		if ($kontakt_id > 0) {
			...
			\delete_post_meta($post_id, '_cmx_beleg_absender_kontakt_id');
			\delete_post_meta($post_id, '_cmx_belegeingang_sender_kontakt_id');
		}
		if ($recipient_kontakt_id > 0) {
			...
			\delete_post_meta($post_id, '_cmx_beleg_empfaenger_kontakt_id');
			\delete_post_meta($post_id, '_cmx_belegeingang_recipient_kontakt_id');
		}
		...
		cmx_belegeingang_apply_amount_meta($post_id, $data);
		cmx_belegeingang_append_positions_note($post_id, $data);
...
\add_action('admin_post_cmx_belegeingang_confirm', function (): void {
	...
	$redirect_to = (string) \add_query_arg('cmx_focus_contact', '1', $redirect_to);
