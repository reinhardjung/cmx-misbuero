...
\add_action('admin_post_cmx_export_belege_list_pdf', function(){
	...
	foreach ((array) $row_items as $item) {
		...
		if ($post_id > 0 && \function_exists(__NAMESPACE__ . '\\cmxbu_get_stable_token')) {
			...
			if ($token !== '') {
				$pdf_url = (string) cmxbu_get_beleg_public_url($post_id, $token);
				...
				if (!empty($upload_cache[$post_id])) {
					$upload_url = (string) cmxbu_get_beleg_public_url($post_id, $token, ['quelle' => 'upload']);
				}
			}
		}
...
<head>
	<meta charset="utf-8">
		...
		.header-logo{width:150px;height:84px;min-width:150px;max-width:150px;min-height:84px;max-height:84px;object-fit:contain}
...
<body>
		...
		<div class="doc-header-logo">
			...
			// Wenn der Ergebnisblock bei 20 oder mehr Zeilen sonst alleine auf einer Folgeseite landen würde,
			// wird die letzte Zeile mitgenommen.
