...
if (!\function_exists(__NAMESPACE__ . '\\cmxbu_send_beleg_mail')) {
	function cmxbu_send_beleg_mail(int $post_id, array $args = []) {
		...
		$download_url = cmxbu_get_beleg_public_url($post_id, $token);

		$kontakt_id = (int) ($args['recipient_contact_id'] ?? 0);
		if ($kontakt_id <= 0) {
...
\add_action('all_admin_notices', function (): void {
	...
	if (!empty($_GET['cmx_beleg_mail_error'])) {
		...
		echo '<div class="notice notice-error is-dismissible"><p><strong>E-Mail konnte nicht vorbereitet oder gesendet werden.</strong> ' . \esc_html($detail_text) . ' <a href="' . \esc_url($settings_url) . '" class="button button-secondary" style="margin-left:8px;" target="_blank" rel="noopener noreferrer">Einstellungen / E-Mail</a></p>';
		echo '</div>';
		return;
	}
