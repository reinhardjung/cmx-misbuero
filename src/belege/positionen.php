...
if (!function_exists(__NAMESPACE__ . '\cmx_artikel_search_variant_entries')) {
	function cmx_artikel_search_variant_entries(int $post_id, string $term = ''): array {
		...
		if (\function_exists(__NAMESPACE__ . '\\cmx_artikel_variant_entry_is_selectable')) {
			$entries = \array_values(\array_filter($entries, static function ($entry): bool {
				return \is_array($entry) && cmx_artikel_variant_entry_is_selectable((array) $entry);
			}));
		}
		$term = \trim($term);
...
add_action('cmx_beleg_positionen_after_add_button', function(): void {
	echo ' <div class="cmx-task-picker-wrap cmx-infrastruktur-picker-wrap" id="cmx-infrastruktur-picker-wrap">';
	echo '<button type="button" class="button button-secondary" id="cmx-add-infrastruktur">Infrastruktur hinzufügen</button>';
	echo '<div class="cmx-task-picker-panel" id="cmx-infrastruktur-picker-panel" style="display:none;">';
	echo '<input type="text" id="cmx-infrastruktur-picker-search" class="regular-text" placeholder="Infrastruktur suchen..." autocomplete="off">';
	echo '<ul id="cmx-infrastruktur-picker-list" class="cmx-task-picker-list"></ul>';
	echo '</div></div>';
}, 30);

add_action('wp_ajax_cmx_search_beleg_infrastrukturen', function(): void {
	if (!\current_user_can('edit_posts')) {
		\wp_send_json_error(['msg' => 'forbidden'], 403);
	}

	$term = isset($_GET['term']) ? \sanitize_text_field(\wp_unslash($_GET['term'])) : '';
	$kontakt_id = isset($_GET['kontakt_id']) ? (int) $_GET['kontakt_id'] : 0;
	$args = [
	...
	if ($kontakt_id > 0) {
		$args['meta_query'] = [[
			'key'     => '_cmx_infrastruktur_kontakt_id',
			'value'   => $kontakt_id,
			'compare' => '=',
			'type'    => 'NUMERIC',
		]];
	...
	$unit_ids = [];
	if (\function_exists(__NAMESPACE__ . '\\cmx_artikel_einheiten_options')) {
		foreach (cmx_artikel_einheiten_options() as $option) {
			$unit_id = (int) ($option['id'] ?? 0);
			$unit_name = \trim((string) ($option['name'] ?? ''));
			if ($unit_id <= 0 || $unit_name === '') {
				continue;
			}
			$unit_ids[\sanitize_title($unit_name)] = $unit_id;
		}
	}
	...
	$unit_aliases = [
		'monatlich'  => ['monatlich', 'monat', 'monate'],
		'stuendlich' => ['stündlich', 'stuendlich', 'stunde', 'stunden'],
		'jaehrlich'  => ['jährlich', 'jaehrlich', 'jahr', 'jahre'],
	];
	$items = [];
	foreach (\get_posts($args) as $infrastruktur) {
		if (!$infrastruktur instanceof \WP_Post) {
			continue;
		}

		$id = (int) $infrastruktur->ID;
		$unit_key = \sanitize_key((string) \get_post_meta($id, '_cmx_infrastruktur_einheit', true));
		$unit = (string) ($unit_labels[$unit_key] ?? '');
		$unit_id = 0;
		foreach ($unit_aliases[$unit_key] ?? [$unit] as $unit_alias) {
			$unit_id = (int) ($unit_ids[\sanitize_title($unit_alias)] ?? 0);
			if ($unit_id > 0) {
				break;
			}
		}
		$owner_id = (int) \get_post_meta($id, '_cmx_infrastruktur_kontakt_id', true);
		$items[] = [
			'id'            => $id,
			'title'         => \get_the_title($id),
			'ip'            => (string) \get_post_meta($id, '_cmx_infrastruktur_server_ip', true),
			'price'         => (string) \get_post_meta($id, '_cmx_infrastruktur_vk', true),
			'unit'          => $unit,
			'unit_id'       => $unit_id,
			'kontakt_title' => $owner_id > 0 ? \get_the_title($owner_id) : '',
...
function cmx_render_position_row($i, $pos) {
	...
	$infrastruktur_id = isset($pos['infrastruktur_id']) ? (int) $pos['infrastruktur_id'] : 0;
	if ($infrastruktur_id > 0 && \get_post_type($infrastruktur_id) !== 'infrastruktur') {
		$infrastruktur_id = 0;
	}
	...
	echo '<td style="min-width:232px">';
	$edit_link = $artikel_id
		? get_edit_post_link($artikel_id, '')
		: ($infrastruktur_id > 0 ? get_edit_post_link($infrastruktur_id, '') : '');
	$edit_label = $artikel_id <= 0 && $infrastruktur_id > 0 ? 'Infrastruktur' : 'Artikel';
	echo '<a href="'.esc_url($edit_link).'" class="cmx-artikel-edit" data-cmx-help-key="beleg_artikel_edit" aria-label="'.esc_attr($edit_label).' bearbeiten" title="'.esc_attr($edit_label).' im neuen Tab bearbeiten" target="_blank" rel="noopener noreferrer" style="'.($edit_link ? '' : 'pointer-events:none; opacity:0.35;').'">✎</a>';
	echo '<input type="hidden" name="cmx_positionen['.$i.'][artikel_id]" class="cmx-artikel-id" value="'.esc_attr($artikel_id).'">';
	echo '<input type="hidden" name="cmx_positionen['.$i.'][infrastruktur_id]" class="cmx-infrastruktur-id" value="'.esc_attr($infrastruktur_id).'">';
...
<span style="display:inline-block; transform:translateY(12px);">✎</span>
	...
	echo '<textarea name="cmx_positionen['.$i.'][beschreibung]" rows="1" style="width:100%;max-width:100%;box-sizing:border-box;height:38px;min-height:38px;resize:vertical;overflow-y:auto;line-height:1.4;">'.$beschreibung.'</textarea>';
	echo '</td>';
	...
	foreach ($positionen as $row) {
		...
		if ($custom === false) {
			continue;
				...
				$infrastruktur_id = isset($row['infrastruktur_id']) ? (int) $row['infrastruktur_id'] : 0;
				if ($infrastruktur_id > 0 && \get_post_type($infrastruktur_id) !== 'infrastruktur') {
					$infrastruktur_id = 0;
				}
		...
		$rabatt       = sanitize_text_field($rabatt_raw);
			...
			if (strlen($beschreibung) > 10000) $beschreibung = substr($beschreibung, 0, 10000);
				...
				$clean[] = [
					...
					'infrastruktur_id' => $infrastruktur_id,
...
add_action('wp_ajax_cmx_get_artikel_vk', function() {
	...
	$variant_index = isset($_POST['variant_index']) && $_POST['variant_index'] !== '' && \is_numeric((string) \wp_unslash($_POST['variant_index']))
		? (int) \wp_unslash($_POST['variant_index'])
		: '';
	$vk = \function_exists(__NAMESPACE__ . '\\cmx_artikel_variant_vk')
		? cmx_artikel_variant_vk($artikel_id, $variant_index, true)
		: get_post_meta($artikel_id, '_cmx_artikel_vk', true);
	...
	foreach ($ids as $id) {
		...
		if (\function_exists(__NAMESPACE__ . '\\cmx_artikel_search_variant_entries')) {
			...
			$all_variant_entries = \function_exists(__NAMESPACE__ . '\\cmx_artikel_admin_variant_entries')
				? (array) cmx_artikel_admin_variant_entries($id)
				: [];
		...
		if (!empty($all_variant_entries) && empty($variant_entries)) {
			continue;
		}
...
function cmx_beleg_positionen_js() {
	...
	jQuery(function($){
		...
		const $infrastrukturPickerWrap = $('#cmx-infrastruktur-picker-wrap');
		const $infrastrukturPickerPanel = $('#cmx-infrastruktur-picker-panel');
		const $infrastrukturPickerSearch = $('#cmx-infrastruktur-picker-search');
		const $infrastrukturPickerList = $('#cmx-infrastruktur-picker-list');
		let taskPickerItems = [];
		let taskPickerActive = -1;
		let taskPickerTimer = null;
		let taskPickerSeq = 0;
		let infrastrukturPickerItems = [];
		let infrastrukturPickerActive = -1;
		let infrastrukturPickerTimer = null;
		let infrastrukturPickerSeq = 0;
		let dragMode = 'row';
		...
		/* ========= Suggest (unverändert) ========= */
			...
			function fetchInfrastrukturen(term, cb){
				$.getJSON(AJAX_URL, {
					action: 'cmx_search_beleg_infrastrukturen',
					term: term || '',
					kontakt_id: currentKontaktId()
				}, function(data){
					cb(Array.isArray(data) ? data : []);
				}).fail(function(){
					cb([]);
			...
			function infrastrukturPickerClose(){
				infrastrukturPickerItems = [];
				infrastrukturPickerActive = -1;
				if ($infrastrukturPickerList.length) $infrastrukturPickerList.empty();
				if ($infrastrukturPickerPanel.length) $infrastrukturPickerPanel.hide();
			}

			function infrastrukturPickerRender(items){
				infrastrukturPickerItems = Array.isArray(items) ? items : [];
				infrastrukturPickerActive = -1;
				if (!$infrastrukturPickerList.length) return;
				if (!infrastrukturPickerItems.length) {
					const hasContact = currentKontaktId() > 0;
					$infrastrukturPickerList.html(
						'<li class="cmx-task-picker-empty">' +
						(hasContact
							? 'Für diesen Kontakt wurden keine Infrastrukturen gefunden.'
							: 'Keine Infrastrukturen gefunden.') +
						'</li>'
					);
					return;
				}

				const html = infrastrukturPickerItems.map((item, index) => {
					const title = (item.title || '').toString();
					const meta = [];
					if (item.ip) meta.push((item.ip || '').toString());
					if (item.unit) meta.push((item.unit || '').toString());
					if (item.price !== undefined && item.price !== null && String(item.price).trim() !== '') {
						meta.push('CHF ' + formatSwiss(parseNumberFlexible(item.price)));
					}
					if (currentKontaktId() <= 0 && item.kontakt_title) {
						meta.push((item.kontakt_title || '').toString());
					}
					return '' +
						'<li data-index="' + index + '">' +
							'<div class="cmx-task-picker-row">' +
								'<span class="cmx-task-picker-title">' + escHtml(title) + '</span>' +
								(meta.length
									? '<span class="cmx-task-picker-meta">' + escHtml(meta.join(' · ')) + '</span>'
									: '') +
							'</div>' +
						'</li>';
				}).join('');
				$infrastrukturPickerList.html(html);
			}

			function infrastrukturPickerFetchAndRender(term){
				const reqSeq = ++infrastrukturPickerSeq;
				fetchInfrastrukturen(term, function(rows){
					if (reqSeq !== infrastrukturPickerSeq) return;
					infrastrukturPickerRender(rows);
				});
			}
			...
			function applyTaskPrefill($row, task){
				...
				const $infrastrukturId = $row.find('.cmx-infrastruktur-id').first();
				...
				$desc.val(info);
					...
					if ($edit.length) {
						$edit.attr('aria-label', 'Artikel bearbeiten');
						$edit.attr('title', 'Artikel im neuen Tab bearbeiten');
						if (artikelId > 0) {
			...
			function applyInfrastrukturPrefill($row, infrastruktur){
				if (!infrastruktur || typeof infrastruktur !== 'object') return;

				const id = parseInt(infrastruktur.id || 0, 10) || 0;
				const title = (infrastruktur.title || '').toString();
				const priceRaw = (infrastruktur.price ?? '').toString().trim();
				const $edit = $row.find('.cmx-artikel-edit').first();

				$row.find('.cmx-artikel-id').first().val('');
				$row.find('.cmx-infrastruktur-id').first().val(id > 0 ? id : '');
				$row.find('.cmx-artikel-name').first().val(title);
				$row.find('.cmx-artikel-variant-index').first().val('');
				$row.find('.cmx-artikel-autocomplete').first().val(title);
				$row.find('.cmx-task-idx, .cmx-task-uid, .cmx-task-projekt-id').val('');
				$row.find('input[name*="[menge]"]').first().val(formatSwiss(1));
				$row.find('input[name*="[preis]"]').first().val(
					priceRaw !== '' ? formatSwiss(parseNumberFlexible(priceRaw)) : ''
				);
				$row.find('input[name*="[rabatt]"]').first().val('');
				$row.find('textarea[name*="[beschreibung]"]').first().val('');
				setRowUnitSelection($row, infrastruktur.unit_id);

				if ($edit.length && id > 0 && infrastruktur.edit_url) {
					$edit.attr('href', (infrastruktur.edit_url || '').toString());
					$edit.attr('aria-label', 'Infrastruktur bearbeiten');
					$edit.attr('title', 'Infrastruktur im neuen Tab bearbeiten');
					$edit.css({ 'pointer-events':'auto', 'opacity':'1' });
				}
			}
			...
			function addPositionRow(prefill){
				...
				if (prefill && typeof prefill === 'object') {
					if (prefill.cmx_source === 'infrastruktur') {
						applyInfrastrukturPrefill(newRow, prefill);
					} else {
			...
			$ctx.find('.cmx-artikel-autocomplete').each(function(){
				...
				let t=null;
					...
					if (it.id) {
						...
						if(it.id){
							$.post(AJAX_URL, { action:'cmx_get_artikel_vk', artikel_id: it.id, variant_index: (it.variant_index ?? '') }, function(resp){
								if(resp && resp.success && resp.data){
				...
				$input.on('input', function(){
					...
					const $row = $input.closest('tr');
						...
						if (artikelId <= 0) {
							$row.find('.cmx-infrastruktur-id').first().val('');
		...
		table.on('blur', 'input[name*="[rabatt]"]', function(){
			...
			function taskPickerChoose(index){
				...
				if (lastIsArticleRow) {
					...
					const infrastructureId = parseInt(
						($lastRow.find('.cmx-infrastruktur-id').first().val() || '').toString(),
						10
					) || 0;
					const articleName = (
						$lastRow.find('.cmx-artikel-name').first().val() || ''
					).toString().trim();
					const hasSelectedPosition =
						(!isNaN(artikelId) && artikelId > 0)
						|| infrastructureId > 0
						|| articleName !== '';
			...
			function infrastrukturPickerChoose(index){
				const idx = parseInt(index, 10);
				if (isNaN(idx) || idx < 0 || idx >= infrastrukturPickerItems.length) return;
				const selected = $.extend({}, infrastrukturPickerItems[idx], {
					cmx_source: 'infrastruktur'
				});
				let $row = $();
				const $lastRow = table.children('tr').last();
				const lastIsArticleRow = $lastRow.length
				...
				if (lastIsArticleRow) {
					const articleId = parseInt(
						($lastRow.find('.cmx-artikel-id').first().val() || '').toString(),
						10
					) || 0;
					const infrastructureId = parseInt(
						($lastRow.find('.cmx-infrastruktur-id').first().val() || '').toString(),
					...
					if (articleId <= 0 && infrastructureId <= 0 && articleName === '') {
						applyInfrastrukturPrefill($lastRow, selected);
						table.trigger('cmx_positionen_rows_changed');
						$row = $lastRow;
					}
				}
			...
			if ($infrastrukturPickerWrap.length) {
				$(document).on('click', '#cmx-add-infrastruktur', function(e){
					e.preventDefault();
					if ($infrastrukturPickerPanel.is(':visible')) {
						infrastrukturPickerClose();
						return;
					}
					taskPickerClose();
					infrastrukturPickerSeq++;
					infrastrukturPickerItems = [];
					infrastrukturPickerActive = -1;
					$infrastrukturPickerPanel.show();
					$infrastrukturPickerSearch.val('');
					$infrastrukturPickerList.html(
						'<li class="cmx-task-picker-empty">Lade Infrastrukturen...</li>'
					);
					infrastrukturPickerFetchAndRender('');
					setTimeout(function(){
						$infrastrukturPickerSearch.trigger('focus');
					}, 0);
				});

				$infrastrukturPickerSearch.on('input', function(){
					if (infrastrukturPickerTimer) clearTimeout(infrastrukturPickerTimer);
					const term = ($(this).val() || '').toString().trim();
					infrastrukturPickerTimer = setTimeout(function(){
						infrastrukturPickerFetchAndRender(term);
					}, 120);
				});

				$infrastrukturPickerSearch.on('keydown', function(e){
					if (!$infrastrukturPickerPanel.is(':visible')) return;
					if (e.key === 'Escape') {
						e.preventDefault();
						infrastrukturPickerClose();
						return;
					}
					if (!infrastrukturPickerItems.length) return;
					if (e.key === 'ArrowDown') {
						e.preventDefault();
						infrastrukturPickerActive =
							(infrastrukturPickerActive + 1 + infrastrukturPickerItems.length)
							% infrastrukturPickerItems.length;
					} else if (e.key === 'ArrowUp') {
						e.preventDefault();
						infrastrukturPickerActive =
							(infrastrukturPickerActive - 1 + infrastrukturPickerItems.length)
						...
						e.preventDefault();
						infrastrukturPickerChoose(infrastrukturPickerActive > -1
							? infrastrukturPickerActive
							: 0);
						return;
					} else {
					...
					$infrastrukturPickerList.children('li[data-index]').removeClass('active')
						.eq(infrastrukturPickerActive).addClass('active');
				});

				$infrastrukturPickerList.on('mousedown', 'li[data-index]', function(e){
					e.preventDefault();
					infrastrukturPickerChoose($(this).attr('data-index'));
				});

				$(document).on('mousedown', function(e){
					if (!$infrastrukturPickerPanel.is(':visible')) return;
					if (
						!$infrastrukturPickerWrap.is(e.target)
						&& $infrastrukturPickerWrap.has(e.target).length === 0
					) {
						infrastrukturPickerClose();
					}
				});
					...
					if (isAbschnitt) {
						...
						// Letzte normale Positionszeile: Inhalte leeren, damit kein zusätzlicher Platzhalter nötig ist
							$row.find('input, textarea, select').each(function(){
								const $el = $(this);
								if ($el.hasClass('cmx-artikel-id')) { $el.val(''); }
								else if ($el.hasClass('cmx-infrastruktur-id')) { $el.val(''); }
								else if ($el.hasClass('cmx-task-idx') || $el.hasClass('cmx-task-uid') || $el.hasClass('cmx-task-projekt-id')) { $el.val(''); }
		...
		<style>
			...
			#cmx-task-picker-search,
			#cmx-infrastruktur-picker-search{ width:100%; margin-bottom:6px; }
			...
			.cmx-pos-actions .button,
			.cmx-pos-actions .cmx-task-picker-wrap{
			...
			#cmx-positionen-table thead,
			#cmx-positionen-table tbody{
				display:block;
				width:100%;
			}
				#cmx-positionen-table thead tr,
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt){
					display:flex;
					flex-wrap:nowrap;
					align-items:center;
					width:100%;
			...
			#cmx-positionen-table thead th,
			#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td{
				display:block;
				min-width:0;
			}
				#cmx-positionen-table thead th:nth-child(1),
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td:nth-child(1){
					flex:1.05 1 220px;
					width:auto !important;
					min-width:190px !important;
				}
				#cmx-positionen-table thead th.cmx-pos-qty-head,
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-qty-cell{
					flex:0 0 224px;
					width:auto !important;
					min-width:224px !important;
				}
				#cmx-positionen-table thead th:nth-child(3),
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td:nth-child(3),
				#cmx-positionen-table thead th:nth-child(4),
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td:nth-child(4){
					flex:0 0 94px;
					width:auto !important;
					min-width:94px !important;
					transform:none;
				}
				#cmx-positionen-table thead th:nth-child(5),
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-total{
				flex:0 1 104px;
				width:auto !important;
				min-width:92px !important;
				}
				#cmx-positionen-table thead th:nth-child(6),
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-beschr-cell{
					flex:2 1 260px;
					width:auto !important;
					min-width:220px !important;
				}
				#cmx-positionen-table thead th:nth-child(7),
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-controls{
					flex:0 0 64px;
			...
			@media (min-width:1701px){
				#post-body.columns-2 #cmx-positionen-table thead th:nth-child(7),
				#post-body.columns-2 #cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-controls{
					flex-basis:84px;
			...
			@media (max-width:1280px) and (min-width:783px){
				...
				#cmx-positionen-table .cmx-pos-beschr-cell textarea{
					width:100% !important;
			...
			#cmx-positionen-table tr.cmx-pos-row-abschnitt{
				display:block;
				width:100%;
			}
			#cmx-positionen-table tr.cmx-pos-row-abschnitt > td.cmx-pos-abschnitt-cell{
				display:block;
				width:100% !important;
				min-width:0 !important;
			}
			@media (min-width:1281px) and (max-width:1700px){
				#post-body.columns-2 #cmx-positionen-table thead{
					display:none;
				}
				#post-body.columns-2 #cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt){
					display:grid;
					grid-template-columns:224px 90px 116px 84px 84px minmax(92px, 1fr) 56px;
					align-items:center;
					column-gap:8px;
					row-gap:10px;
					position:relative;
				...
				#post-body.columns-2 #cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td{
					border:0;
					padding:0 !important;
					width:auto !important;
					min-width:0 !important;
				}
				#post-body.columns-2 #cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td:nth-child(1){grid-column:1;grid-row:1}
				#post-body.columns-2 #cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-qty-cell{grid-column:2 / 4;grid-row:1}
				#post-body.columns-2 #cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td:nth-child(3){grid-column:4;grid-row:1}
				#post-body.columns-2 #cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td:nth-child(4){grid-column:5;grid-row:1}
				#post-body.columns-2 #cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-total{grid-column:6;grid-row:1}
				#post-body.columns-2 #cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-beschr-cell{
					grid-column:1 / 6;
					grid-row:2;
					max-width:none;
					padding-left:0 !important;
				}
				#post-body.columns-2 #cmx-positionen-table .cmx-artikel-edit{
					left:-18px;
				}
				#post-body.columns-2 #cmx-positionen-table .cmx-pos-row td:first-child .cmx-artikel-autocomplete{
					margin-left:0;
					width:100%;
					max-width:100%;
					min-width:0;
				}
				#post-body.columns-2 #cmx-positionen-table .cmx-pos-beschr-cell textarea{
					display:block;
					width:100% !important;
				...
				#post-body.columns-2 #cmx-positionen-table td.cmx-pos-controls{
					grid-column:6 / 8;
					grid-row:2;
					display:flex !important;
					align-items:center;
			...
			@media (max-width:1280px){
				...
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt){
					display:grid;
					grid-template-columns:208px 90px 116px 84px 84px minmax(92px, 1fr) 56px;
					align-items:center;
					column-gap:8px;
					row-gap:8px;
					position:relative;
				...
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-qty-cell{grid-column:2 / 4;grid-row:1}
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td:nth-child(3){grid-column:4;grid-row:1}
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td:nth-child(4){grid-column:5;grid-row:1}
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-total{grid-column:6;grid-row:1}
				#cmx-positionen-table tbody tr.cmx-pos-row:not(.cmx-pos-row-abschnitt) > td.cmx-pos-beschr-cell{
					grid-column:1 / 6;
					grid-row:2;
					max-width:none;
				...
				#cmx-positionen-table .cmx-artikel-edit{
					left:-18px;
				}
				.cmx-pos-row td:first-child .cmx-artikel-autocomplete{
					margin-left:0;
			...
			@media (max-width:782px){
				...
				.cmx-pos-actions > .button,
				.cmx-pos-actions .cmx-task-picker-wrap,
				.cmx-pos-actions .cmx-task-picker-wrap > .button{
					width:100%;
					text-align:center;
				}
			}
...
add_action('wp_ajax_cmx_save_beleg_positionen_order', function() {
	...
	foreach ($rows as $r) {
		...
		if ($custom === false) {
			...
			$infrastruktur_id = isset($r['infrastruktur_id']) ? (int) $r['infrastruktur_id'] : 0;
			if ($infrastruktur_id > 0 && \get_post_type($infrastruktur_id) !== 'infrastruktur') {
				$infrastruktur_id = 0;
			}
