...
function cmx_camt_normalize_beleg_payment_status(string $status): string {
	$status = \str_replace(['ä', 'ö', 'ü', 'Ä', 'Ö', 'Ü'], ['ae', 'oe', 'ue', 'Ae', 'Oe', 'Ue'], $status);
	$status = \sanitize_key($status);
	if ($status === 'uberbezahlt') {
		$status = 'ueberbezahlt';
	}
	if ($status === 'ungeklart') {
		$status = 'ungeklaert';
	}
	return \in_array($status, ['offen', 'teilbezahlt', 'bezahlt', 'ueberbezahlt', 'ungeklaert'], true) ? $status : '';
}

function cmx_camt_beleg_payment_status_label(string $status): string {
	$status = cmx_camt_normalize_beleg_payment_status($status);
	if ($status === 'ueberbezahlt') {
		return 'überbezahlt';
	}
	if ($status === 'ungeklaert') {
		return 'ungeklärt';
	}
	return $status;
}
...
function cmx_camt_assignments_get(): array {
	...
	foreach ($raw as $signature => $row) {
		...
		$allocations = [];
		foreach ((array) ($row['allocations'] ?? []) as $allocation) {
			if (!\is_array($allocation)) {
				continue;
			}
			$beleg_id = (int) ($allocation['beleg_id'] ?? 0);
			if ($beleg_id <= 0 || (string) \get_post_type($beleg_id) !== 'belege' || !\get_post_status($beleg_id)) {
				continue;
			}
			$allocation_amount = cmx_camt_normalize_amount((string) ($allocation['amount'] ?? ''));
			$allocations[] = [
				'beleg_id'           => $beleg_id,
				'assigned_at'        => (string) ($allocation['assigned_at'] ?? ''),
				'booking_date'       => (string) ($allocation['booking_date'] ?? ''),
				'amount'             => $allocation_amount,
				'direction'          => (string) ($allocation['direction'] ?? ''),
				'counterparty'       => (string) ($allocation['counterparty'] ?? ''),
				'reference'          => (string) ($allocation['reference'] ?? ''),
				'previous_status'    => cmx_camt_normalize_beleg_payment_status((string) ($allocation['previous_status'] ?? '')),
				'previous_paid_date' => cmx_camt_normalize_date((string) ($allocation['previous_paid_date'] ?? '')),
				'payment_mode'       => \sanitize_key((string) ($allocation['payment_mode'] ?? '')),
			];
		}
		...
		if (empty($allocations)) {
			...
			$allocations[] = [
				...
				'amount'             => cmx_camt_normalize_amount((string) ($row['amount'] ?? '')),
				'direction'          => (string) ($row['direction'] ?? ''),
				'counterparty'       => (string) ($row['counterparty'] ?? ''),
				'reference'          => (string) ($row['reference'] ?? ''),
				'previous_status'    => cmx_camt_normalize_beleg_payment_status((string) ($row['previous_status'] ?? '')),
		...
		$first = (array) $allocations[0];
		$clean[$signature] = [
			'beleg_id'           => (int) ($first['beleg_id'] ?? 0),
			'assigned_at'        => (string) ($row['assigned_at'] ?? ($first['assigned_at'] ?? '')),
			'booking_date'       => (string) ($row['booking_date'] ?? ($first['booking_date'] ?? '')),
			'amount'             => (string) ($row['amount'] ?? ''),
			'direction'          => (string) ($row['direction'] ?? ($first['direction'] ?? '')),
			'counterparty'       => (string) ($row['counterparty'] ?? ($first['counterparty'] ?? '')),
			'reference'          => (string) ($row['reference'] ?? ($first['reference'] ?? '')),
			'previous_status'    => cmx_camt_normalize_beleg_payment_status((string) ($row['previous_status'] ?? ($first['previous_status'] ?? ''))),
			'previous_paid_date' => cmx_camt_normalize_date((string) ($row['previous_paid_date'] ?? '')),
			'payment_mode'       => \sanitize_key((string) ($row['payment_mode'] ?? ($first['payment_mode'] ?? ''))),
...
function cmx_camt_assignment_allocations(array $assignment): array {
	$allocations = [];
	foreach ((array) ($assignment['allocations'] ?? []) as $allocation) {
		if (!\is_array($allocation)) {
			continue;
		}
		$beleg_id = (int) ($allocation['beleg_id'] ?? 0);
	...
	if (empty($allocations) && (int) ($assignment['beleg_id'] ?? 0) > 0) {
...
function cmx_camt_assignment_total_amount(string $signature): float {
	$total = 0.0;
	foreach (cmx_camt_assignment_allocations(cmx_camt_assignment_get($signature)) as $allocation) {
		$amount = cmx_camt_normalize_amount((string) ($allocation['amount'] ?? ''));
		if ($amount !== '') {
			$total += (float) $amount;
		}
	}
...
function cmx_camt_assignment_remaining_amount(string $signature, array $entry): float {
	$entry_amount = (float) cmx_camt_normalize_amount((string) ($entry['amount'] ?? ''));
	$remaining = (float) \round($entry_amount - cmx_camt_assignment_total_amount($signature), 2);
	return $remaining > 0.0 ? $remaining : 0.0;
}

function cmx_camt_assignment_has_beleg(string $signature, int $beleg_id): bool {
	if ($beleg_id <= 0) {
		return false;
	}
	foreach (cmx_camt_assignment_allocations(cmx_camt_assignment_get($signature)) as $allocation) {
		if ((int) ($allocation['beleg_id'] ?? 0) === $beleg_id) {
			return true;
		}
	}
	return false;
...
function cmx_camt_beleg_is_assigned(int $beleg_id, string $except_signature = ''): bool {
	...
	foreach (cmx_camt_assignments_get() as $signature => $row) {
		...
		foreach (cmx_camt_assignment_allocations((array) $row) as $allocation) {
			if ((int) ($allocation['beleg_id'] ?? 0) === $beleg_id) {
				return true;
			}
		}
...
function cmx_camt_remove_assignment(string $signature): bool {
	...
	$beleg_ids = [];
	foreach (cmx_camt_assignment_allocations($current) as $allocation) {
		$beleg_id = (int) ($allocation['beleg_id'] ?? 0);
		if ($beleg_id > 0) {
			$beleg_ids[] = $beleg_id;
		}
	...
	foreach (\array_values(\array_unique($beleg_ids)) as $beleg_id) {
...
function cmx_camt_append_beleg_signature(int $beleg_id, string $signature, array $entry, float $assigned_amount = 0.0): void {
	...
	$matches[$signature] = [
		...
		'assigned_amount'  => \number_format($assigned_amount, 2, '.', ''),
...
function cmx_camt_store_assignment(string $signature, int $beleg_id, array $entry, float $assigned_amount = 0.0, array $effect = []): bool {
	$signature = \sanitize_key($signature);
	...
	$assignments = cmx_camt_assignments_get();
	$current = \is_array($assignments[$signature] ?? null) ? (array) $assignments[$signature] : [];
	if (cmx_camt_assignment_has_beleg($signature, $beleg_id)) {
		return false;
	}
	...
	$allocation = [
		...
		'counterparty'       => (string) ($entry['counterparty_name'] ?? ''),
		'reference'          => (string) ($entry['reference'] ?? ''),
		'previous_status'    => cmx_camt_normalize_beleg_payment_status((string) ($effect['previous_status'] ?? '')),
		'previous_paid_date' => cmx_camt_normalize_date((string) ($effect['previous_paid_date'] ?? '')),
		'payment_mode'       => \sanitize_key((string) ($effect['payment_mode'] ?? '')),
	];
	$allocations = cmx_camt_assignment_allocations($current);
	$allocations[] = $allocation;
	$first = (array) $allocations[0];
	$assignments[$signature] = [
		'beleg_id'           => (int) ($first['beleg_id'] ?? $beleg_id),
		'assigned_at'        => (string) ($current['assigned_at'] ?? ($first['assigned_at'] ?? \wp_date('c'))),
		'booking_date'       => (string) ($entry['booking_date'] ?? ''),
		'amount'             => (string) ($entry['amount'] ?? ''),
		'direction'          => (string) ($entry['direction'] ?? ''),
		'counterparty'       => (string) ($entry['counterparty_name'] ?? ''),
		'reference'          => (string) ($entry['reference'] ?? ''),
		'previous_status'    => (string) ($first['previous_status'] ?? ''),
		'previous_paid_date' => (string) ($first['previous_paid_date'] ?? ''),
		'payment_mode'       => (string) ($first['payment_mode'] ?? ''),
		'allocations'        => $allocations,
	];
	\update_option((string) \constant(__NAMESPACE__ . '\\CMX_CAMT_ASSIGNMENTS_OPTION'), $assignments, false);
	cmx_camt_append_beleg_signature($beleg_id, $signature, $entry, $assigned_amount);
	return true;
}

function cmx_camt_assign_entry_to_beleg(string $signature, int $beleg_id, array $entry, string $mode = 'manual', float $assigned_amount = 0.0): array {
	$signature = \sanitize_key($signature);
	...
	if (cmx_camt_assignment_has_beleg($signature, $beleg_id)) {
		return [
			'ok'      => false,
			'status'  => 409,
			'message' => 'Diese Buchung ist diesem Beleg bereits zugeordnet.',
		];
	}
	$available_amount = cmx_camt_assignment_remaining_amount($signature, $entry);
	$open_amount = cmx_camt_beleg_open_amount($beleg_id);
	if ($assigned_amount <= 0.0) {
		$assigned_amount = (float) \round(\min($available_amount, $open_amount), 2);
	}
	$assigned_amount = (float) \round($assigned_amount, 2);
	if ($assigned_amount <= 0.0) {
		return [
			'ok'      => false,
			'status'  => 400,
			'message' => 'Bitte einen Zuordnungsbetrag eingeben.',
		];
	}
	if ($assigned_amount > ($available_amount + 0.009)) {
		return [
			'ok'      => false,
			'status'  => 400,
			'message' => 'Der zugeordnete Betrag ist grösser als der verfügbare Zahlungsbetrag.',
		];
	}
	if ($assigned_amount > ($open_amount + 0.009)) {
		return [
			'ok'      => false,
			'status'  => 400,
			'message' => 'Der zugeordnete Betrag ist grösser als der offene Belegbetrag.',
		];
	}

	$assignment_effect = cmx_camt_apply_assignment_to_beleg($beleg_id, $signature, $entry, $assigned_amount);
	if (!cmx_camt_store_assignment($signature, $beleg_id, $entry, $assigned_amount, $assignment_effect)) {
		cmx_camt_revert_assignment_on_beleg($signature, [
			'beleg_id'           => $beleg_id,
			'previous_status'    => (string) ($assignment_effect['previous_status'] ?? 'offen'),
			'previous_paid_date' => (string) ($assignment_effect['previous_paid_date'] ?? ''),
		]);
		return [
			'ok'      => false,
			'status'  => 409,
...
function cmx_camt_beleg_currency(int $beleg_id, string $fallback = 'CHF'): string {
	$meta_key = \defined(__NAMESPACE__ . '\\CMX_BELEG_META_WAEHRUNG')
		? (string) \constant(__NAMESPACE__ . '\\CMX_BELEG_META_WAEHRUNG')
		: '_cmx_beleg_waehrung';
	$currency = \strtoupper(\trim((string) \get_post_meta($beleg_id, $meta_key, true)));
	if (\preg_match('/^[A-Z]{3}$/', $currency)) {
		return $currency;
	}

	$fallback = \strtoupper(\trim($fallback));
	return \preg_match('/^[A-Z]{3}$/', $fallback) ? $fallback : 'CHF';
}
...
function cmx_camt_beleg_open_amount(int $beleg_id): float {
	...
	$status = cmx_camt_normalize_beleg_payment_status((string) \get_post_meta($beleg_id, cmx_camt_beleg_status_meta_key(), true));
	if ($paid <= 0.009 && $status === 'bezahlt') {
		return 0.0;
	}
...
function cmx_camt_apply_assignment_to_beleg(int $beleg_id, string $signature, array $entry, float $assigned_amount = 0.0): array {
	if ($beleg_id <= 0 || (string) \get_post_type($beleg_id) !== 'belege') {
		return [];
	}
	...
	$previous_status = cmx_camt_normalize_beleg_payment_status((string) \get_post_meta($beleg_id, $status_key, true));
	...
	$assigned_amount = (float) \round($assigned_amount, 2);
	if ($assigned_amount <= 0.0) {
		$assigned_amount = (float) cmx_camt_normalize_amount((string) ($entry['amount'] ?? ''));
	}
	$assigned_amount = (float) \round($assigned_amount, 2);
	if ($assigned_amount <= 0.0) {
		return [];
	...
	$open_amount_before = $total_amount > 0.0
		...
		: 0.0;
	$needs_partial_row = $partial_sum_before > 0.009 || $assigned_amount < ($open_amount_before - 0.009);
	if ($needs_partial_row) {
		cmx_camt_beleg_add_partial_payment($beleg_id, $signature, $booking_date, $assigned_amount);
	}

	$paid_sum_after = $needs_partial_row
		? cmx_camt_beleg_partial_sum($beleg_id)
		: $assigned_amount;
	$new_status = 'ungeklaert';
	$payment_mode = 'unclear';
	if ($total_amount > 0.0 && $paid_sum_after < ($total_amount - 0.009)) {
		$new_status = 'teilbezahlt';
		$payment_mode = 'partial';
	} elseif ($total_amount > 0.0 && $paid_sum_after > ($total_amount + 0.009)) {
		$new_status = 'ueberbezahlt';
		$payment_mode = 'overpaid';
	} elseif ($total_amount > 0.0) {
		$new_status = 'bezahlt';
		$payment_mode = 'paid';
	}

	\update_post_meta($beleg_id, $status_key, $new_status);
	if ($new_status === 'teilbezahlt' || $new_status === 'ungeklaert') {
...
function cmx_camt_beleg_can_auto_assign(int $beleg_id, array $entry): bool {
	...
	$open_amount = cmx_camt_beleg_open_amount($beleg_id);
	$entry_amount = (float) cmx_camt_normalize_amount((string) ($entry['amount'] ?? ''));
	if ($open_amount <= 0.009 || $entry_amount <= 0.0 || $entry_amount > ($open_amount + 0.009)) {
		return false;
	}
...
function cmx_camt_revert_assignment_on_beleg(string $signature, array $assignment): void {
	$allocations = cmx_camt_assignment_allocations($assignment);
	if (empty($allocations)) {
		return;
	}
	...
	foreach ($allocations as $allocation) {
		...
		$previous_paid_date = cmx_camt_normalize_date((string) ($allocation['previous_paid_date'] ?? ''));
		if (!\in_array($previous_status, ['bezahlt', 'teilbezahlt', 'ueberbezahlt'], true)) {
			$previous_paid_date = '';
		}
...
function cmx_camt_contact_post_type(): string {
...
function cmx_camt_beleg_contact_id(int $beleg_id): int {
	if ($beleg_id <= 0) {
		return 0;
	}
	if (\function_exists(__NAMESPACE__ . '\\cmx_beleg_kontakt_id_for_post')) {
		$contact_id = (int) cmx_beleg_kontakt_id_for_post($beleg_id);
		if ($contact_id > 0) {
			return $contact_id;
		}
	}
	$meta_keys = ['_cmx_beleg_kontakt_id', '_cmx_beleg_kontakt', 'cmx_beleg_kontakt_id'];
	...
	foreach (\array_values(\array_unique($meta_keys)) as $meta_key) {
		$contact_id = (int) \get_post_meta($beleg_id, $meta_key, true);
...
function cmx_camt_contact_search_terms(string $query): array {
	$query = cmx_camt_clean_ws($query);
	if ($query === '') {
		return [];
	}
	$terms = [$query];
...
function cmx_camt_contact_search_meta_keys(): array {
	$keys = [];
	if (\function_exists(__NAMESPACE__ . '\\cmx_kontakte_search_name_meta_keys')) {
		$keys = \array_merge($keys, (array) cmx_kontakte_search_name_meta_keys());
	} else {
	...
	if (\function_exists(__NAMESPACE__ . '\\cmx_kontakte_search_email_meta_keys')) {
		$keys = \array_merge($keys, (array) cmx_kontakte_search_email_meta_keys());
	} else {
		$keys = \array_merge($keys, [
		...
		if (\function_exists(__NAMESPACE__ . '\\cmx_kommunikation_flat_field_meta_keys')) {
			$keys = \array_merge($keys, (array) cmx_kommunikation_flat_field_meta_keys('email', 10));
		}
	}
...
function cmx_camt_contact_search_values(int $contact_id): array {
	$values = [(string) \get_the_title($contact_id)];
	$name_keys = \function_exists(__NAMESPACE__ . '\\cmx_kontakte_search_name_meta_keys')
		? (array) cmx_kontakte_search_name_meta_keys()
		: [
			\defined(__NAMESPACE__ . '\\CMX_KONTAKTE_META_VORNAME') ? (string) \constant(__NAMESPACE__ . '\\CMX_KONTAKTE_META_VORNAME') : '_cmx_kontakte_vorname',
			\defined(__NAMESPACE__ . '\\CMX_KONTAKTE_META_NACHNAME') ? (string) \constant(__NAMESPACE__ . '\\CMX_KONTAKTE_META_NACHNAME') : '_cmx_kontakte_nachname',
		];
	$email_keys = \function_exists(__NAMESPACE__ . '\\cmx_kontakte_search_email_meta_keys')
		...
		: ['_cmx_email_1', '_cmx_email_2', '_cmx_email_3', 'email_1', 'email_2', 'email_3', 'kontakt_email', 'email', 'e_mail', 'mail'];

	foreach (\array_merge($name_keys, $email_keys) as $meta_key) {
		$value = \get_post_meta($contact_id, (string) $meta_key, true);
		if (\is_scalar($value)) {
			$values[] = (string) $value;
		}
	}
	if (\function_exists(__NAMESPACE__ . '\\cmx_kommunikation_primary_email')) {
		$values[] = (string) cmx_kommunikation_primary_email($contact_id);
	}
...
function cmx_camt_contact_search_score(int $contact_id, string $query, array $terms): int {
	$query_norm = cmx_camt_normalize_name($query);
	if ($contact_id <= 0 || $query_norm === '') {
		return 0;
	}
	$values = cmx_camt_contact_search_values($contact_id);
	$haystack = cmx_camt_normalize_name(\implode(' ', $values));
	if ($haystack === '') {
		return 0;
	}

	$score = 0;
	$title_norm = cmx_camt_normalize_name((string) \get_the_title($contact_id));
	if ($title_norm === $query_norm) {
		$score += 400;
	} elseif ($title_norm !== '' && \str_contains($title_norm, $query_norm)) {
		$score += 260;
	}
	if (\str_contains($haystack, $query_norm)) {
		$score += 180;
	}
	foreach ($terms as $term) {
		$term_norm = cmx_camt_normalize_name((string) $term);
		if ($term_norm !== '' && \str_contains($haystack, $term_norm)) {
			$score += 40;
		}
	}
...
function cmx_camt_contact_search_results(string $query): array {
	$query = cmx_camt_clean_ws($query);
	if ($query === '') {
		return [];
	}
	$terms = cmx_camt_contact_search_terms($query);
	$post_type = cmx_camt_contact_post_type();
	...
	$posts = \array_values(\array_filter($posts, static function (int $contact_id) use ($query, $terms): bool {
		return cmx_camt_contact_search_score($contact_id, $query, $terms) > 0;
	}));
	\usort($posts, static function (int $a, int $b) use ($query, $terms): int {
		$score_a = cmx_camt_contact_search_score($a, $query, $terms);
		$score_b = cmx_camt_contact_search_score($b, $query, $terms);
		if ($score_a !== $score_b) {
			return $score_b <=> $score_a;
		}
		return \strnatcasecmp((string) \get_the_title($a), (string) \get_the_title($b));
	});
	$posts = \array_slice($posts, 0, 12);

	$results = [];
	foreach ($posts as $contact_id) {
...
function cmx_camt_beleg_ids_by_contact(int $contact_id): array {
	if ($contact_id <= 0) {
		return [];
	}
	...
	foreach (\array_values(\array_unique(\array_filter($meta_keys))) as $meta_key) {
		$meta_query[] = [
			'key'     => $meta_key,
...
function cmx_camt_candidate_amount_filter_from_request($value): ?float {
	if ($value === null || \trim((string) $value) === '') {
		return null;
	}
	$normalized = cmx_camt_normalize_amount((string) $value);
	if ($normalized === '') {
		return null;
	}
...
function cmx_camt_beleg_ids_by_explicit_amount_range(?float $amount_min, ?float $amount_max): array {
	...
	foreach ((array) $ids as $beleg_id) {
		$beleg_id = (int) $beleg_id;
		$amount = cmx_camt_beleg_open_amount($beleg_id);
		if ($beleg_id <= 0 || $amount <= 0.0) {
			continue;
		}
		if ($amount_min !== null && $amount < ($amount_min - 0.009)) {
			continue;
		}
		if ($amount_max !== null && $amount > ($amount_max + 0.009)) {
			continue;
		}
		$matches[] = $beleg_id;
	}
...
function cmx_camt_beleg_score_candidate(int $beleg_id, array $entry, float $amount_window = 0.0, int $contact_id = 0): array {
	...
	$status = cmx_camt_normalize_beleg_payment_status((string) \get_post_meta($beleg_id, \defined(__NAMESPACE__ . '\\CMX_BELEG_META_STATUS') ? CMX_BELEG_META_STATUS : '_cmx_beleg_status', true));
	...
	if ($contact_id > 0) {
		if (cmx_camt_beleg_contact_id($beleg_id) === $contact_id) {
			$score += 260;
		} else {
			$score -= 120;
		}
	...
	foreach (cmx_camt_entry_reference_candidates($entry) as $reference_candidate) {
		if (cmx_camt_beleg_title_matches_reference($beleg_id, (string) $reference_candidate)) {
			$reference_title_match = true;
			$score += 360;
			break;
		}
	}
	...
	if (
		...
		&& $entry_amount > 0.0
			&& $amount > ($entry_amount + 0.009)
			&& \in_array($status, ['', 'offen', 'teilbezahlt'], true)
			&& !($doc_ref_title_match || $reference_title_match || $doc_ref_contact_match || $counterparty_match)
	) {
	...
	if ($status === 'offen' || $status === '') {
		$score += 20;
	} elseif ($status === 'teilbezahlt') {
		$score += 35;
	} elseif ($status === 'bezahlt' || $status === 'ueberbezahlt') {
		$score -= 220;
	}
	if ($amount <= 0.009) {
		$score -= 400;
	...
	return [
		...
		'contact'     => $contact,
		'amount'      => $amount,
		'currency'    => cmx_camt_beleg_currency($beleg_id, (string) ($entry['currency'] ?? 'CHF')),
		'direction'   => $direction,
		'paid_date'   => $paid_date,
...
function cmx_camt_collect_beleg_candidates(
	array $entry,
	float $amount_window = 0.0,
	int $contact_id = 0,
	?float $amount_min = null,
	?float $amount_max = null
): array {
	$ids = [];
	if ($contact_id > 0) {
		$ids = \array_merge($ids, cmx_camt_beleg_ids_by_contact($contact_id));
	}
	$doc_ref = \trim((string) ($entry['document_ref'] ?? ''));
	if ($doc_ref !== '') {
	...
	$ids = \array_merge($ids, cmx_camt_beleg_ids_by_explicit_amount_range($amount_min, $amount_max));
	$ids = \array_values(\array_unique(\array_filter(\array_map('intval', $ids))));

	$candidates = [];
	foreach ($ids as $beleg_id) {
		if ($contact_id > 0 && cmx_camt_beleg_contact_id($beleg_id) !== $contact_id) {
			continue;
		}
		$open_amount = cmx_camt_beleg_open_amount($beleg_id);
		if ($amount_min !== null && $open_amount < ($amount_min - 0.009)) {
			continue;
		}
		if ($amount_max !== null && $open_amount > ($amount_max + 0.009)) {
			continue;
		}
		$candidate = cmx_camt_beleg_score_candidate($beleg_id, $entry, $amount_window, $contact_id);
...
function cmx_camt_create_new_beleg_from_entry(array $entry, int $selected_contact_id = 0, string $selected_direction = ''): array {
	$inserted = \wp_insert_post([
		'post_type'   => 'belege',
		'post_status' => 'draft',
		'post_title'  => '',
	...
	$selected_direction = \sanitize_key($selected_direction);
	$direction = \in_array($selected_direction, ['ausgang', 'eingang'], true)
		? $selected_direction
		: (string) ($entry['beleg_direction'] ?? cmx_camt_beleg_direction_from_entry((string) ($entry['direction'] ?? '')));
	...
	if (
		...
		&& (string) \get_post_type($selected_contact_id) === cmx_camt_contact_post_type()
		&& (string) \get_post_status($selected_contact_id) !== ''
	) {
		$kontakt_id = $selected_contact_id;
	} elseif ($counterparty !== '' && $counterparty_norm !== 'NOTPROVIDED') {
...
function cmx_camt_row_status_html(array $entry): string {
	...
	$beleg_id = (int) ($allocations[0]['beleg_id'] ?? 0);
	$url = (string) \get_edit_post_link($beleg_id, 'raw');
	$title = \trim((string) \get_the_title($beleg_id));
	$title = $title !== '' ? $title : ('Beleg #' . $beleg_id);
	if (\count($allocations) > 1) {
		return '<span class="cmx-camt-status cmx-camt-status--assigned">' . \esc_html((string) \count($allocations)) . ' Belege</span>';
	}
...
function cmx_camt_row_status_mode(array $entry): string {
	$signature = (string) ($entry['signature'] ?? '');
	$assignment = cmx_camt_assignment_get($signature);
	if (!empty(cmx_camt_assignment_allocations($assignment))) {
		return 'assigned';
	}
...
function cmx_camt_render_candidates_panel(
	array $entry,
	float $amount_window = 0.0,
	int $contact_id = 0,
	?float $amount_min = null,
	...
	$amount_min = cmx_camt_candidate_amount_filter_from_request($amount_min);
	$amount_max = cmx_camt_candidate_amount_filter_from_request($amount_max);
	$candidates = cmx_camt_collect_beleg_candidates($entry, $amount_window, $contact_id, $amount_min, $amount_max);
	$entry_amount = (float) cmx_camt_normalize_amount((string) ($entry['amount'] ?? ''));
	$assigned_total = cmx_camt_assignment_total_amount($signature);
	$remaining_amount = cmx_camt_assignment_remaining_amount($signature, $entry);
	...
	$selected_contact_title = $contact_id > 0 ? \trim((string) \get_the_title($contact_id)) : '';
	$default_beleg_direction = (string) ($entry['beleg_direction'] ?? cmx_camt_beleg_direction_from_entry((string) ($entry['direction'] ?? '')));
	$income_button_class = $default_beleg_direction === 'ausgang' ? 'button-primary is-selected' : 'button-secondary';
	$expense_button_class = $default_beleg_direction === 'eingang' ? 'button-primary is-selected' : 'button-secondary';
	...
	echo '</div>';
	echo '<div class="cmx-camt-create-wrap">';
	echo '<button type="button" class="button button-primary cmx-camt-create-trigger">Neuen Beleg erstellen</button>';
	echo '<div class="cmx-camt-create-choices" hidden>';
	echo '<span>Belegart:</span>';
	echo '<button type="button" class="button ' . \esc_attr($income_button_class) . ' cmx-camt-create-beleg" data-signature="' . \esc_attr($signature) . '" data-beleg-direction="ausgang" aria-pressed="' . \esc_attr($default_beleg_direction === 'ausgang' ? 'true' : 'false') . '">Einnahme</button>';
	echo '<button type="button" class="button ' . \esc_attr($expense_button_class) . ' cmx-camt-create-beleg" data-signature="' . \esc_attr($signature) . '" data-beleg-direction="eingang" aria-pressed="' . \esc_attr($default_beleg_direction === 'eingang' ? 'true' : 'false') . '">Ausgabe</button>';
	echo '<button type="button" class="button-link cmx-camt-create-cancel">Abbrechen</button>';
	echo '</div>';
	echo '</div>';
	echo '</div>';
	...
	if (!empty($allocations)) {
		...
		echo '<div>';
		echo '<p>Diese Buchung ist bereits zugeordnet:</p>';
		echo '<ul class="cmx-camt-assignment-list">';
		foreach ($allocations as $allocation) {
			$assigned_id = (int) ($allocation['beleg_id'] ?? 0);
			if ($assigned_id <= 0) {
				continue;
			}
			$assigned_title = \trim((string) \get_the_title($assigned_id));
			$assigned_url = (string) \get_edit_post_link($assigned_id, 'raw');
			$allocation_amount = (string) ($allocation['amount'] ?? '');
			echo '<li><a href="' . \esc_url($assigned_url) . '" target="_blank" rel="noopener noreferrer">' . \esc_html($assigned_title !== '' ? $assigned_title : ('Beleg #' . $assigned_id)) . '</a> <span>' . \esc_html(cmx_camt_format_amount($allocation_amount)) . ' CHF</span></li>';
		}
		echo '</ul>';
		echo '<p><strong>Zugeordnet:</strong> ' . \esc_html(cmx_camt_format_amount((string) $assigned_total)) . ' CHF &nbsp; <strong>Verfügbar:</strong> ' . \esc_html(cmx_camt_format_amount((string) $remaining_amount)) . ' CHF</p>';
		echo '</div>';
	...
	echo '<div class="cmx-camt-contact-box" data-signature="' . \esc_attr($signature) . '" data-contact-id="' . \esc_attr((string) $contact_id) . '" data-amount-min="' . \esc_attr($amount_min !== null ? (string) $amount_min : '') . '" data-amount-max="' . \esc_attr($amount_max !== null ? (string) $amount_max : '') . '">';
	echo '<div class="cmx-camt-contact-row">';
	echo '<div class="cmx-camt-filter-field cmx-camt-contact-filter">';
	echo '<label for="cmx-camt-contact-search-' . \esc_attr($signature) . '">Kontakt suchen</label>';
	echo '<input type="search" id="cmx-camt-contact-search-' . \esc_attr($signature) . '" class="regular-text cmx-camt-contact-input" placeholder="Kontakt suchen..." autocomplete="off" data-signature="' . \esc_attr($signature) . '" role="combobox" aria-autocomplete="list" aria-expanded="false" aria-controls="cmx-camt-contact-results-' . \esc_attr($signature) . '">';
	echo '</div>';
	echo '<div class="cmx-camt-amount-filter">';
	echo '<div class="cmx-camt-filter-field">';
	echo '<label for="cmx-camt-amount-min-' . \esc_attr($signature) . '">Betrag von</label>';
	echo '<input type="text" id="cmx-camt-amount-min-' . \esc_attr($signature) . '" class="small-text cmx-camt-amount-min" inputmode="decimal" value="' . \esc_attr($amount_min !== null ? cmx_camt_format_amount($amount_min) : '') . '">';
	echo '</div>';
	echo '<div class="cmx-camt-filter-field">';
	echo '<label for="cmx-camt-amount-max-' . \esc_attr($signature) . '">Betrag bis</label>';
	echo '<input type="text" id="cmx-camt-amount-max-' . \esc_attr($signature) . '" class="small-text cmx-camt-amount-max" inputmode="decimal" value="' . \esc_attr($amount_max !== null ? cmx_camt_format_amount($amount_max) : '') . '">';
	echo '</div>';
	echo '</div>';
	echo '</div>';
	if ($selected_contact_title !== '') {
		echo '<p class="cmx-camt-contact-selected">Ausgewählt: <span class="cmx-camt-contact-chip"><strong>' . \esc_html($selected_contact_title) . '</strong><button type="button" class="cmx-camt-contact-clear" data-signature="' . \esc_attr($signature) . '" aria-label="Kontakt-Auswahl löschen" title="Kontakt-Auswahl löschen">×</button></span></p>';
	}
	echo '<ul id="cmx-camt-contact-results-' . \esc_attr($signature) . '" class="cmx-camt-contact-results" role="listbox" aria-live="polite" style="display:none;"></ul>';
	echo '</div>';

	echo '<div class="cmx-camt-candidates-head" data-signature="' . \esc_attr($signature) . '" data-amount-window="' . \esc_attr((string) $amount_window) . '" data-contact-id="' . \esc_attr((string) $contact_id) . '" data-amount-min="' . \esc_attr($amount_min !== null ? (string) $amount_min : '') . '" data-amount-max="' . \esc_attr($amount_max !== null ? (string) $amount_max : '') . '">';
	echo '<h3 style="margin:0;">Mögliche Belege <span style="font-weight:400;color:#646970;">Zahlungsbetrag ' . \esc_html(cmx_camt_format_amount((string) $entry_amount)) . ' CHF';
	if ($assigned_total > 0.009) {
		echo ', noch zuzuordnen ' . \esc_html(cmx_camt_format_amount((string) $remaining_amount)) . ' CHF';
	}
	echo '</span></h3>';
	echo '</div>';
	if (empty($candidates)) {
	...
	foreach ($candidates as $candidate) {
		...
		$candidate_contact_id = cmx_camt_beleg_contact_id($beleg_id);
		$candidate_contact_url = $candidate_contact_id > 0 ? (string) \get_edit_post_link($candidate_contact_id, 'raw') : '';
		$category = \trim((string) ($candidate['category'] ?? ''));
		$dir_label = (string) ($candidate['direction'] ?? '') === 'ausgang' ? 'Einnahme' : 'Ausgabe';
		$amount_color_class = (string) ($candidate['direction'] ?? '') === 'ausgang'
			? 'cmx-camt-candidate-amount--income'
			: 'cmx-camt-candidate-amount--expense';
		$status = (string) ($candidate['status'] ?? '');
		$already_this_booking = cmx_camt_assignment_has_beleg($signature, $beleg_id);
		$open_amount = (float) ($candidate['amount'] ?? 0.0);
		$currency = (string) ($candidate['currency'] ?? 'CHF');
		$default_amount = (float) \round(\min($remaining_amount, $open_amount), 2);
		$assign_disabled = '';
		$assign_label = 'Zuordnen';
		$assign_label_html = \esc_html($assign_label);
		$assign_class = 'button button-secondary cmx-camt-assign-beleg';
		if ($already_this_booking) {
			$assign_label = 'Bereits zugeordnet';
			$assign_label_html = \esc_html($assign_label);
			$assign_disabled = ' disabled';
		} elseif ($remaining_amount <= 0.009 || $open_amount <= 0.009) {
			$assign_disabled = ' disabled';
		}

		echo '<div class="cmx-camt-candidate">';
		echo '<div class="cmx-camt-candidate-main">';
		echo '<div class="cmx-camt-candidate-title">';
		echo '<a href="' . \esc_url($edit_url) . '">' . \esc_html($title !== '' ? $title : ('Beleg #' . $beleg_id)) . '</a>';
		if ($contact !== '') {
			if ($candidate_contact_url !== '') {
				echo '<a class="cmx-camt-candidate-contact" href="' . \esc_url($candidate_contact_url) . '">' . \esc_html($contact) . '</a>';
			} else {
				echo '<span class="cmx-camt-candidate-contact">' . \esc_html($contact) . '</span>';
			}
		}
		echo '</div>';
		...
		echo '<span class="' . \esc_attr($amount_color_class) . '"><strong>' . \esc_html(cmx_camt_format_amount((string) ($candidate['amount'] ?? '0'))) . '</strong></span>';
		if ((string) ($candidate['invoice_date'] ?? '') !== '') {
			echo '<span>Belegdatum <strong>' . \esc_html(cmx_camt_format_date((string) $candidate['invoice_date'])) . '</strong></span>';
		}
		if ((string) ($candidate['paid_date'] ?? '') !== '') {
			echo '<span>Bezahlt ' . \esc_html(cmx_camt_format_date((string) $candidate['paid_date'])) . '</span>';
		}
		echo '</div>';
		echo '<div class="cmx-camt-candidate-meta cmx-camt-candidate-meta--status">';
		if ($status !== '') {
			echo '<span>Status ' . \esc_html(cmx_camt_beleg_payment_status_label($status)) . '</span>';
		}
		echo '<span>Score ' . \esc_html((string) $score) . '</span>';
		echo '</div>';
		echo '</div>';
		echo '<div class="cmx-camt-candidate-actions">';
		echo '<label><input type="text" inputmode="decimal" class="small-text cmx-camt-assign-amount" aria-label="Dieser Zahlung zuordnen" value="' . \esc_attr(cmx_camt_format_amount($default_amount)) . '"' . ($assign_disabled !== '' ? ' disabled' : '') . '> ' . \esc_html($currency) . '</label>';
...
function cmx_bank_import_render_log_page(): void {
	...
	$income_color = \function_exists(__NAMESPACE__ . '\\cmx_admin_global_color')
		? cmx_admin_global_color('gruen', '#5B8C2A')
		: '#5B8C2A';
	$expense_color = \function_exists(__NAMESPACE__ . '\\cmx_admin_global_color')
		? cmx_admin_global_color('rot', '#B42318')
		: '#B42318';
	$mouse_green = \function_exists(__NAMESPACE__ . '\\cmx_admin_global_color')
		? cmx_admin_global_color('mausgruen', '#8CE99A')
		: '#8CE99A';
	...
	.cmx-camt-wrap .button,
	.cmx-camt-wrap .button-secondary,
	.cmx-camt-wrap .button-primary{
		display:inline-flex;
		align-items:center;
	...
	.cmx-camt-wrap input[type="text"],
	.cmx-camt-wrap input[type="search"],
	.cmx-camt-wrap input[type="number"]{
		height:36px;
		min-height:36px;
		padding:4px 10px;
	...
	.cmx-camt-wrap input[type="text"]:focus,
	.cmx-camt-wrap input[type="search"]:focus,
	.cmx-camt-wrap input[type="number"]:focus{
		border-color:#2271b1;
	...
	.cmx-camt-entry-row.is-assigned.is-selected td,
	.cmx-camt-entry-row.is-booked.is-selected td{
		background:' . \esc_attr($mouse_green) . ' !important;
	}
	.cmx-camt-chip{
	...
	.cmx-camt-create-wrap{
		display:flex;
		justify-content:flex-end;
	}
	.cmx-camt-create-trigger[hidden]{
		display:none;
	}
	.cmx-camt-create-choices{
		display:flex;
		align-items:center;
		justify-content:flex-end;
		gap:8px;
	...
	.cmx-camt-create-choices[hidden]{
		display:none;
	}
	.cmx-camt-create-choices > span{
		font-weight:600;
	}
	.cmx-camt-entry-meta{
		display:grid;
	...
	.cmx-camt-assignment-list{
		margin:6px 0;
		padding-left:18px;
	}
	.cmx-camt-assignment-list li{margin:2px 0}
	...
	.cmx-camt-contact-box label{
		display:block;
		margin:0 0 6px;
		font-weight:600;
	}
	.cmx-camt-contact-row{
		display:flex;
		gap:8px;
		align-items:flex-end;
		flex-wrap:wrap;
	}
	.cmx-camt-filter-field{
		display:flex;
		flex-direction:column;
		align-items:flex-start;
		gap:6px;
	}
	.cmx-camt-contact-box .cmx-camt-filter-field label{
		display:block;
		margin:0;
		line-height:1.2;
	}
	.cmx-camt-amount-filter{
		display:flex;
		align-items:flex-end;
		gap:8px;
		margin-left:auto;
	...
	.cmx-camt-amount-filter .cmx-camt-filter-field{
		width:82.5px;
		flex:0 0 82.5px;
	}
	.cmx-camt-amount-filter input{
		width:82.5px;
		min-width:82.5px;
		max-width:82.5px;
		flex:none;
	}
	.cmx-camt-contact-results{
		position:absolute;
		z-index:100004;
		left:12px;
		top:78px;
	...
	.cmx-camt-contact-results li{
		margin:0;
		padding:8px 10px;
		cursor:pointer;
		color:#162033;
	}
	.cmx-camt-contact-results li.is-active,
	.cmx-camt-contact-results li:hover{background:#e5f3ff}
	.cmx-camt-contact-results li.is-empty{
		cursor:default;
		color:#646970;
	}
	.cmx-camt-contact-selected{margin:8px 0 0;color:#56687d}
	.cmx-camt-contact-chip{
		display:inline-flex;
		align-items:center;
		gap:6px;
		padding:2px 7px;
	...
	.cmx-camt-candidate-main{min-width:0}
	.cmx-camt-candidate-title{
		display:flex;
		align-items:center;
		gap:16px;
		flex-wrap:wrap;
	...
	.cmx-camt-candidate-title .cmx-camt-candidate-contact{
		font-weight:400;
	...
	.cmx-camt-candidate-amount--income{
		color:' . \esc_attr($income_color) . ';
	}
	.cmx-camt-candidate-amount--expense{
		color:' . \esc_attr($expense_color) . ';
	}
	.cmx-camt-candidate-meta--status{
		margin-top:8px;
	}
	.cmx-camt-candidate-actions{
		display:grid;
		grid-template-columns:1fr;
		gap:8px;
		justify-items:end;
	}
	.cmx-camt-candidate-actions label{
		display:flex;
		align-items:center;
		justify-content:flex-start;
		gap:6px;
	...
	.cmx-camt-candidate-actions .button{
		width:120px;
		min-width:120px;
		text-align:center;
	}
	.cmx-camt-assign-amount{
		width:82.5px;
		min-width:82.5px;
		max-width:82.5px;
		flex:0 0 82.5px;
	...
	@media (max-width: 782px){
		.cmx-camt-candidate{grid-template-columns:1fr}
		.cmx-camt-candidate-actions{justify-items:start}
	}
	</style>';
	...
	echo '<p class="cmx-camt-intro"><code>camt.053</code> und <code>camt.054</code> werden gemeinsam eingelesen und über die Buchungsmerkmale zusammengeführt,<br>damit dieselbe Buchung nicht doppelt zugeordnet wird.<br>Bei <strong>Revolut Business</strong> <code>XML &bull; camt.053 (V12)</code> auswählen.</p>';
	echo '</div>';
...
(function(){
	...
	function getPanelContactId(source){
		var panel = source ? source.closest(".cmx-camt-contact-box, .cmx-camt-candidates-head") : null;
		if (!panel && detail) {
			panel = detail.querySelector(".cmx-camt-candidates-head");
		}
		var raw = panel ? (panel.getAttribute("data-contact-id") || "0") : "0";
		var num = parseInt(raw, 10);
		return isNaN(num) ? 0 : num;
	}

	function getPanelAmountFilter(source, kind){
		var box = source ? source.closest(".cmx-camt-contact-box") : null;
		if (!box && detail) {
			box = detail.querySelector(".cmx-camt-contact-box");
		}
		var input = box ? box.querySelector(kind === "min" ? ".cmx-camt-amount-min" : ".cmx-camt-amount-max") : null;
		if (input) {
		...
		return panel ? String(panel.getAttribute(kind === "min" ? "data-amount-min" : "data-amount-max") || "").trim() : "";
	}

	function loadDetail(signature, amountWindow, contactId, amountMin, amountMax){
		...
		fd.append("contact_id", String(contactId || 0));
		fd.append("amount_min", String(amountMin || ""));
		fd.append("amount_max", String(amountMax || ""));
		fetch(ajaxUrl, {method:"POST", body:fd, credentials:"same-origin"})
			.then(function(r){ return r.json(); })
			.then(function(res){
				if (!res || !res.success) {
	...
	function contactResults(inputEl){
		var box = inputEl ? inputEl.closest(".cmx-camt-contact-box") : null;
		return box ? box.querySelector(".cmx-camt-contact-results") : null;
	}

	function setContactResultsOpen(inputEl, isOpen){
		var results = contactResults(inputEl);
		if (!results) return;
		results.style.display = isOpen ? "" : "none";
		if (inputEl) {
	...
	function setActiveContactResult(results, index, inputEl){
		if (!results) return;
		var items = Array.prototype.slice.call(results.querySelectorAll(".cmx-camt-contact-result"));
		if (!items.length) {
		...
		if (active) {
			...
			if (inputEl) inputEl.setAttribute("aria-activedescendant", active.id || "");
		}
	}

	function activeContactResult(results){
		return results ? results.querySelector(".cmx-camt-contact-result.is-active") : null;
	}

	function chooseContactResult(item){
		if (!item) return;
		var contactBox = item.closest(".cmx-camt-contact-box");
		var signature = contactBox ? (contactBox.getAttribute("data-signature") || "") : "";
		var contactId = parseInt(item.getAttribute("data-contact-id") || "0", 10);
		if (signature && contactId > 0) {
			loadDetail(
				signature,
				getPanelAmountWindow(item),
				contactId,
				getPanelAmountFilter(item, "min"),
				getPanelAmountFilter(item, "max")
			);
		}
	}

	function searchContacts(inputEl){
		var query = inputEl ? String(inputEl.value || "") : "";
		var results = contactResults(inputEl);
		if (!results) return;
		if (query.trim().length < 2) {
			results.innerHTML = "";
			setContactResultsOpen(inputEl, false);
			return;
		}
		var fd = new FormData();
		fd.append("action", "cmx_camt_search_contacts");
		fd.append("nonce", nonce);
		...
		fetch(ajaxUrl, {method:"POST", body:fd, credentials:"same-origin"})
			...
			.then(function(res){
				...
				var items = (res.data && res.data.results) ? res.data.results : [];
				if (!items.length) {
					results.innerHTML = '<li class="is-empty">Kein Kontakt gefunden.</li>';
					setContactResultsOpen(inputEl, true);
					return;
				}
				results.innerHTML = items.map(function(item){
					var id = 'cmx-camt-contact-result-' + escapeHtml(item.id);
					return '<li id="' + id + '" class="cmx-camt-contact-result" role="option" tabindex="-1" data-contact-id="' + escapeHtml(item.id) + '">' + escapeHtml(item.label) + '</li>';
				}).join("");
				setContactResultsOpen(inputEl, true);
				setActiveContactResult(results, 0, inputEl);
			})
			.catch(function(err){
				results.innerHTML = '<li class="is-empty" style="color:#b32d2e;">' + escapeHtml(err && err.message ? err.message : "Kontakte konnten nicht geladen werden.") + '</li>';
				setContactResultsOpen(inputEl, true);
			});
	}
	...
	document.addEventListener("click", function(e){
		...
		if (assignBtn) {
			...
			var contactId = getPanelContactId(assignBtn);
			var candidate = assignBtn.closest(".cmx-camt-candidate");
			var amountInput = candidate ? candidate.querySelector(".cmx-camt-assign-amount") : null;
			var assignAmount = amountInput ? String(amountInput.value || "") : "";
			...
			fd.append("amount_window", String(amountWindow));
			fd.append("contact_id", String(contactId || 0));
			fd.append("amount_min", getPanelAmountFilter(assignBtn, "min"));
			fd.append("amount_max", getPanelAmountFilter(assignBtn, "max"));
			fd.append("assign_amount", assignAmount);
			fetch(ajaxUrl, {method:"POST", body:fd, credentials:"same-origin"})
				.then(function(r){ return r.json(); })
				.then(function(res){
					if (!res || !res.success) {
		...
		var createTrigger = e.target.closest(".cmx-camt-create-trigger");
		if (createTrigger) {
			e.preventDefault();
			var createWrap = createTrigger.closest(".cmx-camt-create-wrap");
			var createChoices = createWrap ? createWrap.querySelector(".cmx-camt-create-choices") : null;
		...
		var createCancel = e.target.closest(".cmx-camt-create-cancel");
		if (createCancel) {
			e.preventDefault();
			var cancelWrap = createCancel.closest(".cmx-camt-create-wrap");
			var cancelChoices = cancelWrap ? cancelWrap.querySelector(".cmx-camt-create-choices") : null;
			var cancelledTrigger = cancelWrap ? cancelWrap.querySelector(".cmx-camt-create-trigger") : null;
			if (cancelChoices) {
				cancelChoices.hidden = true;
			}
			if (cancelledTrigger) {
				cancelledTrigger.hidden = false;
			}
			return;
		}
		...
		if (createBtn) {
			...
			var belegDirection = createBtn.getAttribute("data-beleg-direction") || "";
			if (!signature || (belegDirection !== "ausgang" && belegDirection !== "eingang")) return;
			var amountWindow = getPanelAmountWindow(createBtn);
			var contactId = getPanelContactId(createBtn);
			...
			fd.append("amount_window", String(amountWindow));
			fd.append("contact_id", String(contactId || 0));
			fd.append("beleg_direction", belegDirection);
			fd.append("amount_min", getPanelAmountFilter(createBtn, "min"));
			fd.append("amount_max", getPanelAmountFilter(createBtn, "max"));
			fetch(ajaxUrl, {method:"POST", body:fd, credentials:"same-origin"})
				.then(function(r){ return r.json(); })
				.then(function(res){
					if (!res || !res.success) {
		...
		if (unassignBtn) {
			...
			var contactId = getPanelContactId(unassignBtn);
			...
			fd.append("amount_window", String(amountWindow));
			fd.append("contact_id", String(contactId || 0));
			fd.append("amount_min", getPanelAmountFilter(unassignBtn, "min"));
			fd.append("amount_max", getPanelAmountFilter(unassignBtn, "max"));
			fetch(ajaxUrl, {method:"POST", body:fd, credentials:"same-origin"})
				.then(function(r){ return r.json(); })
				.then(function(res){
					if (!res || !res.success) {
		...
		var contactResult = e.target.closest(".cmx-camt-contact-result");
		if (contactResult) {
			e.preventDefault();
			chooseContactResult(contactResult);
			return;
		}

		var contactClear = e.target.closest(".cmx-camt-contact-clear");
		if (contactClear) {
			e.preventDefault();
			var signature = contactClear.getAttribute("data-signature") || "";
			if (signature) {
				loadDetail(
					signature,
					getPanelAmountWindow(contactClear),
					0,
					getPanelAmountFilter(contactClear, "min"),
					getPanelAmountFilter(contactClear, "max")
				);
			}
			return;
		}

		if (!e.target.closest(".cmx-camt-contact-box")) {
			document.querySelectorAll(".cmx-camt-contact-input").forEach(function(inputEl){
				setContactResultsOpen(inputEl, false);
			});
		}
	});
	...
	document.addEventListener("input", function(e){
		var contactInput = e.target.closest(".cmx-camt-contact-input");
		if (!contactInput) return;
		window.clearTimeout(contactSearchTimer);
		contactSearchTimer = window.setTimeout(function(){
			searchContacts(contactInput);
		}, 220);
	});

	function applyCandidateAmountFilters(inputEl){
		var box = inputEl ? inputEl.closest(".cmx-camt-contact-box") : null;
		var signature = box ? String(box.getAttribute("data-signature") || "") : "";
		if (!signature) return;
		loadDetail(
			signature,
			getPanelAmountWindow(inputEl),
			getPanelContactId(inputEl),
			getPanelAmountFilter(inputEl, "min"),
			getPanelAmountFilter(inputEl, "max")
		);
	}

	document.addEventListener("change", function(e){
		var amountFilter = e.target.closest(".cmx-camt-amount-min, .cmx-camt-amount-max");
		if (amountFilter) {
			applyCandidateAmountFilters(amountFilter);
		}
	});

	document.addEventListener("keydown", function(e){
		var amountFilter = e.target.closest(".cmx-camt-amount-min, .cmx-camt-amount-max");
		if (amountFilter && e.key === "Enter") {
			e.preventDefault();
			applyCandidateAmountFilters(amountFilter);
			return;
		}
		var contactInput = e.target.closest(".cmx-camt-contact-input");
		if (!contactInput) return;
		var results = contactResults(contactInput);
		var isOpen = results && results.style.display !== "none";
		if (e.key === "ArrowDown" || e.key === "ArrowUp") {
			e.preventDefault();
			if (!isOpen && results && results.children.length) {
				setContactResultsOpen(contactInput, true);
			}
			var items = results ? Array.prototype.slice.call(results.querySelectorAll(".cmx-camt-contact-result")) : [];
			if (!items.length) return;
			var current = items.indexOf(activeContactResult(results));
			setActiveContactResult(results, e.key === "ArrowDown" ? current + 1 : current - 1, contactInput);
			return;
		}
		if (e.key === "Enter") {
			if (!isOpen) return;
			var active = activeContactResult(results);
			if (active) {
				e.preventDefault();
				chooseContactResult(active);
			}
			return;
		}
		if (e.key === "Escape") {
			if (isOpen) {
				e.preventDefault();
				setContactResultsOpen(contactInput, false);
			}
		}
	});
...
\add_action('wp_ajax_cmx_camt_search_contacts', function (): void {
	if (!cmx_camt_current_user_can()) {
		\wp_send_json_error(['message' => 'forbidden'], 403);
	}
	\check_ajax_referer('cmx_camt_ajax', 'nonce');
	...
	\wp_send_json_success([
		'results' => cmx_camt_contact_search_results($query),
	]);
});
...
\add_action('wp_ajax_cmx_camt_load_candidates', function (): void {
	...
	$amount_min = cmx_camt_candidate_amount_filter_from_request($_POST['amount_min'] ?? null);
	$amount_max = cmx_camt_candidate_amount_filter_from_request($_POST['amount_max'] ?? null);
	$state = cmx_camt_state_get();
	$entry = \is_array($state['entries'][$signature] ?? null) ? (array) $state['entries'][$signature] : [];
	...
	\wp_send_json_success(['html' => cmx_camt_render_candidates_panel($entry, $amount_window, $contact_id, $amount_min, $amount_max)]);
});
...
\add_action('wp_ajax_cmx_camt_assign_beleg', function (): void {
	...
	$amount_min = cmx_camt_candidate_amount_filter_from_request($_POST['amount_min'] ?? null);
	$amount_max = cmx_camt_candidate_amount_filter_from_request($_POST['amount_max'] ?? null);
	$assign_amount = cmx_camt_normalize_amount((string) ($_POST['assign_amount'] ?? ''));
	$state = cmx_camt_state_get();
	$entry = \is_array($state['entries'][$signature] ?? null) ? (array) $state['entries'][$signature] : [];
	if ($signature === '' || $beleg_id <= 0 || empty($entry)) {
		\wp_send_json_error(['message' => 'Ungültige Zuordnung.'], 400);
	}
	$result = cmx_camt_assign_entry_to_beleg($signature, $beleg_id, $entry, 'manual', $assign_amount !== '' ? (float) $assign_amount : 0.0);
	if (empty($result['ok'])) {
	...
	\wp_send_json_success(\array_merge(
		...
		[
			'html' => cmx_camt_render_candidates_panel($entry, $amount_window, $contact_id, $amount_min, $amount_max),
		]
	));
});
...
\add_action('wp_ajax_cmx_camt_create_beleg', function (): void {
	...
	$amount_window = cmx_camt_amount_window_from_request($_POST['amount_window'] ?? 0);
	$contact_id = (int) ($_POST['contact_id'] ?? 0);
	$beleg_direction = \sanitize_key((string) ($_POST['beleg_direction'] ?? ''));
	$amount_min = cmx_camt_candidate_amount_filter_from_request($_POST['amount_min'] ?? null);
	$amount_max = cmx_camt_candidate_amount_filter_from_request($_POST['amount_max'] ?? null);
	if (!\in_array($beleg_direction, ['ausgang', 'eingang'], true)) {
		\wp_send_json_error(['message' => 'Bitte Einnahme oder Ausgabe auswählen.'], 400);
	}
	$state = cmx_camt_state_get();
	$entry = \is_array($state['entries'][$signature] ?? null) ? (array) $state['entries'][$signature] : [];
	if ($signature === '' || empty($entry)) {
	...
	$current_assignment = cmx_camt_assignment_get($signature);
	$current_allocations = cmx_camt_assignment_allocations($current_assignment);
	$current_beleg_id = (int) (($current_allocations[0]['beleg_id'] ?? 0));
	...
	$created = cmx_camt_create_new_beleg_from_entry($entry, $contact_id, $beleg_direction);
	if (empty($created['ok'])) {
	...
	$entry_amount = (float) cmx_camt_normalize_amount((string) ($entry['amount'] ?? ''));
	if ($beleg_id > 0) {
		\update_post_meta($beleg_id, cmx_camt_beleg_status_meta_key(), 'offen');
		\delete_post_meta($beleg_id, cmx_camt_beleg_paid_meta_key());
	}
	$result = $beleg_id > 0 ? cmx_camt_assign_entry_to_beleg($signature, $beleg_id, $entry, 'manual', $entry_amount) : [];
	if ($beleg_id <= 0 || empty($result['ok'])) {
...
\add_action('wp_ajax_cmx_camt_toggle_entry_state', function (): void {
	...
	if (!empty(cmx_camt_assignment_allocations(cmx_camt_assignment_get($signature)))) {
