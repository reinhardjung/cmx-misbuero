...
function cmx_camt_beleg_contact_id(int $beleg_id): int {
	...
	foreach (['_cmx_beleg_kontakt_id', '_cmx_beleg_kontakt'] as $meta_key) {
...
function cmx_camt_contact_search_results(string $query): array {
	...
	$posts = [];
	foreach ($terms as $term) {
		$posts = \array_merge($posts, (array) \get_posts(\array_merge($lookup_args, [
			's' => (string) $term,
		])));
	}
	...
	foreach ($terms as $term) {
		foreach (cmx_camt_contact_search_meta_keys() as $meta_key) {
			$meta_query[] = [
				'key'     => $meta_key,
				'value'   => (string) $term,
				'compare' => 'LIKE',
...
function cmx_camt_beleg_ids_by_contact(int $contact_id): array {
	...
	$ids = \get_posts([
		...
		'meta_query'     => [
			'relation' => 'OR',
			[
				'key'     => '_cmx_beleg_kontakt_id',
				'value'   => (string) $contact_id,
				'compare' => '=',
			],
			[
				'key'     => '_cmx_beleg_kontakt',
				'value'   => (string) $contact_id,
				'compare' => '=',
			],
		],
...
function cmx_camt_collect_beleg_candidates(array $entry, float $amount_window = 0.0, int $contact_id = 0): array {
	$ids = [];
	if ($contact_id > 0) {
...
function cmx_camt_render_candidates_panel(array $entry, float $amount_window = 0.0, int $contact_id = 0): string {
	$signature = (string) ($entry['signature'] ?? '');
	$assignment = cmx_camt_assignment_get($signature);
	$allocations = cmx_camt_assignment_allocations($assignment);
	$amount_window = cmx_camt_amount_window_from_request($amount_window);
	$contact_id = $contact_id > 0 ? $contact_id : 0;
	$candidates = cmx_camt_collect_beleg_candidates($entry, $amount_window, $contact_id);
	$entry_amount = (float) cmx_camt_normalize_amount((string) ($entry['amount'] ?? ''));
	...
	echo '<div class="cmx-camt-contact-box" data-signature="' . \esc_attr($signature) . '" data-contact-id="' . \esc_attr((string) $contact_id) . '">';
	...
	echo '<div class="cmx-camt-candidates-head" data-signature="' . \esc_attr($signature) . '" data-amount-window="' . \esc_attr((string) $amount_window) . '" data-contact-id="' . \esc_attr((string) $contact_id) . '">';
	echo '<h3 style="margin:0;">Mögliche Belege <span style="font-weight:400;color:#646970;">Zahlung ' . \esc_html(cmx_camt_format_amount((string) $entry_amount)) . ' CHF, verfügbar ' . \esc_html(cmx_camt_format_amount((string) $remaining_amount)) . ' CHF</span></h3>';
	echo '</div>';
	if (empty($candidates)) {
	...
	foreach ($candidates as $candidate) {
		...
		echo '<label>Dieser Zahlung zuordnen:<input type="number" step="0.01" min="0" class="small-text cmx-camt-assign-amount" value="' . \esc_attr(\number_format($default_amount, 2, '.', '')) . '"' . ($assign_disabled !== '' ? ' disabled' : '') . '> CHF</label>';
...
(function(){
	...
	function loadDetail(signature, amountWindow, contactId){
	...
	function chooseContactResult(item){
		...
		if (signature && contactId > 0) {
			loadDetail(signature, 0, contactId);
		}
	}
...
\add_action('wp_ajax_cmx_camt_load_candidates', function (): void {
	...
	\wp_send_json_success(['html' => cmx_camt_render_candidates_panel($entry, $amount_window, $contact_id)]);
});
...
\add_action('wp_ajax_cmx_camt_assign_beleg', function (): void {
	...
	\wp_send_json_success(\array_merge(
		...
		[
			'html' => cmx_camt_render_candidates_panel($entry, $amount_window, $contact_id),
		]
	));
});
