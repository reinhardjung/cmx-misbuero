...
\add_action('save_post_scanner', function (int $post_id): void {
	...
	$selected_ids = \function_exists(__NAMESPACE__ . '\\cmx_scanner_get_posted_relation_ids')
		? cmx_scanner_get_posted_relation_ids(CMX_SCANNER_REL_ARTIKEL_META)
		: [];
	$has_submitted_value = \function_exists(__NAMESPACE__ . '\\cmx_scanner_has_posted_relation_value')
		? cmx_scanner_has_posted_relation_value(CMX_SCANNER_REL_ARTIKEL_META)
		: \array_key_exists(CMX_SCANNER_REL_ARTIKEL_META, $_POST);
	$has_zero_value = \function_exists(__NAMESPACE__ . '\\cmx_scanner_posted_relation_has_zero')
		? cmx_scanner_posted_relation_has_zero(CMX_SCANNER_REL_ARTIKEL_META)
		: false;
	...
	foreach ($selected_ids as $selected_id) {
		...
		if ($selected_id <= 0 || \get_post_type($selected_id) !== 'artikel') {
			continue;
		}
	...
	if (!empty($valid_ids)) {
		if (\function_exists(__NAMESPACE__ . '\\cmx_scanner_store_relation_ids')) {
			cmx_scanner_store_relation_ids($post_id, CMX_SCANNER_REL_ARTIKEL_META, $valid_ids);
		} else {
			\update_post_meta($post_id, CMX_SCANNER_REL_ARTIKEL_META, \count($valid_ids) === 1 ? (int) $valid_ids[0] : $valid_ids);
		}
		if (\function_exists(__NAMESPACE__ . '\\cmx_scanner_link_docs_to_artikel')) {
			foreach ($valid_ids as $artikel_id) {
				cmx_scanner_link_docs_to_artikel((int) $artikel_id, $doc_ids);
			}
		}
	...
	if (\function_exists(__NAMESPACE__ . '\\cmx_scanner_store_relation_ids')) {
		cmx_scanner_store_relation_ids($post_id, CMX_SCANNER_REL_ARTIKEL_META, [$new_artikel_id]);
	} else {
