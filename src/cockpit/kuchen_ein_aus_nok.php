...
function cmx_render_kuchen_ein_aus_nok(): void {
		...
		if ($q->have_posts()) {
			foreach ($q->posts as $pid) {
				if (
					...
					&& \function_exists(__NAMESPACE__ . '\\cmx_cockpit_post_date')
					&& !cmx_cockpit_date_in_range(cmx_cockpit_post_date((int) $pid), $range)
				) {
					continue;
				}
	...
	$plugin_main = dirname(__DIR__, 2) . '/cmx-misbuero.php';
	$chartjs_url = plugins_url('assets/chart.umd.min.js', $plugin_main);
