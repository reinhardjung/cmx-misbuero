...
$contact_button_url = \trim((string) ($contact['button_url'] ?? ''));
$contact_uses_form = $contact_button_url === '' || \stripos($contact_button_url, 'mailto:') === 0;
$contact_button_new_window = !$contact_uses_form && \wp_http_validate_url($contact_button_url) !== false;
$contact_form_url = $contact_uses_form
	? \add_query_arg('cmx_website_contact', '1', \home_url('/'))
	: '';
$contact_form_token = $contact_uses_form ? Renderer::contact_form_token() : '';
$contact_submission_status = isset($_GET['cmx_contact_status']) && !\is_array($_GET['cmx_contact_status'])
	? \sanitize_key((string) \wp_unslash($_GET['cmx_contact_status']))
	: '';
$contact_was_sent = $contact_submission_status === 'sent';
$contact_has_error = $contact_submission_status === 'error';
...
<body class="mib-website-page">
	...
	<main>
					...
					<div class="mib-card-grid mib-card-grid-three">
						<?php foreach (\array_slice((array) ($services['items'] ?? []), 0, 3) as $item): ?>
							<article class="mib-service-card">
								...
								<a class="mib-service-more" href="#kontakt"><?php echo \esc_html__('Mehr erfahren', 'cmx-misbuero'); ?> →</a>
							</article>
						<?php endforeach; ?>
					</div>
				</div>
		...
		<?php if (!empty($contact['enabled'])): ?>
			<section id="kontakt" class="mib-contact">
				<div class="mib-site-container">
					<div class="mib-contact-panel<?php echo ($contact_was_sent || $contact_has_error) ? ' mib-contact-panel-open' : ''; ?>"<?php echo $contact_uses_form ? ' data-mib-contact-panel' : ''; ?>>
						<div>
						...
						<div class="mib-contact-actions">
							<?php if ($contact_uses_form && !$contact_was_sent): ?>
								<button type="button" class="mib-button mib-button-light" data-mib-contact-open aria-expanded="false" aria-controls="mib-contact-form"><?php echo Icons::render('mail'); ?><?php echo \esc_html((string) ($contact['button_text'] ?? __('Kontakt aufnehmen', 'cmx-misbuero'))); ?></button>
							<?php else: ?>
								<a class="mib-button mib-button-light" href="<?php echo \esc_url($contact_button_url); ?>"<?php echo $contact_button_new_window ? ' target="_blank" rel="noopener noreferrer"' : ''; ?>><?php echo Icons::render('mail'); ?><?php echo \esc_html((string) ($contact['button_text'] ?? __('Kontakt aufnehmen', 'cmx-misbuero'))); ?></a>
							<?php endif; ?>
							<?php if ($phone !== ''): ?>
						...
						<?php if ($contact_uses_form): ?>
							<form id="mib-contact-form" class="mib-contact-form" action="<?php echo \esc_url($contact_form_url); ?>" method="post" data-mib-contact-form<?php echo $contact_has_error ? '' : ' hidden'; ?>>
								<input type="hidden" name="action" value="cmx_website_contact">
								<input type="hidden" name="token" value="<?php echo \esc_attr($contact_form_token); ?>">
								<label class="mib-contact-honeypot" aria-hidden="true">Website<input type="text" name="company_website" value="" tabindex="-1" autocomplete="off"></label>
								<div class="mib-contact-form-row">
									<label><span><?php echo \esc_html__('Vorname', 'cmx-misbuero'); ?></span><input type="text" name="first_name" autocomplete="given-name" required maxlength="100"></label>
									<label><span><?php echo \esc_html__('Nachname', 'cmx-misbuero'); ?></span><input type="text" name="last_name" autocomplete="family-name" required maxlength="100"></label>
									<label><span><?php echo \esc_html__('E-Mail', 'cmx-misbuero'); ?></span><input type="email" name="email" autocomplete="email" required maxlength="190"></label>
									<label><span><?php echo \esc_html__('Telefon', 'cmx-misbuero'); ?></span><input type="tel" name="phone" autocomplete="tel" maxlength="100"></label>
								</div>
								<label class="mib-contact-message"><span><?php echo \esc_html__('Nachricht', 'cmx-misbuero'); ?></span><textarea name="message" rows="5" required maxlength="10000"></textarea></label>
								<div class="mib-contact-form-footer">
									<button type="submit" class="mib-button mib-button-light"><?php echo \esc_html__('Absenden', 'cmx-misbuero'); ?></button>
									<span class="mib-contact-form-status is-error" data-mib-contact-status role="alert"<?php echo $contact_has_error ? '' : ' hidden'; ?>><?php echo $contact_has_error ? \esc_html__('Die Nachricht konnte nicht gesendet werden. Bitte versuchen Sie es erneut.', 'cmx-misbuero') : ''; ?></span>
								</div>
							</form>
							<div class="mib-contact-success" data-mib-contact-success role="status"<?php echo $contact_was_sent ? '' : ' hidden'; ?>><?php echo \esc_html((string) ($contact['thank_you_text'] ?? __('Vielen Dank für die Nachricht – wir kümmern uns darum.', 'cmx-misbuero'))); ?></div>
						<?php endif; ?>
					</div>
				</div>
			</section>
	...
	<?php if ($contact_uses_form && !empty($contact['enabled'])): ?>
		<script>
			(function(){
				var panel = document.querySelector('[data-mib-contact-panel]');
				var openButton = panel ? panel.querySelector('[data-mib-contact-open]') : null;
				var form = panel ? panel.querySelector('[data-mib-contact-form]') : null;
				if (!panel || !openButton || !form) return;

				openButton.addEventListener('click', function(){
					form.hidden = false;
					panel.classList.add('mib-contact-panel-open');
			...
			(function(){
				...
				link.addEventListener('click', function(event){
					...
					fetch(link.getAttribute('data-mib-magic-login-url'), {
						...
						if (!result.ok || !payload || payload.success === false) {
							showStatus(data && data.message ? data.message : 'Der Anmelde-Link konnte nicht gesendet werden.');
							return;
						}
						...
						var message = data && data.message ? data.message : defaultMessage;
						if (data && data.recipient_hint) {
							message += ' – gesendet an ' + data.recipient_hint;
						}
						showStatus(message);
					}).catch(function(){
						showStatus('Der Anmelde-Link konnte nicht gesendet werden.');
					}).finally(function(){
